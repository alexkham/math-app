...
// @academic[formula_callout:Number of Proper Subsets
// $$\\text{number of proper subsets of } A = 2^{|A|} - 1$$
// /set-theory/formulas#number_of_proper_subsets]@
...
// @academic[formula_callout:Cardinality of Power Set
// $$|\\mathcal{P}(A)| = 2^{|A|}$$
// /set-theory/formulas#cardinality_of_power_set]@
...
// Six questions. Four that merely restated a section heading were cut; two were
// invented from confusions the notation section names explicitly.
const faqQuestions = {
  obj1: {
    question: "What is the difference between a subset and a proper subset?",
    answer: "A subset allows equality, a proper subset does not. A ⊆ B holds when every element of A is in B, including the case A = B. A ⊂ B additionally requires at least one element of B to be missing from A. So {1, 2} ⊆ {1, 2} is true, but {1, 2} ⊂ {1, 2} is false.",
    sectionId: "2"
  },
  obj2: {
    question: "Is the empty set a subset of every set?",
    answer: "Yes. The subset rule asks whether every element of the empty set also belongs to the other set. Since the empty set has no elements, there is nothing that could fail the test, so the condition holds automatically. This is called vacuous truth. It follows that ∅ ⊆ A for every set A, including ∅ itself.",
    sectionId: "1"
  },
  obj3: {
    question: "How many proper subsets does a set have?",
    answer: "A set with n elements has 2ⁿ − 1 proper subsets. The full count of subsets is 2ⁿ, since each element is either included or excluded, and proper subsets exclude only one case: the set itself. A set of 3 elements therefore has 8 subsets and 7 proper subsets. The [subset counting table](!/set-theory/formulas) lists more values.",
    sectionId: "4"
  },
  obj4: {
    question: "What is the power set of the empty set?",
    answer: "It is {∅}, a set containing one element. The power set collects every subset, and the empty set has exactly one subset — itself. So 𝒫(∅) is not empty: its cardinality is 1, not 0. Confusing {∅} with ∅ is the standard error here; one is a box holding an empty box, the other is no box at all.",
    sectionId: "5"
  },
  obj5: {
    question: "What is the difference between ∈ and ⊆?",
    answer: "The first is membership, the second is containment. Write 1 ∈ A when 1 is an element of A, and {1} ⊆ A when the whole set {1} sits inside A. The left side of ∈ is an element; the left side of ⊆ is a set. Mixing them is the most common error in set theory, and it runs in both directions.",
    sectionId: "1"
  },
  obj6: {
    question: "Does ⊂ always mean proper subset?",
    answer: "No. This site and most modern texts use ⊂ for proper containment and ⊆ when equality is allowed. But many older and European authors write ⊂ for any subset and reserve ⊊ for the proper case. The symbol alone is not enough — check how a given text defines it before trusting the distinction. See [all set theory symbols](!/math-symbols/set-theory).",
    sectionId: "2"
  }
}
