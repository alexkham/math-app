    ...
    entries: [
      {
        id: 'delta-increment',
        tex: `$\\Delta x$ · $\\Delta y$`,
        read: `delta x, delta y`,
        means: `Actual change. $\\Delta x = x_2 - x_1$, and $\\Delta y = f(x + \\Delta x) - f(x)$ is what the function really does over that step. Exact, no approximation. Capital delta only — the [keyboard](!/keyboard) has $\\Delta$ under Greek letters.`,
        cases: `$\\Delta$ shrinking toward zero is the whole content of the [limit definition](!/calculus/limits) of the derivative. In numerical work it stays finite and is the step size.`,
        alsoWritten: `$h$ for $\\Delta x$ in limit definitions, where the delta is cumbersome inside a fraction.`,
        confusedWith: `Lowercase $\\delta$ is not this. It is the tolerance in an epsilon-delta argument, or a variation in the calculus of variations — a quantity you choose, not a change you measure.`,
        sameGlyphElsewhere: ``,
      },
      {
        id: 'dx',
        tex: `$dx$`,
        read: `dee x`,
        means: `The differential of the independent variable — an increment you assign. $dx = \\Delta x$ exactly, by definition. It is not infinitely small, and nothing here requires it to be.`,
        cases: `Written large in error estimation, tiny in approximation, symbolic under an [integral sign](!/calculus/integrals). Same object each time; only the size of the number changes.`,
        alsoWritten: `$\\partial x$ once other variables are held fixed.`,
        confusedWith: `The $d$ is not a factor multiplying $x$, and $dx$ is not "d times x". It is one symbol. The substitution rule in [integration](!/calculus/integrals) manipulates it algebraically, which is what makes the reading tempting and wrong.`,
        sameGlyphElsewhere: `The same $d$ opens $\\frac{d}{dx}$ as an operator on [higher-order derivatives](!/calculus/derivatives/higher-order), where it is not a quantity at all.`,
      },
      {
        id: 'dy',
        tex: `$dy$`,
        read: `dee y`,
        means: `The differential of the dependent variable: $dy = f'(x)\\,dx$. Not the actual change in $y$ — the change predicted by the tangent line. It is defined, never measured.`,
        cases: `On a curve, $dy \\neq \\Delta y$ and the gap is the approximation error. On a straight line they are equal for every step size, which is the only case where the distinction disappears.`,
        alsoWritten: `$df$ when the function is named rather than its output.`,
        confusedWith: `$\\Delta y$ is what happened; $dy$ is what the tangent predicted. The section above measures the gap. Treating them as interchangeable is the commonest error with differentials, and it silently discards the entire error term.`,
        sameGlyphElsewhere: ``,
      },
      {
        id: 'dy-over-dx-ratio',
        tex: `$\\frac{dy}{dx}$`,
        read: `dee y by dee x`,
        means: `Everywhere else this is one indivisible symbol. Here, and only here, it is a genuine quotient of two defined quantities — which is what licenses the algebra in the [chain rule](!/calculus/derivatives/rules) and in separation of variables.`,
        cases: `Divide $dy = f'(x)\\,dx$ through by $dx$ and the derivative falls out. That step is legitimate on this page and illegitimate on [Derivatives](!/calculus/derivatives), where $dy$ and $dx$ have not been defined as objects.`,
        alsoWritten: `$df/dx$, $\\frac{d}{dx}f$ — the operator form, which deliberately breaks the quotient reading.`,
        confusedWith: `The permission is local. $\\frac{d^2y}{dx^2}$ is **not** a quotient of $d^2y$ and $dx^2$ — [second-order differentials](!/calculus/derivatives/higher-order) do not cancel the way first-order ones do, and the notation gives no warning.`,
        sameGlyphElsewhere: ``,
      },
    ],
