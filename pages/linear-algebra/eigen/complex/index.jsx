...
// // obj9 — summary capstone: complex-eigenvalue facts at a glance
// // obj9 — recognising the pair, reading it, and what it predicts
// const complexEigenvalues = {
//   kicker: 'Eigen \u00B7 complex eigenvalues',
//   title: 'Reading a conjugate pair',
//   tallyLabel: 'readings',
//   intro: 'A complex pair is not a failure of the real case \u2014 it is a rotation, and its two parts say by how much and through what angle. The modulus and argument are the whole content, and everything below is one of them applied.',
//   footnote: 'The single idea underneath: a real matrix with eigenvalues $re^{\\pm i\\theta}$ acts on the corresponding real plane as a rotation by $\\theta$ scaled by $r$. Recognition, canonical form and stability are all that statement read at different moments \u2014 which is why the modulus and argument are worth computing before anything else.',
//   groups: [
//     {
//       heading: 'Recognising the pair',
//       identities: [
//         {
//           name: 'When they appear',
//           anchor: '#1',
//           formula: '$\\operatorname{tr}(A)^2 - 4\\det(A) < 0$',
//           condition: '$2 \\times 2$ real matrices',
//           note: 'The discriminant settles it before any solving. A negative value means no real direction is mapped to a multiple of itself \u2014 the transformation turns every vector, so no real eigenvector can exist.',
//         },
//         {
//           name: 'Conjugate pairing',
//           anchor: '#2',
//           formula: '$\\lambda = a + bi \\Rightarrow \\bar{\\lambda} = a - bi$',
//           condition: '$A$ real \u2014 not for complex entries',
//           key: true,
//           note: 'Complex roots of a real polynomial arrive in pairs, and the eigenvectors pair with them: $\\mathbf{v}$ and $\\bar{\\mathbf{v}}$. Nothing has to be computed twice \u2014 finding one half of the pair gives the other by conjugation.',
//         },
//         {
//           name: 'Odd dimension',
//           anchor: '#6',
//           formula: '$n$ odd $\\Rightarrow$ at least one real $\\lambda$',
//           condition: 'real matrices only',
//           note: 'Complex roots come in pairs, so an odd number of them cannot pair off entirely and at least one must be real. This is why a rotation in $\\mathbb{R}^3$ has an axis while a rotation in $\\mathbb{R}^2$ has none \u2014 the leftover real eigenvalue is the axis.',
//         },
//       ],
//     },
//     {
...
//           condition: 'the scaling factor',
//           key: true,
//           note: 'How much the transformation stretches the plane per application. Since $r^2 = \\det$ of the $2 \\times 2$ block, the determinant is the area scaling and the modulus is its square root \u2014 consistent with the [geometric reading](!/linear-algebra/determinants/geometry).',
//         },
//         {
//           name: 'Argument',
//           anchor: '#3',
//           formula: '$\\theta = \\arctan(b/a)$',
//           condition: 'the rotation angle',
//           note: 'How far the plane turns per application. A pair with $a = 0$ is a quarter turn; the sign of $b$ picks the direction, and the two conjugates give the two orientations of the same rotation.',
//         },
//         {
//           name: 'Real canonical form',
//           anchor: '#5',
//           formula: '$\\begin{bmatrix} a & -b \\\\ b & a \\end{bmatrix} = r \\cdot R(\\theta)$',
//           condition: 'similar over $\\mathbb{R}$, no complex arithmetic',
//           note: 'The pair does not have to be handled in $\\mathbb{C}$ at all. Over $\\mathbb{R}$ the matrix is similar to a scaled rotation block, so the whole analysis stays real \u2014 which is what makes complex eigenvalues usable in applied work.',
//         },
//       ],
//     },
//     {
//       heading: 'What the pair predicts',
//       identities: [
//         {
//           name: 'Discrete stability',
//           anchor: '#7',
//           formula: '$|\\lambda| < 1$ for every eigenvalue',
//           condition: 'for $\\mathbf{x}_{k+1} = A\\mathbf{x}_k$',
//           strict: true,
//           note: 'The modulus alone decides whether iteration spirals inward or outward; the argument only sets how fast it turns. Note $|\\lambda| = 1$ is the boundary case \u2014 a pure rotation that neither grows nor decays.',
//         },
//         {
//           name: 'Continuous stability',
//           anchor: '#7',
//           formula: '$\\operatorname{Re}(\\lambda) < 0$ for every eigenvalue',
//           condition: 'for $\\mathbf{x}\' = A\\mathbf{x}$',
//           strict: true,
//           note: 'A different criterion for a different system, and confusing the two is the usual error. Discrete systems compare the modulus against one; continuous systems compare the real part against zero \u2014 because the solution is $e^{\\lambda t}$ and its growth is governed by $\\operatorname{Re}(\\lambda)$.',
//         },
//       ],
//     },
//   ],
...
//             title="Reading a conjugate pair"
//             source="/linear-algebra/eigen/complex"
//           >
//             <IdentitySheet data={complexEigenvalues} theme="navy" variant="ledger" />
//           </DiagramFrame>,
//           `The real canonical form is what keeps this practical. A conjugate pair can be handled entirely in $\\mathbb{R}$: the matrix is similar to a block $\\begin{bmatrix} a & -b \\\\ b & a \\end{bmatrix}$, which is a rotation by $\\theta$ scaled by $r$, and no complex arithmetic is required at any point. The complex numbers are a route to the answer rather than a property of it.`,
...
// obj3 — aggregation: modulus regimes and their instantaneous geometric action
const spiralRegimes = {
  kicker: 'Eigen \u00B7 complex pairs',
  title: 'Three regimes, set by the modulus',
  tallyLabel: 'regimes',
  intro: 'The argument $\\theta$ fixes how fast the plane turns and never affects whether it grows. That is decided entirely by the modulus, and the three cases below are the only ones there are.',
  footnote: 'The determinant of the $2 \\times 2$ block is $a^2 + b^2 = r^2$, so the [area scaling](!/linear-algebra/determinants/geometry) and the modulus are the same fact stated twice \u2014 which is why the determinant can be read as a stability marker without computing an eigenvalue at all.',
  slots: [
    { key: 'action',  label: 'geometric action' },
    { key: 'det',     label: 'det of the 2\u00D72 block' },
    { key: 'orbit',   label: 'orbit of a point' },
  ],
  groups: [
    {
      heading: 'Neither growing nor decaying',
      types: [
        {
          name: 'On the unit circle',
          anchor: '#3',
          shape: 'antidiagonal',
          condition: '$r = 1$',
          properties: {
            action: 'pure rotation by $\\theta$, no scaling',
            det: '$\\det A = 1$',
            orbit: 'a closed circle',
          },
          note: 'Lengths are preserved exactly, so the matrix is [orthogonal](!/linear-algebra/matrix/types). Every orbit closes if $\\theta$ is a rational multiple of $2\\pi$ and fills the circle densely otherwise \u2014 the boundary case in every stability argument.',
        },
      ],
    },
    {
      heading: 'Growing or decaying',
      types: [
        {
          name: 'Outside the unit circle',
          anchor: '#7',
          shape: 'dense',
          condition: '$r > 1$',
          properties: {
            action: 'rotation with outward scaling by $r$',
            det: '$\\det A > 1$',
            orbit: 'a spiral outward',
          },
          note: 'Each application turns by $\\theta$ and stretches by $r$, so iterates diverge while circling. In a discrete system this is instability \u2014 and the divergence is geometric, so it is fast.',
        },
        {
          name: 'Inside the unit circle',
          anchor: '#7',
          shape: 'block',
          condition: '$r < 1$',
          properties: {
            action: 'rotation with inward scaling by $r$',
            det: '$\\det A < 1$',
            orbit: 'a spiral inward to the origin',
          },
          note: 'The stable case for iteration: every orbit converges to the origin, still turning as it goes. This is what $|\\lambda| < 1$ means geometrically, and why the condition is stated on the modulus rather than on the real part.',
        },
      ],
    },
  ],
