...
// obj4 - the naive route the closed form replaces: A^k by repeated multiplication
const matrixPowersByHand = {
  kicker: 'Eigen \u00B7 matrix powers',
  title: 'Four powers, computed the long way',
  intro: 'Each state is the previous one multiplied by $A$ again. Three multiplications to reach $A^4$, and every one of them depends on the one before it.',
  operation: 'power',
  A: [[1, 2], [4, 3]],
  powers: 4,
  nameA: 'A',
  anchor: '#2',
  noteLabel: 'What the invariant columns show',
  note: 'The determinant runs $(-5)^k$ and the trace runs $5^k + (-1)^k$ \u2014 the eigenvalues of $A$ raised to the power, and nothing else. Neither reads the entries. That is why a closed form exists at all: the growth was never a property of the sixteen numbers on display, only of the two hiding behind them.',
}
  ...
  const genericSections=[
    ...
    },
    {
        ...
        content:[
          ...
          <DiagramFrame
            key={'obj4-diagram'}
            id="matrix-powers-by-hand"
            title="Four powers, computed the long way"
            source="/linear-algebra/eigen/diagonalization"
          >
            <MatrixOperation data={matrixPowersByHand} theme="navy" variant="iterate" />
          </DiagramFrame>,
