...
// export default function SubspacesPage({seoData, sectionsContent, introContent, obj7Table, obj8Table, summaryTable, faqQuestions, schemas}) {
...
// obj10 — summary capstone: subspace verdict landscape
// obj10 — candidates sorted by which clause of the test they pass or fail
const subspaceRecognition = {
  kicker: 'Vector spaces \u00B7 recognition',
  title: 'Subspace or not, and which clause decides',
  tallyLabel: 'candidates',
  intro: 'The [test](#2) has three clauses \u2014 contains $\\mathbf{0}$, closed under addition, closed under scaling \u2014 and a candidate fails by failing one of them. Sorting by which clause is what turns a list of examples into a diagnostic.',
  footnote: 'Note the near-misses are near in different ways. An affine set is closed under nothing that matters because it misses the origin; the first quadrant contains the origin and survives addition but not negative scaling. Both are the same distance from being a subspace and fail for unrelated reasons \u2014 which is why all three clauses have to be checked rather than the one that looks likeliest.',
  groups: [
    {
      heading: 'Passes \u2014 by definition or trivially',
      identities: [
        {
        ...
        },
        {
          ...
          anchor: '#3',
          formula: '$\\{\\mathbf{0}\\}$',
          condition: '$\\dim = 0$; the smallest',
          strict: true,
          note: 'All three clauses hold vacuously: $\\mathbf{0} + \\mathbf{0} = \\mathbf{0}$ and $c\\mathbf{0} = \\mathbf{0}$. Its dimension is zero, not one \u2014 the [empty set is its basis](!/linear-algebra/vector-spaces/dimension), since $\\mathbf{0}$ is dependent on its own and cannot belong to one.',
        },
        {
          name: 'Any span',
          anchor: '#4',
          formula: '$\\operatorname{Span}\\{\\mathbf{v}_1, \\ldots, \\mathbf{v}_k\\}$',
          condition: 'automatically a subspace',
          key: true,
          note: 'A combination of combinations is a combination, so closure is immediate. This is the cheapest way to prove something is a subspace \u2014 exhibit it as a span and the test is already passed. Lines and planes through the origin are the $k = 1$ and $k = 2$ cases.',
        },
      ],
    },
    {
      heading: 'Passes \u2014 because linearity forces it',
      identities: [
        {
          name: 'Null space',
          anchor: '#5',
          formula: '$\\operatorname{Null}(A) = \\{\\mathbf{x} : A\\mathbf{x} = \\mathbf{0}\\}$',
          condition: 'subspace of $\\mathbb{R}^n$',
          key: true,
          note: 'If $A\\mathbf{u} = \\mathbf{0}$ and $A\\mathbf{v} = \\mathbf{0}$ then $A(c\\mathbf{u} + d\\mathbf{v}) = \\mathbf{0}$ \u2014 one line, both closure clauses. The [homogeneous](!/linear-algebra/linear-systems/homogeneous) right-hand side is what makes it work, which is exactly why $A\\mathbf{x} = \\mathbf{b}$ below fails.',
        },
        {
          name: 'Column and row spaces',
          anchor: '#6',
          formula: '$\\operatorname{Col}(A)$, $\\operatorname{Row}(A)$',
          condition: 'spans of the columns and rows',
          note: 'Both are spans, so they inherit the result above without further argument. Together with the null space and left null space these are the [four fundamental subspaces](!/linear-algebra/vector-spaces/fundamental-spaces).',
        },
      ],
    },
    {
      heading: 'Fails \u2014 and which clause',
      identities: [
        {
          name: 'Line or plane off the origin',
          anchor: '#4',
          formula: '$\\{\\mathbf{x}_0 + t\\mathbf{v}\\}$, $\\mathbf{x}_0 \\notin \\operatorname{Span}\\{\\mathbf{v}\\}$',
          condition: 'fails clause 1 \u2014 no $\\mathbf{0}$',
          strict: true,
          note: 'Affine rather than linear. The origin test is the cheapest of the three and should be run first \u2014 it disqualifies this immediately, with no closure argument needed.',
        },
        {
          name: 'Solution set of $A\\mathbf{x} = \\mathbf{b}$',
          anchor: '#9',
        ...
        },
        {
          ...
          anchor: '#2',
          formula: '$\\{(x, y) : x \\geq 0,\\, y \\geq 0\\}$',
          condition: 'fails clause 3 only',
          strict: true,
          note: 'Contains the origin and is closed under addition \u2014 two clauses out of three. Multiplying by $-1$ leaves the set, and that alone disqualifies it. The instructive case: passing most of the test is not passing it.',
        },
      ],
    },
  ],
    ...
    },
    {
        ...
        content:[
          ...
          <DiagramFrame
            ...
            title="Subspace or not, and which clause decides"
            source="/linear-algebra/vector-spaces/subspaces"
          >
            <IdentitySheet data={subspaceRecognition} theme="navy" variant="ledger" />
          </DiagramFrame>,
          `In practice the origin test comes first because it is nearly free. Two of the three failures below are settled by it alone — an affine line and the solution set of $A\\mathbf{x} = \\mathbf{b}$ both miss $\\mathbf{0}$, and neither needs a closure argument. Running the clauses in the order cheap-to-expensive means most non-subspaces are disqualified before any real work.`,
