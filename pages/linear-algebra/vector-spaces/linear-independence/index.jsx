...
//   obj1: {
//     title: `Definition`,
//     content: `A set of vectors $\\{\\mathbf{v}_1, \\mathbf{v}_2, \\dots, \\mathbf{v}_k\\}$ in a vector space $V$ is linearly independent when the only linear combination producing the zero vector is the trivial one:

// @academic[formula_callout:Linear Independence Equation
...
// /linear-algebra/formulas#linear_independence_equation]@

// @academic[formulas_link:Browse all linear algebra formulas
// /linear-algebra/formulas]@

// If a nontrivial solution exists — any assignment of scalars, not all zero, that produces $\\mathbf{0}$ — the set is linearly dependent.
...
// @academic[formula_callout:Linear Independence Matrix Test
...
// /linear-algebra/formulas#linear_independence_matrix_test]@
...
//   obj4: {
//     title: `Testing Independence: The Determinant`,
//     content: `When the number of vectors equals the dimension of the space, the independence question reduces to a single number. Arrange $n$ vectors in $\\mathbb{R}^n$ as columns of an $n \\times n$ matrix $A$. The set is independent precisely when this matrix has nonzero determinant:

// @academic[formula_callout:Linear Independence Determinant Test
// $$\\{\\mathbf{v}_1, \\ldots, \\mathbf{v}_n\\} \\subset \\mathbb{R}^n \\text{ is independent} \\iff \\det[\\mathbf{v}_1\\ \\cdots\\ \\mathbf{v}_n] \\neq 0$$
// /linear-algebra/formulas#linear_independence_determinant_test]@
...
// @academic[formula_callout:Wronskian Test
...
// /linear-algebra/formulas#wronskian_test]@
...
// obj5 — aggregation: behavior of independence under set operations
// obj5 — what survives an operation on an independent set, and what does not
const independenceProperties = {
  kicker: 'Vector spaces \u00B7 independence',
  title: 'What operations do to an independent set',
  tallyLabel: 'operations',
  intro: 'Independence is fragile in one direction and robust in the other: removing vectors can never break it, adding them usually does. The two entries that read &quot;may or may not&quot; are the ones worth reading closely.',
  footnote: 'The asymmetry has a single source. Independence is a statement that no nontrivial combination gives $\\mathbf{0}$, so a smaller set has fewer combinations to worry about and cannot acquire a relation it did not have. Adding a vector adds combinations, and only one of them needs to vanish for the property to be lost.',
  laws: [
    {
      name: 'Any subset',
      anchor: '#5',
      statement: '$S$ independent $\\Rightarrow$ every $T \\subseteq S$ independent',
      verdict: 'holds',
      requires: 'any subset, including the empty one',
      note: 'Removing vectors can never create a dependence. A nontrivial combination among the survivors would have been a nontrivial combination in the original set, with zero coefficients on the removed vectors \u2014 so the original was never independent to begin with.',
    },
    {
      name: 'Adding a vector outside the span',
      anchor: '#5',
      statement: '$\\mathbf{v} \\notin \\operatorname{span}(S) \\Rightarrow S \\cup \\{\\mathbf{v}\\}$ independent',
      verdict: 'conditional',
      holdsWhen: '$\\mathbf{v}$ lies outside $\\operatorname{span}(S)$',
      note: 'A genuinely new direction extends the set safely. This is the step that builds a [basis](!/linear-algebra/vector-spaces/basis) one vector at a time \u2014 keep adding from outside the current span until the span is everything.',
    },
    {
      name: 'Adding a vector inside the span',
      anchor: '#5',
      statement: '$\\mathbf{v} \\in \\operatorname{span}(S) \\Rightarrow S \\cup \\{\\mathbf{v}\\}$ dependent',
      verdict: 'fails',
      failsWhen: '$\\mathbf{v}$ is any combination of $S$',
      note: 'The new vector is already expressible, and writing it as a combination and moving everything to one side gives a nontrivial relation immediately. Note this includes $\\mathbf{v} = \\mathbf{0}$, which lies in every span \u2014 which is why no independent set can contain the zero vector.',
    },
    {
      name: 'Exceeding the dimension',
      anchor: '#5',
      statement: '$|S| > n$ in an $n$-dimensional space $\\Rightarrow$ dependent',
      verdict: 'fails',
      failsWhen: 'more vectors than dimensions',
      note: 'No counting or computation required \u2014 the conclusion follows from the size alone. This is the fastest of all the [independence tests](#9), and the reason a homogeneous system with more unknowns than equations always has a nontrivial solution.',
    },
    {
      name: 'Reaching the dimension',
      anchor: '#5',
      statement: '$|S| = n$ and $S$ independent $\\Rightarrow S$ is a basis',
      verdict: 'conditional',
      holdsWhen: 'the space has dimension exactly $n$',
      note: 'Spanning comes free at the maximum independent size \u2014 no separate check needed. The converse shortcut also holds: $n$ vectors that span are automatically independent. Either property plus the right count gives the other.',
    },
    {
      name: 'Removing from a dependent set',
      anchor: '#6',
      statement: 'dependence may or may not be repaired',
      verdict: 'conditional',
      verdictLabel: 'Depends which',
      holdsWhen: 'the removed vector participates in every dependence relation',
      failsWhen: 'other relations survive without it',
      commonError: 'assuming any removal restores independence',
      note: 'Removing a vector that appears in the only relation repairs the set; removing one that does not leaves the relation intact. Which vectors are candidates is read off the [dependence relation](#6) itself, not guessed.',
      witness: {
        label: 'Both outcomes',
        lines: [
          'S = {(1,0), (2,0), (0,1)} \u2014 dependent, one relation',
    ...
    },
    {
        ...
        content:[
          ...
          <DiagramFrame
            ...
            title="What operations do to an independent set"
            source="/linear-algebra/vector-spaces/linear-independence"
          >
            <PropertyLawCard data={independenceProperties} theme="navy" />
          </DiagramFrame>,
        ]
    },
    {
