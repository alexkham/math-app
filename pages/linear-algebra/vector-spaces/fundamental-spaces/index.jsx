...
// // //       "Unified four-subspace reference card by ambient space, dimension, and basis method"
// // //     ],
...
// // // export default function FundamentalSubspacesPage({seoData, sectionsContent, introContent, obj7Table, obj9Table, summaryTable, faqQuestions, schemas}) {
...
// // // obj10 — summary capstone: the four-subspace structural reference card
// // // obj10 — the four subspaces, paired by the space they live in
// // const fourSubspaceCard = {
// //   kicker: 'Vector spaces \u00B7 reference card',
// //   title: 'The four fundamental subspaces',
// //   tallyLabel: 'subspaces',
// //   intro: 'Two live in $\\mathbb{R}^n$ and two in $\\mathbb{R}^m$, and within each pair they are orthogonal complements. Every entry gives its dimension and how to compute a basis \u2014 all four come out of the same row reduction.',
// //   footnote: 'One reduction of $A$ yields all four bases. The pairing is what makes the picture close: $\\operatorname{Row}(A)$ and $\\operatorname{Null}(A)$ split the domain, $\\operatorname{Col}(A)$ and $\\operatorname{Null}(A^{\\mathsf{T}})$ split the codomain, and the two dimensions they share is the [rank](!/linear-algebra/matrix/rank) \u2014 which is why row rank equals column rank.',
// //   groups: [
// //     {
// //       heading: 'In the domain \u2014 \u211D\u207F',
// //       identities: [
// //         {
// //           name: 'Row space',
// //           anchor: '#3',
// //           formula: '$\\operatorname{Row}(A) = \\operatorname{Col}(A^{\\mathsf{T}})$',
// //           condition: 'dimension $r$ \u2014 the rank',
// //           note: 'Span of the rows. Row-reduce $A$ and take the **nonzero rows of the echelon form** \u2014 row operations preserve the row space exactly, which is why the reduced rows still span it. Note the contrast with the column space, where the echelon form must not be used.',
// //         },
// //         {
// //           name: 'Null space',
// //           anchor: '#4',
// //           formula: '$\\operatorname{Null}(A) = \\{\\mathbf{x} : A\\mathbf{x} = \\mathbf{0}\\}$',
// //           condition: 'dimension $n - r$ \u2014 the nullity',
// //           note: 'Everything the matrix sends to zero. Reduce to RREF and set each free variable to $1$ in turn, the others to $0$ \u2014 one basis vector per free variable, which is where the count $n - r$ comes from.',
// //         },
// //       ],
// //     },
// //     {
// //       heading: 'In the codomain \u2014 \u211D\u1D50',
// //       identities: [
// //         {
// //           name: 'Column space',
// //           anchor: '#2',
...
// //           anchor: '#5',
// //           formula: '$\\operatorname{Null}(A^{\\mathsf{T}}) = \\{\\mathbf{y} : A^{\\mathsf{T}}\\mathbf{y} = \\mathbf{0}\\}$',
// //           condition: 'dimension $m - r$',
// //           note: 'The directions no output ever reaches. Row-reduce $A^{\\mathsf{T}}$, or augment as $[A^{\\mathsf{T}} \\mid I_m]$ to get the combining coefficients as well. Named &quot;left&quot; because $A^{\\mathsf{T}}\\mathbf{y} = \\mathbf{0}$ is the same as $\\mathbf{y}^{\\mathsf{T}}A = \\mathbf{0}^{\\mathsf{T}}$.',
// //         },
// //       ],
// //     },
// //     {
// //       heading: 'What ties the pairs together',
// //       identities: [
// //         {
// //           name: 'Domain split',
// //           anchor: '#6',
// //           anchorLabel: 'Dimension accounting',
// //           formula: '$r + (n - r) = n$',
// //           condition: 'rank\u2013nullity',
// //           key: true,
// //           note: 'The row space and null space are orthogonal complements in $\\mathbb{R}^n$, so their dimensions sum to $n$ and they meet only at the origin. Every vector in the domain splits uniquely into a part each.',
// //         },
// //         {
// //           name: 'Codomain split',
// //           anchor: '#7',
// //           anchorLabel: 'Orthogonal complements',
// //           formula: '$r + (m - r) = m$',
// //           condition: 'the transpose statement',
// //           key: true,
// //           note: 'The same accounting on the other side: column space and left null space are orthogonal complements in $\\mathbb{R}^m$. The shared $r$ is what makes the two splits one theorem rather than two.',
// //         },
// //       ],
// //     },
// //   ],
...
// // export default function FundamentalSubspacesPage({seoData, sectionsContent, introContent, obj7Table, obj9Table, fourSubspaceCard, faqQuestions, schemas}) {
...
// //           <DiagramFrame
// //             key={'obj10-diagram'}
// //             id="four-subspace-card"
// //             title="The four fundamental subspaces"
// //             source="/linear-algebra/vector-spaces/fundamental-spaces"
// //           >
// //             <IdentitySheet data={fourSubspaceCard} theme="navy" variant="ledger" />
// //           </DiagramFrame>,
// //         ]
// //     },
// //     // {
...
// // obj9 — aggregation: matrix types × four subspaces
// const subspacesByType = {
//   kicker: 'Vector spaces \u00B7 worked cases',
//   title: 'The four subspaces, by matrix type',
//   tallyLabel: 'cases',
//   intro: 'Three matrices with the same four subspaces computed for each. The point is the pattern across them \u2014 how much of each space collapses as the rank falls.',
//   footnote: 'Rank is the only variable. At $r = n$ nothing is lost and both null spaces are trivial; at $r = 1$ almost everything is lost and the null spaces are hyperplanes. Every row obeys $r + \\text{nullity} = n$ on the domain side and $r + \\text{left nullity} = m$ on the codomain side \u2014 the [dimension accounting](#6) with three sets of numbers substituted in.',
//   slots: [
//     { key: 'col',     label: 'Col(A)' },
//     { key: 'row',     label: 'Row(A)' },
//     { key: 'nul',     label: 'Null(A)' },
//     { key: 'leftNul', label: 'Null(A\u1D40)' },
//   ],
//   groups: [
//     {
//       heading: 'Nothing collapses',
//       types: [
//         {
...
//           condition: '$r = n = m$',
//           properties: {
//             col: 'all of $\\mathbb{R}^n$',
//             row: 'all of $\\mathbb{R}^n$',
//             nul: '$\\{\\mathbf{0}\\}$',
//             leftNul: '$\\{\\mathbf{0}\\}$',
//           },
//           note: 'Both null spaces are trivial, which is the same statement as invertibility \u2014 see the [equivalence](!/linear-algebra/matrix/inverse). The map reaches everything and destroys nothing, so the four-subspace picture degenerates to two full spaces.',
//         },
//       ],
//     },
//     {
//       heading: 'Almost everything collapses',
//       types: [
//         {
//           name: 'Rank one',
//           anchor: '#9',
//           shape: 'dense',
//           condition: '$A = \\mathbf{u}\\mathbf{v}^{\\mathsf{T}}$, $r = 1$',
//           properties: {
//             col: 'the line through $\\mathbf{u}$',
//             row: 'the line through $\\mathbf{v}$',
//             nul: 'hyperplane, $\\dim = n - 1$',
//             leftNul: 'hyperplane, $\\dim = m - 1$',
//           },
//           note: 'The opposite extreme. One direction survives in each space and everything else collapses \u2014 the null space is the entire hyperplane orthogonal to $\\mathbf{v}$, which is as much as a nonzero matrix can destroy.',
//         },
//       ],
//     },
//     {
//       heading: 'Collapse split evenly',
//       types: [
//         {
//           name: 'Symmetric projection',
//           anchor: '#9',
//           shape: 'block',
//           condition: '$A^2 = A$, $A = A^{\\mathsf{T}}$',
//           properties: {
//             col: 'the target subspace $S$',
//             row: 'the same as $\\operatorname{Col}(A)$',
//             nul: '$S^{\\perp}$',
//             leftNul: 'the same as $\\operatorname{Null}(A)$',
//           },
//           note: 'Symmetry collapses the four spaces into two: row space equals column space, and left null equals null. This is the only row where that happens, and it is what makes a [projection](!/linear-algebra/orthogonality/projections) decompose $\\mathbb{R}^n$ into $S \\oplus S^{\\perp}$ so cleanly.',
//         },
//       ],
//     },
//   ],
...
//           <DiagramFrame
//             key={'obj9-diagram'}
//             id="subspaces-by-type"
//             title="The four subspaces, by matrix type"
//             source="/linear-algebra/vector-spaces/fundamental-spaces"
//           >
//             <ObjectTypeProfile data={subspacesByType} theme="navy" variant="grid" />
//           </DiagramFrame>,
//           `The third row is the one that does something the others do not. Symmetry makes $\\operatorname{Row}(A) = \\operatorname{Col}(A)$ and $\\operatorname{Null}(A^{\\mathsf{T}}) = \\operatorname{Null}(A)$, so the four subspaces become two — and the two that remain are orthogonal complements of each other. That is exactly the decomposition $\\mathbb{R}^n = S \\oplus S^{\\perp}$ a projection performs, read off the subspace structure rather than from the projection formula.`,
//         ]
//     },
//     {
//         id:'10',
