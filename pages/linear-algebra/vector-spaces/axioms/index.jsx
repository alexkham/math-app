...
//       "How the axioms instantiate across the canonical vector spaces"
//     ],
...
// export default function VectorSpaceAxiomsPage({seoData, sectionsContent, introContent, obj2Table, obj8Table, obj9Table, summaryTable, faqQuestions, schemas}) {
...
// obj2 — aggregation: the ten axioms (reference card)
// obj2 — the ten axioms, grouped by what each one governs
const vectorSpaceAxioms = {
  kicker: 'Vector spaces \u00B7 definition',
  title: 'The ten axioms',
  tallyLabel: 'axioms',
  intro: 'Five govern addition, four govern scalar multiplication, and one ties the scalar field to the vectors. Nothing here mentions arrows, coordinates or dimension \u2014 that is the point of the list.',
  footnote: 'A set satisfying all ten is a vector space whatever its elements are \u2014 polynomials, matrices, functions, sequences. Everything proved from these applies to all of them at once, which is the whole return on stating them abstractly.',
  groups: [
    {
      heading: 'Addition',
      identities: [
        {
          name: 'Closure',
          anchor: '#8',
          anchorLabel: 'Non-examples',
          formula: '$\\mathbf{u} + \\mathbf{v} \\in V$',
          condition: 'for all $\\mathbf{u}, \\mathbf{v} \\in V$',
          strict: true,
          note: 'Addition may not leave the set. The most frequently violated axiom in practice \u2014 a set can look like a vector space and fail here alone, which is what disqualifies most of the non-examples.',
        },
        {
          name: 'Commutativity',
          formula: '$\\mathbf{u} + \\mathbf{v} = \\mathbf{v} + \\mathbf{u}$',
          condition: 'for all $\\mathbf{u}, \\mathbf{v} \\in V$',
          note: 'Order does not matter. Worth noticing this is assumed rather than derived \u2014 [matrix multiplication](!/linear-algebra/matrix/operations) shows what an operation without it looks like.',
        },
        {
          name: 'Associativity',
          formula: '$(\\mathbf{u} + \\mathbf{v}) + \\mathbf{w} = \\mathbf{u} + (\\mathbf{v} + \\mathbf{w})$',
          condition: 'for all $\\mathbf{u}, \\mathbf{v}, \\mathbf{w} \\in V$',
          note: 'Grouping does not matter, so a sum of many vectors needs no parentheses at all. This is what makes [linear combinations](!/linear-algebra/vectors/linear-combinations) well defined as written.',
        },
        {
          name: 'Zero vector',
          anchor: '#9',
          anchorLabel: 'Consequences',
          formula: '$\\exists\\, \\mathbf{0} \\in V : \\mathbf{v} + \\mathbf{0} = \\mathbf{v}$',
          condition: 'one such element, for every $\\mathbf{v}$',
          note: 'An additive identity. The axiom asserts existence only \u2014 uniqueness is a consequence, proved in two lines by assuming two zeros and adding them to each other.',
        },
        {
          name: 'Additive inverse',
          anchor: '#9',
          anchorLabel: 'Consequences',
          formula: '$\\forall\\, \\mathbf{v} \\;\\exists\\, -\\mathbf{v} : \\mathbf{v} + (-\\mathbf{v}) = \\mathbf{0}$',
          condition: 'one inverse per element',
          note: 'Every vector can be undone. Together with the four above this makes $V$ an abelian group under addition \u2014 the scalar axioms are what turn a group into a vector space.',
        },
      ],
    },
    {
      ...
      identities: [
        {
          ...
          anchor: '#8',
          anchorLabel: 'Non-examples',
          formula: '$c\\mathbf{v} \\in V$',
          condition: 'for all scalars $c$ and $\\mathbf{v} \\in V$',
          strict: true,
          note: 'Scaling may not leave the set either. Checking this and closure under addition is the whole test for whether a subset is a [subspace](!/linear-algebra/vector-spaces/subspaces) \u2014 the other eight are inherited.',
        },
        {
          name: 'Associativity of scalars',
          formula: '$c(d\\mathbf{v}) = (cd)\\mathbf{v}$',
          condition: 'for all scalars $c, d$',
          note: 'Scaling twice equals scaling once by the product. The two multiplications are different operations \u2014 $cd$ happens in the field, $c(d\\mathbf{v})$ happens in $V$ \u2014 and the axiom is the claim that they agree.',
        },
        {
          name: 'Distributivity over vectors',
          formula: '$c(\\mathbf{u} + \\mathbf{v}) = c\\mathbf{u} + c\\mathbf{v}$',
          condition: 'for all scalars $c$',
          note: 'Scaling spreads over a sum of vectors. This and the next axiom are exactly what [linearity](!/linear-algebra/transformations/properties) demands of a transformation \u2014 the definition is these two conditions and nothing more.',
        },
        {
          name: 'Distributivity over scalars',
          formula: '$(c + d)\\mathbf{v} = c\\mathbf{v} + d\\mathbf{v}$',
          condition: 'for all scalars $c, d$',
          note: 'The mirror of the axiom above, spreading over a sum of scalars instead. Both are needed and neither implies the other.',
        },
      ],
    },
    {
      heading: 'Tying the two together',
      identities: [
        {
          name: 'Multiplicative identity',
          anchor: '#3',
          anchorLabel: 'Scalar field',
          formula: '$1\\mathbf{v} = \\mathbf{v}$',
          condition: '$1$ the identity of the scalar field',
          strict: true,
          key: true,
          note: 'The one that looks redundant and is not. Define scaling so that every $c\\mathbf{v}$ equals $\\mathbf{0}$ and the other nine axioms all hold \u2014 this axiom is the only thing ruling that out, and it is what ties $V$ to its field of scalars.',
        },
      ],
    },
  ],
...
export default function VectorSpaceAxiomsPage({seoData, sectionsContent, introContent, vectorSpaceAxioms, obj8Table, obj9Table, summaryTable, faqQuestions, schemas}) {
    ...
    },
    {
        ...
        content:[
          ...
          <DiagramFrame
            ...
            title="The ten axioms"
            source="/linear-algebra/vector-spaces/axioms"
          >
