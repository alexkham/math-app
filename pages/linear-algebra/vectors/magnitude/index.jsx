...
//       "Vector normalization",
//       "Family tree of concepts derived from the norm"
//     ],
...
// }
//    }
// export default function VectorMagnitudePage({seoData, sectionsContent, introContent, obj3Table, summaryTable, faqQuestions, schemas}) {
...
// obj3 — aggregation: the three structural properties of the Euclidean norm
// obj3 — the three norm axioms plus the identities that follow from them
const normProperties = {
  kicker: 'Vectors \u00B7 magnitude',
  title: 'Properties of the norm',
  tallyLabel: 'properties',
  intro: 'Three axioms define a norm; everything below them is derived. Watch the equality cases \u2014 each one identifies a geometric configuration, not just an edge case in the algebra.',
  footnote: 'Any function satisfying the first three is a norm, and the Euclidean length is only one of them. The [taxicab and maximum norms](!/linear-algebra/orthogonality/inner-product) satisfy all three while measuring distance differently \u2014 which is why the axioms are stated separately from the square-root formula that happens to satisfy them.',
  laws: [
    {
      name: 'Non-negativity',
      anchor: '#3',
      statement: '$\\|\\mathbf{v}\\| \\geq 0$',
      verdict: 'conditional',
      holdsWhen: 'equality iff $\\mathbf{v} = \\mathbf{0}$',
      note: 'Length is never negative, and only the zero vector has zero length. The second half is the load-bearing part \u2014 without it a nonzero vector could have length zero and [normalization](#6) would break, since dividing by the magnitude assumes it is nonzero.',
    },
    {
      name: 'Absolute homogeneity',
      anchor: '#3',
      statement: '$\\|c\\mathbf{v}\\| = |c| \\, \\|\\mathbf{v}\\|$',
      verdict: 'holds',
      requires: 'any scalar $c$',
      commonError: 'dropping the absolute value \u2014 $\\|{-2}\\mathbf{v}\\| = 2\\|\\mathbf{v}\\|$, not $-2\\|\\mathbf{v}\\|$',
      note: 'Scaling by $c$ multiplies the length by $|c|$, not by $c$. The absolute value is doing real work: for $c > 0$ the direction is preserved, for $c < 0$ it reverses, and in both cases the length grows by the same factor. A negative length would violate the axiom above.',
    },
    {
      ...
      verdict: 'conditional',
      holdsWhen: 'equality iff $\\mathbf{u}$ and $\\mathbf{v}$ point the same way',
      note: 'The direct path is never longer than the tip-to-tail path. Equality holds exactly when one vector is a non-negative multiple of the other \u2014 the triangle degenerates to a line. This is the axiom that makes [distance](#4) a metric, and it follows from the [Cauchy\u2013Schwarz inequality](!/linear-algebra/orthogonality/inner-product).',
      witness: {
        label: 'Both extremes',
        lines: [
          'u = (3, 0), v = (4, 0):   \u20167\u2016 = 7 = 3 + 4    equality',
    ...
    },
    {
      ...
      anchor: '#3',
      statement: '$\\bigl|\\, \\|\\mathbf{u}\\| - \\|\\mathbf{v}\\| \\,\\bigr| \\leq \\|\\mathbf{u} - \\mathbf{v}\\|$',
      verdict: 'holds',
      requires: 'derived, not assumed',
      note: 'A lower bound to go with the upper one. It says two vectors of very different lengths cannot be close together \u2014 which is what makes the norm a continuous function, and is used constantly in analysis.',
    },
    {
      name: 'Norm and the dot product',
      anchor: '#2',
      statement: '$\\|\\mathbf{v}\\|^2 = \\mathbf{v} \\cdot \\mathbf{v}$',
      verdict: 'holds',
      requires: 'the Euclidean norm specifically',
      note: 'Length is the square root of the self-[dot product](!/linear-algebra/vectors/dot-product). This is the bridge between the two: positive definiteness of the dot product is the same statement as non-negativity of the norm, seen from the other side.',
    },
    {
      name: 'Unit vector',
      anchor: '#5',
      statement: '$\\|\\hat{\\mathbf{v}}\\| = 1$ where $\\hat{\\mathbf{v}} = \\mathbf{v} / \\|\\mathbf{v}\\|$',
      verdict: 'conditional',
      holdsWhen: '$\\mathbf{v} \\neq \\mathbf{0}$',
      failsWhen: '$\\mathbf{v} = \\mathbf{0}$ \u2014 division by zero',
      note: 'Immediate from absolute homogeneity with $c = 1/\\|\\mathbf{v}\\|$. The zero vector cannot be normalised, which is the same fact as it having no [direction](!/linear-algebra/vectors/properties).',
    },
  ],
}
    ...
    },
    {
        ...
        content:[
          ...
          <DiagramFrame
            ...
            id="norm-properties"
            title="Properties of the norm"
            source="/linear-algebra/vectors/magnitude"
          >
            <PropertyLawCard data={normProperties} theme="navy" />
          </DiagramFrame>,
        ]
    },
    {
