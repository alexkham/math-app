...
`
  ...
  // obj7 — summary capstone: 3 operations × 3 modes
  // obj6 — the eight laws that make ℝⁿ a vector space, drawn from sections 2 and 5
  const operationLaws = {
    kicker: 'Vectors \u00B7 algebraic foundation',
    title: 'The eight laws the operations obey',
    tallyLabel: 'laws',
    intro: 'Four govern addition, four govern scalar multiplication. Together with closure they are exactly the [vector space axioms](!/linear-algebra/vector-spaces/axioms) \u2014 which is why anything proved from them holds for polynomials and matrices too, not just arrows.',
    footnote: 'Every law here is unconditional in $\\mathbb{R}^n$, which is what makes the space well behaved and also what makes it unremarkable. The interesting cases are elsewhere: [matrix multiplication](!/linear-algebra/matrix/operations) breaks commutativity and cancellation, and the [dot product](!/linear-algebra/vectors/dot-product) breaks closure. Seeing what a space looks like when nothing fails is what makes those failures legible.',
    laws: [
      {
        name: 'Commutativity of addition',
        anchor: '#2',
        statement: '$\\mathbf{a} + \\mathbf{b} = \\mathbf{b} + \\mathbf{a}$',
        verdict: 'holds',
        requires: 'component-wise, so it inherits from $\\mathbb{R}$',
        note: 'Order of addition does not matter. Geometrically, the parallelogram is the same figure traversed two ways \u2014 tip-to-tail in either order lands on the same diagonal.',
      },
      {
        name: 'Associativity of addition',
        anchor: '#2',
        statement: '$(\\mathbf{a} + \\mathbf{b}) + \\mathbf{c} = \\mathbf{a} + (\\mathbf{b} + \\mathbf{c})$',
        verdict: 'holds',
        requires: 'any three vectors in the same $\\mathbb{R}^n$',
        note: 'Grouping does not matter, so a chain of additions needs no parentheses. This is what lets [linear combinations](!/linear-algebra/vectors/linear-combinations) be written as a single sum.',
      },
      {
        name: 'Additive identity',
        anchor: '#2',
        statement: '$\\mathbf{a} + \\mathbf{0} = \\mathbf{a}$',
        verdict: 'holds',
        requires: '$\\mathbf{0}$ the vector of all zeros',
        note: 'The zero vector leaves everything unchanged. It is the origin \u2014 the one point every [subspace](!/linear-algebra/vector-spaces/subspaces) must contain, since without it there is no identity to add.',
      },
      {
        name: 'Additive inverse',
        anchor: '#2',
        statement: '$\\mathbf{a} + (-\\mathbf{a}) = \\mathbf{0}$',
        verdict: 'holds',
        requires: 'one inverse per vector',
        note: 'Every vector can be undone by the arrow pointing the opposite way. This is what makes subtraction definable at all \u2014 $\\mathbf{a} - \\mathbf{b}$ is shorthand for $\\mathbf{a} + (-\\mathbf{b})$, not a separate operation.',
      },
      {
        name: 'Associativity with scalars',
        anchor: '#5',
        statement: '$c(d\\mathbf{a}) = (cd)\\mathbf{a}$',
        verdict: 'holds',
        requires: 'any scalars $c, d$',
        note: 'Scaling twice equals scaling once by the product. Note the two multiplications are different operations \u2014 $cd$ happens between numbers, $c(d\\mathbf{a})$ between a number and a vector \u2014 and the law is the claim that they agree.',
      },
      {
        name: 'Distributivity over vector addition',
        anchor: '#5',
        statement: '$c(\\mathbf{a} + \\mathbf{b}) = c\\mathbf{a} + c\\mathbf{b}$',
        verdict: 'holds',
        requires: 'any scalar $c$',
        note: 'Scaling spreads over a sum of vectors. Together with the next law this is precisely what [linearity](!/linear-algebra/transformations/properties) demands of a transformation \u2014 the definition is these two conditions and nothing more.',
      },
      {
        name: 'Distributivity over scalar addition',
        anchor: '#5',
        statement: '$(c + d)\\mathbf{a} = c\\mathbf{a} + d\\mathbf{a}$',
        verdict: 'holds',
        requires: 'any scalars $c, d$',
        note: 'The mirror of the law above \u2014 spreading over a sum of scalars rather than of vectors. Both are needed, and neither follows from the other.',
      },
      {
        name: 'Multiplicative identity',
        anchor: '#5',
        statement: '$1\\mathbf{a} = \\mathbf{a}$',
        verdict: 'holds',
        requires: '$1$ the scalar identity',
        commonError: 'dismissing this as trivial \u2014 without it, defining $c\\mathbf{a} = \\mathbf{0}$ for every $c$ satisfies all seven laws above',
        note: 'The one that looks like it need not be stated. It is what ties the scalars to the vectors: drop it and scaling could collapse everything to $\\mathbf{0}$ while every other law still held.',
      },
    ],
  }
  ...
  const genericSections=[
    ...
    },
    {
        ...
        content:[
          ...
          <DiagramFrame
            key={'obj6-diagram'}
            id="vector-operation-laws"
            title="The eight laws the operations obey"
            source="/linear-algebra/vectors/basic-operations"
          >
            <PropertyLawCard data={operationLaws} theme="navy" />
          </DiagramFrame>,
        ]
    },
    {
