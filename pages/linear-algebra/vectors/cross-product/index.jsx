...
//   // obj5 — aggregation: 5 algebraic properties
//   const crossProductLaws = {
//     kicker: 'Vectors · cross product',
//     title: 'Properties of the cross product',
//     tallyLabel: 'entries',
//     intro: 'Linear in each slot and distributive, like the dot product — but anti-commutative rather than commutative, and not associative at all. The two failures are the ones worth holding onto.',
//     footnote: 'Both failures come from the same source: the output is a vector perpendicular to its inputs, so it depends on their order and on which pair you multiply first. The [dot product](!/linear-algebra/vectors/dot-product) returns a scalar and fails in the opposite direction — it commutes freely but leaves $\\mathbb{R}^3$ entirely.',
//     laws: [
//       {
//         name: 'Anti-commutativity',
//         statement: '$\\mathbf{a} \\times \\mathbf{b} = -(\\mathbf{b} \\times \\mathbf{a})$',
//         verdict: 'conditional',
//         verdictLabel: 'Order flips sign',
//         holdsWhen: 'always — the sign is the rule, not an exception',
//         note: 'Swapping the inputs reverses the output vector. The magnitude is unchanged, so the parallelogram area is the same; only the perpendicular direction flips, which is the right-hand rule reading itself backwards.',
//       },
//       {
//         name: 'Distributivity',
//         statement: '$\\mathbf{a} \\times (\\mathbf{b} + \\mathbf{c}) = \\mathbf{a} \\times \\mathbf{b} + \\mathbf{a} \\times \\mathbf{c}$',
//         verdict: 'holds',
//         requires: 'spreads over sums on either side',
//         note: 'Combined with scalar factoring this makes the cross product bilinear — linear in each slot separately, which is what lets it be computed by the [determinant formula](!/linear-algebra/determinants).',
//       },
//       {
//         name: 'Scalar factoring',
//         statement: '$(c\\mathbf{a}) \\times \\mathbf{b} = c(\\mathbf{a} \\times \\mathbf{b}) = \\mathbf{a} \\times (c\\mathbf{b})$',
//         verdict: 'holds',
//         requires: 'any scalar $c$',
//         note: 'Scalars migrate freely between slots and out front. A negative $c$ reverses the output, consistent with anti-commutativity.',
//       },
//       {
//         name: 'Self-cross product',
//         statement: '$\\mathbf{a} \\times \\mathbf{a} = \\mathbf{0}$',
//         verdict: 'conditional',
//         verdictLabel: 'Always zero',
//         holdsWhen: 'for every $\\mathbf{a}$, including $\\mathbf{0}$',
//         note: 'Forced by anti-commutativity: $\\mathbf{a} \\times \\mathbf{a} = -(\\mathbf{a} \\times \\mathbf{a})$, and the only vector equal to its own negative is $\\mathbf{0}$. Geometrically a vector spans no parallelogram with itself, so there is no area and no unique perpendicular.',
//       },
//       {
//         name: 'Associativity',
//         statement: '$\\mathbf{a} \\times (\\mathbf{b} \\times \\mathbf{c}) \\neq (\\mathbf{a} \\times \\mathbf{b}) \\times \\mathbf{c}$',
//         verdict: 'fails',
//         failsWhen: 'in general — parenthesisation changes the result',
//         note: 'The triple product expansion $\\mathbf{a} \\times (\\mathbf{b} \\times \\mathbf{c}) = \\mathbf{b}(\\mathbf{a}\\cdot\\mathbf{c}) - \\mathbf{c}(\\mathbf{a}\\cdot\\mathbf{b})$ is the workaround: it rewrites a nested cross product without one.',
//         witness: {
//           label: 'Witness',
//           lines: [
//             'a = i,  b = i,  c = j',
...
//       {
//         name: 'Cancellation',
//         statement: '$\\mathbf{a} \\times \\mathbf{b} = \\mathbf{a} \\times \\mathbf{c} \\nRightarrow \\mathbf{b} = \\mathbf{c}$',
//         verdict: 'fails',
//         failsWhen: 'always, for $\\mathbf{a} \\neq \\mathbf{0}$',
//         note: 'The cross product discards whatever is parallel to $\\mathbf{a}$ — the mirror image of the [dot product](!/linear-algebra/vectors/dot-product), which discards whatever is perpendicular. Between them nothing is lost, which is why the two together determine a vector completely.',
//         witness: {
//           label: 'Witness',
//           lines: [
//             'a = (1, 0, 0),  b = (0, 1, 0),  c = (1, 1, 0)',
//             'a × b = a × c = (0, 0, 1),  yet b ≠ c',
//           ],
//         },
//       },
//     ],
...
//           <DiagramFrame
//             key={'obj5-diagram'}
//             id="cross-product-laws"
//             title="Properties of the cross product"
//             source="/linear-algebra/vectors/cross-product"
//           >
//             <PropertyLawCard data={crossProductLaws} theme="navy" variant="docket" />
//           </DiagramFrame>,
//         ]
//     },
//     {
...
`
  ...
  // obj8 — comparison: dot product vs cross product across 6 attributes
  // obj8 — the two products, opposed attribute by attribute
  const productComparison = {
    kicker: 'Vectors \u00B7 two products',
    title: 'Dot against cross',
    tallyLabel: 'products',
    intro: 'Two ways of multiplying vectors that disagree on almost every attribute. Where one is zero the other is largest, and between them they measure the two things a pair of vectors has: how much they agree, and how much they span.',
    footnote: 'The opposition is systematic rather than coincidental. One extracts $\\cos\\theta$ and one extracts $\\sin\\theta$, and every difference below follows: zero at opposite angles, scalar against vector, commutative against sign-reversing. Given both products, $\\theta$ is determined completely \u2014 neither alone suffices.',
    slots: [
      { key: 'output',   label: 'output' },
      { key: 'domain',   label: 'defined in' },
      { key: 'order',    label: 'swapping the arguments' },
      { key: 'zeroWhen', label: 'zero when' },
      { key: 'reading',  label: 'geometric reading' },
    ],
    groups: [
      {
        heading: 'Measures agreement',
        types: [
          {
            name: 'Dot product',
            anchor: '#8',
            shape: 'diagonal',
            condition: '$\\mathbf{a} \\cdot \\mathbf{b} = \\|\\mathbf{a}\\|\\|\\mathbf{b}\\|\\cos\\theta$',
            properties: {
              output: 'a scalar',
              domain: 'any $\\mathbb{R}^n$',
              order: 'unchanged \u2014 commutative',
              zeroWhen: 'perpendicular, $\\theta = \\pi/2$',
              reading: 'projection of one onto the other',
            },
            note: 'Extracts $\\cos\\theta$, so it is largest when the vectors align and vanishes when they are perpendicular. Leaves $\\mathbb{R}^n$ entirely \u2014 see [dot product properties](!/linear-algebra/vectors/dot-product).',
          },
        ],
      },
      {
        heading: 'Measures spread',
        types: [
          {
            name: 'Cross product',
            anchor: '#8',
            shape: 'skew',
            condition: '$\\|\\mathbf{a} \\times \\mathbf{b}\\| = \\|\\mathbf{a}\\|\\|\\mathbf{b}\\|\\sin\\theta$',
            properties: {
              output: 'a vector',
              domain: 'only $\\mathbb{R}^3$',
              order: 'sign reverses \u2014 anti-commutative',
              zeroWhen: 'parallel, $\\theta = 0$ or $\\pi$',
              reading: 'area of the parallelogram spanned',
            },
            note: 'Extracts $\\sin\\theta$, so it is largest when the vectors are perpendicular and vanishes when they are parallel \u2014 the exact opposite of the row above. Stays inside $\\mathbb{R}^3$, and only exists there.',
          },
        ],
      },
    ],
...
Placing vectors rather than numbers in the top row makes this a notational device rather than a genuine determinant. Still, applying cofactor expansion along that top row reproduces the component formula term by term. The determinant layout keeps the six constituent products and their signs organized in a form that is easy to remember and hard to scramble.`,
  ...
  notation: {
    title: `Cross Product Notation`,
    lead: `The multiplication sign that finally became order-sensitive, the triple product with only one legal parse, and the three ways to spell one basis.`,
    inherited: `Bold vectors — [vector notation](!/linear-algebra/vectors/basic-operations#notation); the dot — [dot product notation](!/linear-algebra/vectors/dot-product#notation); the symbolic-determinant device is the body's own **Algebraic Definition** above.`,
    entries: [
      {
        id: 'cross-mark',
      ...
      {
        id: 'triple-product-parse',
        tex: `$\\mathbf{a} \\cdot (\\mathbf{b} \\times \\mathbf{c})$`,
        read: `a dot, b cross c`,
      ...
      {
        id: 'basis-spellings',
        tex: `$\\mathbf{i}, \\mathbf{j}, \\mathbf{k}$`,
        read: `The standard basis of R-three`,
        means: `This page's spelling of the three coordinate directions — bold letters, as in **Standard Basis Cross Products** below. Two rival spellings coexist: the hatted $\\hat{\\imath}, \\hat{\\jmath}, \\hat{k}$ of physics, per [norm notation](!/linear-algebra/vectors/magnitude#notation), and the subscripted $\\mathbf{e}_1, \\mathbf{e}_2, \\mathbf{e}_3$ that survives beyond three dimensions.`,
        cases: `The $\\mathbf{e}_i$ form is the only one that scales — $\\mathbb{R}^7$ has an $\\mathbf{e}_5$ but no letter for it; $\\mathbf{i}, \\mathbf{j}, \\mathbf{k}$ are a three-dimensional luxury.`,
        alsoWritten: `$\\mathbf{u}_x, \\mathbf{u}_y, \\mathbf{u}_z$ — axis-subscripted, in engineering texts.`,
        confusedWith: `The [imaginary unit](!/complex-numbers/basics). Unbolded $i$ is $\\sqrt{-1}$ — and the collision is historical, not accidental: Hamilton's quaternions $i, j, k$ are the ancestors of both this basis and the cross product itself.`,
      },
    ],
    ...
    },
    {
        ...
        content:[
          ...
          <DiagramFrame
            ...
            source="/linear-algebra/vectors/cross-product"
          >
            <ObjectTypeProfile data={productComparison} theme="navy" variant="grid" />
          </DiagramFrame>,
          `Every row opposes because the two products extract different halves of the same angle. The dot product carries $\\cos\\theta$ and the cross product $\\sin\\theta$, so one peaks exactly where the other vanishes — and together they pin $\\theta$ down completely, which neither can do alone. The remaining differences follow from the outputs: a scalar has no orientation to reverse, while a vector does, which is why one commutes and the other flips sign.`,
        ]
    },
    {
        id:'9',
