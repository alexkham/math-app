...
// obj12 — summary capstone: the algorithm's workflow at a glance
// obj12 — the algorithm as stages, with the fork in how the answer is read
const eliminationStages = {
  kicker: 'Linear systems \u00B7 the algorithm',
  title: 'Gaussian elimination, stage by stage',
  tallyLabel: 'stages',
  intro: 'Two stages are compulsory and the third is a choice. Forward elimination and classification happen once; after that, back substitution and Gauss\u2013Jordan reach the same solution set by different amounts of work.',
  footnote: 'The whole algorithm is three [row operations](#2) applied until the shape is triangular, and every one of them is reversible \u2014 which is why the solution set is unchanged at every step. Elimination does not solve the system so much as rewrite it into a form where the solution can be read.',
  groups: [
    {
      heading: 'Compulsory \u2014 every system, every time',
      identities: [
        {
          name: 'Forward elimination',
          anchor: '#3',
          formula: '$[A \\mid \\mathbf{b}] \\to$ row echelon form',
          condition: 'swaps and row additions, left to right',
          key: true,
          note: 'Sweep column by column, using each pivot to zero everything beneath it. This is the expensive stage at roughly $\\tfrac{2}{3}n^3$ operations, and it is where [partial pivoting](#11) belongs \u2014 choosing the largest available pivot rather than the first nonzero one.',
        },
        {
          name: 'Classify the columns',
          anchor: '#4',
          formula: 'pivot columns vs free columns',
          condition: 'and check for $[\\,0 \\cdots 0 \\mid d \\neq 0\\,]$',
          key: true,
          note: 'Everything diagnostic happens here. The pivot count is the [rank](!/linear-algebra/matrix/rank), the free columns are the parameters, and a contradiction row settles [solvability](!/linear-algebra/linear-systems/solvability) before any solution is attempted.',
        },
      ],
    },
    {
      heading: 'A choice \u2014 two ways to read the answer',
      identities: [
        {
          name: 'Back substitution',
          anchor: '#5',
          formula: 'solve upward from the last pivot row',
          condition: 'stops at REF',
          note: 'Less arithmetic: roughly $n^2$ operations on top of the elimination, and no further row operations at all. The usual choice by hand and the one every numerical library uses, since the triangular form is already enough to solve from.',
        },
        {
          name: 'Gauss\u2013Jordan',
          anchor: '#6',
          formula: 'continue to reduced row echelon form',
          condition: 'zero above the pivots, scale pivots to $1$',
          strict: true,
          note: 'Roughly $50\\%$ more arithmetic than back substitution for the same solution, so it is not the method of choice for solving. Its value is elsewhere: the RREF is unique, which makes it the right form for reading a [null space basis](!/linear-algebra/linear-systems/echelon-form) or inverting a matrix.',
        },
      ],
    },
    {
      ...
      identities: [
        {
          ...
          anchor: '#7',
          formula: 'a point, or $\\mathbf{x}_p + \\operatorname{Null}(A)$, or empty',
          condition: 'decided at the classification stage',
          note: 'Never anything else. Unique when every column has a pivot, a translated subspace when some do not, empty when a contradiction row appeared \u2014 and which of the three it is was known before the solution was computed.',
        },
      ],
    },
  ],
...
Every serious numerical implementation of Gaussian elimination uses partial pivoting by default. In the [LU decomposition](!/linear-algebra/decompositions/lower-upper), the row swaps are recorded in a [permutation matrix](!/linear-algebra/matrix/types#12) $P$, giving the factorization $PA = LU$.`,
    before: ``,
    after: ``,
    link: ``,
  },
  ...
  const genericSections=[
    ...
    },
    {
        ...
        content:[
          ...
          <DiagramFrame
            key={'obj12-diagram'}
            id="elimination-stages"
            title="Gaussian elimination, stage by stage"
            source="/linear-algebra/linear-systems/gaussian-elimination"
          >
