...
// }
//    }
// export default function EchelonFormPage({
//   seoData,
//   sectionsContent,
//   introContent,
//   obj3Table,
...
// obj11 — summary capstone: everything readable from RREF
// obj11 — everything one reduction yields, sorted by what it is read from
const rrefReadings = {
  kicker: 'Linear systems \u00B7 summary',
  title: 'Everything readable from the RREF',
  tallyLabel: 'readings',
  intro: 'One reduction answers all of these. What differs is where in the reduced form the answer sits \u2014 the pivot positions, the free columns, or the augmented column.',
  footnote: 'Nothing below requires a second computation. That is the return on reducing: rank, nullity, three bases, consistency, uniqueness and invertibility all come out of a single echelon form, which is why [Gaussian elimination](!/linear-algebra/linear-systems/gaussian-elimination) is the one algorithm worth doing by hand.',
  groups: [
    {
      heading: 'From the pivot count',
      identities: [
        {
          name: 'Rank',
          anchor: '#10',
          formula: '$\\operatorname{rank}(A) = $ number of pivots',
          condition: 'count them; nothing else needed',
          note: 'Pivots correspond to independent rows and to independent columns simultaneously, which is why [row rank equals column rank](!/linear-algebra/matrix/rank) \u2014 they are the same pivots counted two ways.',
        },
        {
          name: 'Nullity',
          anchor: '#10',
          formula: '$n - \\operatorname{rank}(A)$',
          condition: 'the number of free columns',
          note: 'Every column without a pivot is a free variable, and every free variable contributes one dimension to the null space. Rank-nullity is not a separate theorem here so much as a restatement of the column count.',
        },
      ],
    },
    {
      heading: 'From the pivot positions \u2014 bases',
      identities: [
        {
        ...
        },
        {
          ...
          anchor: '#4',
          formula: 'nonzero rows of the RREF',
          condition: 'of the reduced form, not the original',
          strict: true,
          note: 'The exact opposite of the rule above, and the reason both are stated here together. Row operations preserve the row space itself, so the reduced rows still span it \u2014 and they are independent, which the original rows may not be.',
        },
        {
          name: 'Null space basis',
          anchor: '#7',
          formula: 'one vector per free variable',
          condition: 'set each free variable to $1$ in turn',
          note: 'Read from the parametric vector form. The count matches the nullity above, and each basis vector is the solution obtained by turning one free variable on and the rest off.',
        },
      ],
    },
    {
      heading: 'From the augmented column',
      identities: [
        {
          name: 'Consistency',
          anchor: '#8',
          formula: 'no row $[\\,0 \\cdots 0 \\mid d\\,]$, $d \\neq 0$',
          condition: '$\\operatorname{rank}(A) = \\operatorname{rank}([A \\mid \\mathbf{b}])$',
          note: 'Such a row asserts $0 = d$, so the system has no solution. Equivalently $\\mathbf{b}$ lies outside the column space \u2014 the augmented column raises the rank, and that is what the contradiction row records.',
        },
        {
          name: 'Uniqueness',
          anchor: '#6',
          formula: 'a pivot in every column of $A$',
          condition: 'no free variables',
          note: 'Consistency gives at least one solution, no free variables gives at most one. Both conditions are needed \u2014 an inconsistent system has none however many pivots it has.',
        },
        {
          name: 'Invertibility',
          anchor: '#9',
          formula: '$\\operatorname{rref}(A) = I_n$',
          condition: 'square and full rank',
          key: true,
          note: 'The two conditions above at once, for a square matrix: a pivot in every row and every column leaves the identity. This is one line of the [invertibility equivalence](!/linear-algebra/matrix/inverse), and reduction is how it is checked in practice.',
        },
      ],
    },
  ],
    ...
    },
    {
        ...
        content:[
          ...
          <DiagramFrame
            ...
            id="rref-readings"
            title="Everything readable from the RREF"
            source="/linear-algebra/linear-systems/echelon-form"
          >
            <IdentitySheet data={rrefReadings} theme="navy" variant="ledger" />
          </DiagramFrame>,
