...
// //   // obj3 — the Invertible Matrix Theorem, rendered as an equivalence ring
// //   const invertibilityRing = {
// //     kicker: 'Square matrices · TFAE',
// //     title: 'The invertibility equivalence',
// //     subject: 'A is invertible',
// //     tallyLabel: 'statements',
// //     intro: 'Twelve statements about an $n \\times n$ matrix, no one of them primary. Any may be taken as the definition and the rest derived — which is why the picture is a ring and not a list.',
// //     statements: [
// //       { text: '$\\det(A) \\neq 0$', domain: 'det' },
// //       { text: 'A is a product of [elementary matrices](!/linear-algebra/matrix/operations)', domain: 'det' },
// //       { text: '0 is not an [eigenvalue](!/linear-algebra/eigen) of A', domain: 'det' },
// //       { text: '$\\operatorname{rank}(A) = n$ — full rank', domain: 'rank' },
// //       { text: '$\\operatorname{rref}(A) = I_n$', domain: 'rank' },
// //       { text: 'The columns of A are [linearly independent](!/linear-algebra/vector-spaces/linear-independence)', domain: 'space' },
// //       { text: 'The columns of A [span](!/linear-algebra/vector-spaces/span) $\\mathbb{R}^n$', domain: 'space' },
// //       { text: 'The columns of A form a [basis](!/linear-algebra/vector-spaces/basis) for $\\mathbb{R}^n$', domain: 'space' },
// //       { text: 'The rows of A are linearly independent', domain: 'space' },
// //       { text: '$\\operatorname{null}(A) = \\{\\mathbf{0}\\}$', domain: 'space' },
// //       { text: '$A\\mathbf{x} = \\mathbf{0}$ has only the trivial solution', domain: 'system' },
// //       { text: '$A\\mathbf{x} = \\mathbf{b}$ has a unique solution for every $\\mathbf{b} \\in \\mathbb{R}^n$', domain: 'system' },
// //     ],
// //     balance: 'The twelve are not twelve independent claims so much as one fact seen from four sides. $\\operatorname{rank}(A) = n$ and $\\operatorname{rref}(A) = I_n$ are the same statement in two notations, and the row condition follows from the column condition because [row rank equals column rank](!/linear-algebra/matrix/rank). What the tagging exposes is how much of the theorem lives in the subspace corner — five of twelve — against a determinant corner that gets stated first and remembered longest.',
// //   }
...
// // export default function MatrixInversePage({seoData, sectionsContent, introContent, invertibilityRing, obj8Table, obj9Table, summaryTable, faqQuestions, schemas}) {
...
// //         content:[
// //           sectionsContent.obj3.content,
// //           <EquivalenceRing key={'obj3-ring'} data={invertibilityRing} theme="navy" variant="ring" />,
// //         ]
// //     },
// //     {
// //         id:'4',
...
//   // obj6 — identities the inverse satisfies, indexed against the page sections
//   const inverseProperties = {
//     kicker: 'Matrix · inverse',
//     title: 'Properties of the inverse',
//     tallyLabel: 'identities',
//     intro: 'Seven identities that mirror the rules for reciprocals — with two that do not. The order reversal and the missing sum rule are where the analogy with real numbers stops.',
//     footnote: 'Every identity here presupposes the matrices involved are invertible, which is exactly what the [equivalence above](#3) characterises. Sum and difference are absent from this list because no rule exists — that absence is the entry, not an omission.',
//     laws: [
//       {
//         name: 'Involution',
//         anchor: '#6',
...
//         note: 'Inverting twice returns the original. The inverse relation is symmetric — if $B$ is the inverse of $A$ then $A$ is the inverse of $B$, so neither matrix in the pair is privileged.',
//       },
//       {
...
//         verdict: 'conditional',
//         holdsWhen: 'both $A$ and $B$ invertible',
//         commonError: 'writing $(AB)^{-1} = A^{-1}B^{-1}$ — the order must reverse',
//         note: 'The order reverses, because $(AB)(B^{-1}A^{-1}) = A(BB^{-1})A^{-1} = AA^{-1} = I$ only in that arrangement. This extends to any number of factors: $(A_1A_2\\cdots A_k)^{-1} = A_k^{-1}\\cdots A_2^{-1}A_1^{-1}$. The reversal is the same one the [transpose](!/linear-algebra/matrix/operations) performs, and both trace back to matrix multiplication not commuting.',
//       },
//       {
//         name: 'Inverse of a transpose',
//         anchor: '#6',
//         statement: '$(A^{\\mathsf{T}})^{-1} = (A^{-1})^{\\mathsf{T}}$',
//         verdict: 'conditional',
//         holdsWhen: '$A$ invertible',
//         note: 'The two operations commute — transpose then invert, or invert then transpose, and the result is the same. Worth contrasting with the product rule directly above: there the order matters, here it does not.',
//       },
//       {
//         name: 'Scalar multiple',
//         anchor: '#6',
//         statement: '$(cA)^{-1} = \\tfrac{1}{c}A^{-1}$',
//         verdict: 'conditional',
//         holdsWhen: '$A$ invertible and $c \\neq 0$',
//         note: 'Scalars pass through and invert. The condition $c \\neq 0$ is doing real work — $0 \\cdot A$ is the zero matrix, which is never invertible whatever $A$ was.',
//       },
//       {
//         name: 'Powers',
//         anchor: '#6',
//         statement: '$(A^k)^{-1} = (A^{-1})^k$',
//         verdict: 'conditional',
//         holdsWhen: '$A$ invertible, $k$ a positive integer',
//         note: 'Follows from the product rule applied $k$ times; the order reversal cancels out because every factor is the same matrix. This is what lets negative exponents be defined at all, with $A^{-k}$ meaning either side of the identity.',
//       },
//       {
//         name: 'Determinant of the inverse',
//         anchor: '#6',
//         statement: '$\\det(A^{-1}) = \\dfrac{1}{\\det(A)}$',
//         verdict: 'conditional',
//         holdsWhen: '$\\det(A) \\neq 0$',
//         note: 'Immediate from $\\det(AB) = \\det(A)\\det(B)$ applied to $AA^{-1} = I$. It also gives a one-line proof that a singular matrix has no inverse: there is no real number whose product with zero is one. See [determinant properties](!/linear-algebra/determinants/properties).',
//       },
//       {
...
//         statement: '$(A + B)^{-1} \\neq A^{-1} + B^{-1}$',
//         verdict: 'fails',
//         failsWhen: 'in general — no rule exists',
//         note: 'There is no product-style identity for the inverse of a sum, and $A + B$ may not even be invertible when both $A$ and $B$ are. The Woodbury identity handles a restricted case, but nothing does in general. This entry exists so the absence is stated rather than inferred from silence.',
//         witness: {
//           label: 'Witness',
//           lines: [
//             'A = I,  B = −I,  both invertible',
...
// export default function MatrixInversePage({seoData, sectionsContent, introContent, invertibilityRing, inverseProperties, obj8Table, obj9Table, summaryTable, faqQuestions, schemas}) {
...
//           <DiagramFrame
//             key={'obj3-diagram'}
//             id="invertibility-equivalence"
//             title="The invertibility equivalence"
//             source="/linear-algebra/matrix/inverse"
//             formats={['png', 'pdf', 'svg']}
//           >
//             <EquivalenceRing data={invertibilityRing} theme="navy" variant="ring" />
//           </DiagramFrame>,
//         ]
//     },
//     {
...
//             source="/linear-algebra/matrix/inverse"
//           >
//             <PropertyLawCard data={inverseProperties} theme="navy" />
//           </DiagramFrame>,
//         ]
//     },
//     {
...
const keyWords = [
  ...
  // obj8 — aggregation: inverses of special matrix types
  // obj8 — six structured types, each with a cheaper route to the inverse
  const specialInverses = {
    kicker: 'Matrix \u00B7 inverse',
    title: 'When structure makes the inverse cheap',
    tallyLabel: 'types',
    intro: 'Each of these is invertible under a condition readable off the matrix, and each has an inverse that inherits the same structure. Recognising the type is worth more than any general algorithm.',
    footnote: 'Two things recur down the table. The invertibility condition is always local \u2014 a diagonal entry, a block, never a determinant \u2014 and the inverse always belongs to the same family, which is what makes these routes composable. General [row reduction](#4) discards both advantages and should be the fallback rather than the first move.',
    slots: [
      { key: 'invertibleWhen', label: 'invertible when' },
      { key: 'inverse',        label: 'inverse' },
      { key: 'cost',           label: 'cost' },
    ],
    groups: [
      {
        heading: 'The inverse is free',
        types: [
          {
            name: 'Orthogonal',
            anchor: '#8',
            shape: 'orthogonal',
            condition: '$Q^{\\mathsf{T}}Q = I$',
            properties: {
              invertibleWhen: 'always \u2014 $\\det Q = \\pm 1$',
              inverse: '$Q^{-1} = Q^{\\mathsf{T}}$',
              cost: 'none \u2014 transpose',
            },
            note: 'The only row where no arithmetic happens at all. Always invertible, and the inverse is a relabelling of the entries \u2014 which is why [orthogonal factors](!/linear-algebra/decompositions/qr) are worth arranging for in a factorization.',
          },
        ],
      },
      {
        heading: 'The inverse is entrywise or blockwise',
        types: [
          {
            name: 'Diagonal',
            anchor: '#8',
            shape: 'diagonal',
            condition: '$D = \\operatorname{diag}(d_1, \\ldots, d_n)$',
            properties: {
              invertibleWhen: 'every $d_i \\neq 0$',
              inverse: '$\\operatorname{diag}(1/d_1, \\ldots, 1/d_n)$',
              cost: '$n$ divisions',
            },
            note: 'Reciprocate the diagonal. The condition is visible without computing anything, since the determinant is the product of exactly those entries.',
          },
          {
            name: 'Block diagonal',
            anchor: '#8',
            shape: 'block',
            condition: '$\\operatorname{diag}(A_1, \\ldots, A_k)$',
            properties: {
              invertibleWhen: 'every block $A_i$ invertible',
              inverse: '$\\operatorname{diag}(A_1^{-1}, \\ldots, A_k^{-1})$',
              cost: 'invert each block',
            },
            note: 'The diagonal case with numbers replaced by blocks. An $n \\times n$ matrix in $k$ equal blocks costs $k$ inversions of size $n/k$ rather than one of size $n$ \u2014 a saving of $k^2$, since the cost is cubic.',
          },
        ],
      },
      {
        heading: 'The inverse comes from substitution',
        types: [
          {
            name: 'Upper triangular',
            anchor: '#8',
            shape: 'upper',
            condition: 'zeros below the diagonal',
            properties: {
              invertibleWhen: 'every diagonal entry $\\neq 0$',
              inverse: 'upper triangular',
              cost: 'back substitution, $n^2$',
            },
            note: 'Solve for the columns from the bottom up. The inverse stays upper triangular, which is why an [LU factorization](!/linear-algebra/decompositions/lower-upper) is worth keeping rather than discarding after the first solve.',
          },
          {
            name: 'Lower triangular',
            anchor: '#8',
            shape: 'lower',
            condition: 'zeros above the diagonal',
            properties: {
              invertibleWhen: 'every diagonal entry $\\neq 0$',
              inverse: 'lower triangular',
              cost: 'forward substitution, $n^2$',
            },
            note: 'The mirror image, worked from the top down. Together with the row above, these are the two solves an LU-based inverse actually performs.',
          },
        ],
      },
      {
        heading: 'Structure preserved but no shortcut',
        types: [
          {
            name: 'Symmetric',
            anchor: '#8',
            ...
            properties: {
              invertibleWhen: '$\\det(A) \\neq 0$',
              inverse: 'also symmetric',
              cost: 'half of a general solve',
            },
            note: 'The one row whose condition is not local \u2014 symmetry constrains the inverse without deciding whether it exists. The saving is real but modest: only one triangle need be computed. When the matrix is also positive definite, [Cholesky](!/linear-algebra/decompositions/cholesky) halves it again.',
          },
        ],
      },
    ],
    ...
    },
    {
        ...
        content:[
          ...
          <DiagramFrame
            ...
            source="/linear-algebra/matrix/inverse"
          >
            <ObjectTypeProfile data={specialInverses} theme="navy" variant="stack" />
          </DiagramFrame>,
