...
$A$ is invertible. The [determinant](!/linear-algebra/determinants) $\\det(A) \\neq 0$. The [rank](!/linear-algebra/matrix/rank) of $A$ equals $n$. The columns of $A$ are [linearly independent](!/linear-algebra/vector-spaces/linear-independence). The rows of $A$ are linearly independent. The columns of $A$ [span](!/linear-algebra/vector-spaces/span) $\\mathbb{R}^n$. The columns of $A$ form a [basis](!/linear-algebra/vector-spaces/basis) for $\\mathbb{R}^n$. The homogeneous system $Ax = \\mathbf{0}$ has only the trivial solution. The system $Ax = \\mathbf{b}$ has a unique solution for every $\\mathbf{b} \\in \\mathbb{R}^n$. The [null space](!/linear-algebra/vector-spaces/fundamental-subspaces) of $A$ is $\\{\\mathbf{0}\\}$. The [reduced row echelon form](!/linear-algebra/linear-systems/rref) of $A$ is $I$. The matrix $A$ is a product of elementary matrices. Zero is not an [eigenvalue](!/linear-algebra/eigenvalues-vectors) of $A$.
