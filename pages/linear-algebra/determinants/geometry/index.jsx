...
//  export default function DeterminantGeometryPage({
//    seoData,
//    sectionsContent,
//    introContent,
//    obj4Table,
...
// obj8 — summary capstone: what det(A) means dimension by dimension
// obj8 — the same three readings, dimension by dimension
const determinantGeometry = {
  kicker: 'Determinants \u00B7 geometry',
  title: 'What the determinant measures, by dimension',
  tallyLabel: 'dimensions',
  intro: 'The magnitude is a volume, the sign is an orientation, and zero is a collapse. Only the word for &quot;volume&quot; changes as the dimension rises \u2014 which is the argument for calling the general case a volume at all.',
  footnote: 'Read down the column and the pattern is one definition stated four ways: $|\\det|$ is the factor by which the unit cube\u2019s content is scaled, the sign says whether the image is reflected, and $\\det = 0$ says the image has no content because it fits inside a smaller subspace. Length, area and volume are not analogies here \u2014 they are the $n = 1, 2, 3$ cases of the same quantity.',
  groups: [
    {
      heading: 'Where the picture is available',
      identities: [
        {
          name: 'One dimension',
          anchor: '#3',
          formula: '$\\det[a] = a$',
          condition: 'length scaling; sign = direction',
          note: 'A segment on a line. The determinant is the number itself, and its sign says whether the direction is preserved or reversed \u2014 the degenerate case that makes the general statement believable rather than surprising.',
        },
        {
          name: 'Two dimensions',
          anchor: '#1',
          formula: '$|\\det A| = $ area of the parallelogram',
          condition: 'sign: counterclockwise $+$, clockwise $-$',
          key: true,
          note: 'The columns of $A$ span a parallelogram, and the determinant is its signed area. Zero means the columns are parallel and the parallelogram has collapsed to a line \u2014 which is the same statement as the columns being [linearly dependent](!/linear-algebra/vector-spaces/linear-independence).',
        },
        {
          name: 'Three dimensions',
          anchor: '#2',
          formula: '$|\\det A| = $ volume of the parallelepiped',
          condition: 'sign: right-handed $+$, left-handed $-$',
          note: 'Three columns span a parallelepiped. The sign distinguishes a right-handed frame from a left-handed one \u2014 the same distinction the [cross product](!/linear-algebra/vectors/cross-product) encodes, and the reason $\\mathbf{a} \\cdot (\\mathbf{b} \\times \\mathbf{c})$ is a determinant.',
        },
      ],
    },
    {
      ...
      identities: [
        {
          name: 'Four dimensions and above',
          anchor: '#3',
          formula: '$|\\det A| = $ volume of the image of the unit cube',
          condition: 'orientation preserved $+$ or reversed $-$',
          key: true,
          note: 'No picture, and none needed \u2014 the algebra is unchanged. This is where the determinant stops being a description of something already visible and becomes the definition of $n$-dimensional content, which is what the [change of variables formula](#6) then relies on.',
        },
        {
          name: 'Determinant zero',
          anchor: '#5',
          formula: '$\\det A = 0$',
    ...
    },
    {
        ...
        content:[
          ...
          <DiagramFrame
            ...
            title="What the determinant measures, by dimension"
            source="/linear-algebra/determinants/geometry"
          >
