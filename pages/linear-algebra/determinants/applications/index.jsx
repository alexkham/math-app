...
//    export default function DeterminantApplicationsPage({
//      seoData,
//      sectionsContent,
//      introContent,
//      obj1Table,
...
// obj7 — summary capstone: each application at a glance
// obj7 — six uses, split by whether the determinant computes or decides
const determinantApplications = {
  kicker: 'Determinants \u00B7 applications',
  title: 'Six places a determinant does the work',
  tallyLabel: 'applications',
  intro: 'Three of these produce a value and three produce a verdict. The split matters because the first group is almost never the fastest route to that value \u2014 while the second group has no competitor at all.',
  footnote: 'Where a determinant computes something, elimination usually computes it faster; where a determinant decides something, nothing else decides it as cleanly. That is the honest summary of the determinant\u2019s role: a poor algorithm and an excellent criterion.',
  groups: [
    {
      heading: 'Producing a value \u2014 correct, rarely fastest',
      identities: [
        {
          name: 'Cramer\u2019s rule',
          anchor: '#1',
          formula: '$x_i = \\dfrac{\\det(A_i)}{\\det(A)}$',
          condition: '$\\det(A) \\neq 0$; $A_i$ has column $i$ replaced by $\\mathbf{b}$',
          strict: true,
          note: 'Each component as a ratio of determinants. Costs $n{+}1$ determinants against one [elimination](!/linear-algebra/linear-systems/gaussian-elimination), so it is a formula rather than a method \u2014 valuable because it exhibits the solution as an explicit function of the entries, which is what sensitivity analysis needs.',
        },
        {
          name: 'Adjugate inverse',
          anchor: '#2',
          formula: '$A^{-1} = \\dfrac{1}{\\det A}\\operatorname{adj}(A)$',
          condition: '$\\det(A) \\neq 0$',
          strict: true,
          note: 'Every entry of the inverse in closed form. Practical at $2 \\times 2$ and defensible at $3 \\times 3$; beyond that the cofactor count makes it unusable numerically. Its real work is theoretical \u2014 it proves the inverse entries are rational functions of $A$.',
        },
        {
          name: 'Structured determinants',
          anchor: '#6',
          formula: 'Vandermonde: $\\prod_{i<j}(x_j - x_i)$',
          condition: 'the matrix has exploitable structure',
          note: 'Closed forms that skip the computation entirely. The Vandermonde value is nonzero exactly when the nodes are distinct, which is the statement that polynomial interpolation through distinct points has a unique solution.',
        },
      ],
    },
    {
      heading: 'Producing a verdict \u2014 nothing else does it as well',
      identities: [
        {
          name: 'Characteristic polynomial',
          anchor: '#4',
          formula: '$p(\\lambda) = \\det(A - \\lambda I)$',
          condition: 'roots are the [eigenvalues](!/linear-algebra/eigen)',
          key: true,
          note: 'The determinant is what converts &quot;this homogeneous system has a nontrivial solution&quot; into a polynomial equation in $\\lambda$. Without it there is no equation to solve, which is why eigenvalue theory begins here rather than with eigenvectors.',
        },
        {
          name: 'Cross product',
          anchor: '#3',
          formula: '$\\mathbf{a} \\times \\mathbf{b} = \\det\\begin{bmatrix}\\hat{\\imath} & \\hat{\\jmath} & \\hat{k} \\\\ \\mathbf{a} \\\\ \\mathbf{b}\\end{bmatrix}$',
          condition: '$\\mathbb{R}^3$ only',
          note: 'A mnemonic that is also an explanation. Expanding along the first row reproduces the component formula, and the determinant being zero when two rows match is exactly why $\\mathbf{a} \\times \\mathbf{a} = \\mathbf{0}$ \u2014 see [cross product](!/linear-algebra/vectors/cross-product).',
        },
        {
          name: 'Wronskian',
          anchor: '#5',
          formula: '$W(x) = \\det\\bigl[f_i^{(j)}(x)\\bigr]$',
          condition: '$W \\neq 0$ somewhere $\\Rightarrow$ independent',
          strict: true,
          note: 'One-directional, and the direction matters. A nonzero value at a single point proves independence; $W \\equiv 0$ proves nothing in general \u2014 $x^2$ and $x|x|$ are independent with $W$ identically zero. The converse holds only for solutions of one linear ODE.',
        },
      ],
    },
  ],
  ...
  const genericSections=[
    ...
    },
    {
        ...
        content:[
          ...
          <DiagramFrame
            key={'obj7-diagram'}
            id="determinant-applications"
            title="Six places a determinant does the work"
            source="/linear-algebra/determinants/applications"
          >
            <IdentitySheet data={determinantApplications} theme="navy" variant="ledger" />
          </DiagramFrame>,
