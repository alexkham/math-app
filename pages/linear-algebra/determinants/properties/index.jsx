...
// // obj9 — the invertibility equivalence, rendered as a ring
// const invertibilityRing = {
//   kicker: 'Determinants · TFAE',
//   title: 'One question, twelve answers',
//   subject: 'A is invertible',
//   tallyLabel: 'statements',
//   intro: 'The determinant is one entry in this ring, not its centre. Every statement below implies every other, so any of them could have been the definition and the determinant chosen for convenience rather than priority.',
//   statements: [
//     { text: '$\\det(A) \\neq 0$', domain: 'det' },
//     { text: 'A is a product of [elementary matrices](!/linear-algebra/matrix/operations)', domain: 'det' },
//     { text: 'All [eigenvalues](!/linear-algebra/eigen) of A are nonzero', domain: 'det' },
//     { text: '$\\operatorname{rank}(A) = n$', domain: 'rank' },
//     { text: 'The [reduced row echelon form](!/linear-algebra/linear-systems/echelon-form) of A is $I_n$', domain: 'rank' },
...
//     { text: 'The [null space](!/linear-algebra/vector-spaces/fundamental-spaces) of A is $\\{\\mathbf{0}\\}$', domain: 'space' },
//     { text: '$A\\mathbf{x} = \\mathbf{b}$ has exactly one solution for every $\\mathbf{b}$', domain: 'system' },
//     { text: '$A\\mathbf{x} = \\mathbf{0}$ has only the trivial solution', domain: 'system' },
//   ],
//   balance: 'Only three of the twelve mention the determinant at all. The other nine settle invertibility through rank, subspace structure, or solvability — none of which requires computing a determinant. That is worth holding onto, because for large $n$ the determinant is among the [most expensive routes](!/linear-algebra/matrix/inverse) to an answer the row reduction already gave you on the way.',
// }
...
// This equivalence sits at the center of a web of conditions that are all mutually equivalent for an $n \\times n$ matrix $A$ — the twelve below are either all true or all false.
...
//         content:[
//           sectionsContent.obj9.content,
//           <EquivalenceRing key={'obj9-ring'} data={invertibilityRing} theme="navy" variant="ring" />,
//         ]
//     },
//     {
//         id:'10',
...
// obj10 — summary capstone: every property in one card
// obj10 — every determinant property, indexed against the sections above
const determinantProperties = {
  kicker: 'Determinants · summary',
  title: 'What each operation does to the determinant',
  tallyLabel: 'properties',
  intro: 'Eight properties, each linked to the section that establishes it. The three row operations at the top are the ones that make computation by reduction possible — everything below follows from them.',
  footnote: 'The pattern worth carrying away: operations that preserve the row space leave the determinant alone or scale it predictably, and only a swap changes its sign. That is why [row reduction](#4) can compute a determinant at all — every step\u2019s effect is known in advance.',
  laws: [
    {
      name: 'Row swap',
      anchor: '#1',
      statement: '$R_i \\leftrightarrow R_k \\;\\Rightarrow\\; \\det(B) = -\\det(A)$',
      verdict: 'conditional',
      verdictLabel: 'Sign flips',
      holdsWhen: 'any two distinct rows exchanged',
      note: 'The only operation that changes the sign. An even number of swaps restores it, which is why the sign of a permutation is well defined and why the determinant can be built from permutations at all. Swapping a row with itself changes nothing, consistent with the rule.',
    },
    {
      name: 'Row scaling',
      anchor: '#2',
      statement: '$kR_i \\to R_i \\;\\Rightarrow\\; \\det(B) = k\\det(A)$',
      verdict: 'conditional',
      holdsWhen: 'one row scaled by $k$',
      commonError: 'applying this to the whole matrix — see the next entry',
      note: 'The determinant is linear in each row separately. Scaling a single row by $k$ scales the determinant by $k$; scaling by $k = 0$ produces a zero row and a zero determinant, which is the same fact as a dependent row set.',
    },
    {
      name: 'Scaling the whole matrix',
      anchor: '#2',
      statement: '$\\det(kA) = k^n\\det(A)$',
      verdict: 'conditional',
      holdsWhen: '$A$ is $n \\times n$',
      commonError: 'writing $\\det(kA) = k\\det(A)$',
      note: 'Scaling every entry means scaling all $n$ rows, so the factor applies $n$ times. This is the single most common determinant error, and it comes from reading the row-scaling rule one line too broadly.',
      witness: {
        label: 'Witness',
        lines: [
          'A = I\u2082,  det(A) = 1',
    ...
    },
    {
      name: 'Row addition',
      anchor: '#3',
      statement: '$R_i + cR_k \\to R_i \\;\\Rightarrow\\; \\det(B) = \\det(A)$',
      verdict: 'holds',
      requires: '$i \\neq k$',
      note: 'The determinant is untouched. This is the operation [Gaussian elimination](!/linear-algebra/linear-systems/gaussian-elimination) leans on hardest — it can be applied freely without bookkeeping, which is what makes reduction a practical route to the determinant.',
    },
    {
      ...
      verdict: 'holds',
      requires: '$A$ square',
      note: 'Every statement about rows has a column counterpart, and neither is primary. Cofactor expansion may be taken along any row or any column for the same reason.',
    },
    {
      ...
      verdict: 'conditional',
      holdsWhen: '$A$ and $B$ both $n \\times n$',
      note: 'The determinant is a homomorphism from matrix multiplication to real multiplication. Geometrically the volume scaling factors compose, which is the whole content of the identity. Note there is deliberately no corresponding rule for sums.',
    },
    {
      ...
      verdict: 'conditional',
      holdsWhen: '$\\det(A) \\neq 0$',
      note: 'Falls straight out of the multiplicative property applied to $AA^{-1} = I$. It also proves a singular matrix has no [inverse](!/linear-algebra/matrix/inverse) \u2014 no real number multiplied by zero gives one.',
    },
    {
      name: 'Sum of matrices',
      anchor: '#5',
      statement: '$\\det(A + B) \\neq \\det(A) + \\det(B)$',
      verdict: 'fails',
      failsWhen: 'in general \u2014 no rule exists',
      note: 'Listed because its absence is otherwise inferred from silence. The determinant is linear in each row separately, which is a much weaker statement than being linear in the matrix, and the two are easy to conflate.',
      witness: {
        label: 'Witness',
        lines: [
          'A = [[1,0],[0,0]],  B = [[0,0],[0,1]]',
  ...
  const genericSections=[
    ...
    },
    {
        ...
        content:[
          ...
          <DiagramFrame
            key={'obj9-diagram'}
            id="determinant-invertibility-ring"
            title="One question, twelve answers"
            source="/linear-algebra/determinants/properties"
            formats={['png', 'pdf', 'svg']}
          >
    ...
    },
    {
        ...
        content:[
          ...
          <DiagramFrame
            ...
            source="/linear-algebra/determinants/properties"
          >
            <PropertyLawCard data={determinantProperties} theme="navy" />
          </DiagramFrame>,
        ]
    },
]
