...
$\\det(A) \\neq 0$. The matrix $A$ is invertible. The [rank](!/linear-algebra/matrices/rank) of $A$ equals $n$. The columns of $A$ are [linearly independent](!/linear-algebra/vector-spaces/linear-independence). The rows of $A$ are linearly independent. The columns of $A$ [span](!/linear-algebra/vector-spaces/span) $\\mathbb{R}^n$. The columns of $A$ form a [basis](!/linear-algebra/vector-spaces/basis) for $\\mathbb{R}^n$. The system $Ax = \\mathbf{b}$ has exactly one solution for every $\\mathbf{b} \\in \\mathbb{R}^n$. The homogeneous system $Ax = \\mathbf{0}$ has only the trivial solution. The [null space](!/linear-algebra/vector-spaces/fundamental-subspaces) of $A$ is $\\{\\mathbf{0}\\}$. The matrix $A$ can be written as a product of elementary matrices. The [reduced row echelon form](!/linear-algebra/linear-systems/rref) of $A$ is the identity matrix $I_n$. All [eigenvalues](!/linear-algebra/eigenvalues-vectors) of $A$ are nonzero.
