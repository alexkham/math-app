...
const keyWords = [
  ...
  // obj6 — aggregation: classification of quadratic forms / symmetric matrices by definiteness
  // obj6 — the definiteness classes, read off the eigenvalue signs
  const definitenessClasses = {
    kicker: 'Decompositions \u00B7 quadratic forms',
    title: 'Definiteness, read from the signs',
    tallyLabel: 'classes',
    intro: 'The quadratic form $\\mathbf{x}^{\\mathsf{T}}A\\mathbf{x}$ is classified entirely by the signs of the eigenvalues \u2014 not their sizes. Five classes, and the surface each one describes follows immediately.',
    footnote: 'In the eigenbasis the form becomes $\\sum \\lambda_i y_i^2$, and the sign question answers itself: a sum of squares with all-positive weights is positive, with mixed weights it takes both signs. Everything below is that substitution read off. In practice [Cholesky](!/linear-algebra/decompositions/cholesky) tests definiteness faster than computing the eigenvalues does.',
    slots: [
      { key: 'signs',   label: 'eigenvalue signs' },
      { key: 'form',    label: 'sign of the form' },
      { key: 'surface', label: 'level surface' },
    ],
    groups: [
      {
        heading: 'One sign throughout',
        types: [
          {
            ...
            properties: {
              signs: 'all strictly positive',
              form: '$> 0$ for every $\\mathbf{x} \\neq \\mathbf{0}$',
              surface: 'ellipsoid',
            },
            note: 'The best-behaved case: invertible, [Cholesky](!/linear-algebra/decompositions/cholesky)-factorizable, and the form has a strict minimum at the origin. This is the condition a covariance matrix and a Hessian at a minimum both satisfy.',
          },
          {
            ...
            condition: 'all $\\lambda_i < 0$',
            properties: {
              signs: 'all strictly negative',
              form: '$< 0$ for every $\\mathbf{x} \\neq \\mathbf{0}$',
              surface: 'inverted ellipsoid',
            },
            note: 'The mirror image, and testable by checking that $-A$ is positive definite. A strict maximum rather than a minimum.',
          },
        ],
      },
      {
        heading: 'One sign, with zeros',
        types: [
          {
            name: 'Positive semi-definite',
            anchor: '#6',
            shape: 'block',
            condition: 'all $\\lambda_i \\geq 0$, at least one $= 0$',
            properties: {
              signs: 'non-negative, some zero',
              form: '$\\geq 0$; vanishes on the null space',
              surface: 'degenerate ellipsoid \u2014 a cylinder',
            },
            note: 'Singular, so not invertible, and the form is flat along the null space rather than curving. The surface is a cylinder because one axis has no curvature at all \u2014 the zero eigenvalue is that direction.',
          },
          {
            name: 'Negative semi-definite',
            anchor: '#6',
            shape: 'strictUpper',
            condition: 'all $\\lambda_i \\leq 0$, at least one $= 0$',
            properties: {
              signs: 'non-positive, some zero',
              form: '$\\leq 0$; vanishes on the null space',
              surface: 'inverted cylinder',
            },
            note: 'The mirror of the row above. Both semi-definite cases are the boundary between definite and indefinite, which is why they appear exactly where an optimisation problem becomes degenerate.',
          },
        ],
      },
      {
        heading: 'Both signs present',
        types: [
          {
            name: 'Indefinite',
            anchor: '#6',
            shape: 'skew',
            condition: 'some $\\lambda_i > 0$ and some $< 0$',
            properties: {
              signs: 'mixed',
  ...
  const genericSections=[
    ...
    },
    {
        ...
        content:[
          ...
          <DiagramFrame
            key={'obj6-diagram'}
            id="definiteness-classes"
            title="Definiteness, read from the signs"
            source="/linear-algebra/decompositions/spectral"
          >
            <ObjectTypeProfile data={definitenessClasses} theme="navy" variant="grid" />
          </DiagramFrame>,
          `The classification is immediate once the decomposition is in hand. Substituting $\\mathbf{x} = Q\\mathbf{y}$ turns the form into $\\sum \\lambda_i y_i^2$ — a weighted sum of squares — and the sign of a sum of squares is decided by the signs of the weights and nothing else. The magnitudes only stretch the surface; they never change which of the five classes it belongs to.`,
        ]
    },
    {
        id:'7',
