...
// obj10 — summary capstone: closure under operations on transformations
// obj10 — every way of combining linear maps, and what each one preserves
const combinationProperties = {
  kicker: 'Transformations · summary',
  title: 'How linear maps combine',
  tallyLabel: 'operations',
  intro: 'Every operation here produces another linear map \u2014 that closure is what lets the set of all linear maps be a vector space in its own right, rather than merely a collection.',
  footnote: 'Read downward and the entries build on each other: composition and inversion give $\\mathcal{L}(V,V)$ the structure of an algebra, while sum and scalar multiple give $\\mathcal{L}(V,W)$ the structure of a vector space. The last entry is the two facts combined.',
  laws: [
    {
      name: 'Composition',
      anchor: '#6',
      statement: '$(S \\circ T)(\\mathbf{u}) = S\\bigl(T(\\mathbf{u})\\bigr)$',
      verdict: 'conditional',
      verdictLabel: 'Stays linear',
      holdsWhen: 'the codomain of $T$ matches the domain of $S$',
      note: 'Apply $T$\u2019s linearity inside $S$\u2019s and the result falls out. In matrix terms this is exactly [matrix multiplication](!/linear-algebra/matrix/operations) \u2014 which is why that product is defined the way it is, and why it fails to commute: $S \\circ T$ and $T \\circ S$ are different maps.',
    },
    {
      name: 'Inverse',
      anchor: '#7',
      statement: '$T^{-1} \\circ T = I, \\quad T \\circ T^{-1} = I$',
      verdict: 'conditional',
      verdictLabel: 'Stays linear',
      holdsWhen: '$T$ is a bijection',
      failsWhen: '$T$ has nontrivial kernel or misses part of the codomain',
      note: 'The inverse of a linear bijection is itself linear \u2014 not obvious, but true. An invertible map between spaces of equal dimension is an isomorphism, meaning the two spaces are structurally the same object written twice. See the [invertibility equivalence](!/linear-algebra/matrix/inverse) for the matrix form of the condition.',
    },
    {
      name: 'Sum',
      anchor: '#8',
      statement: '$(S + T)(\\mathbf{v}) = S(\\mathbf{v}) + T(\\mathbf{v})$',
      verdict: 'conditional',
      verdictLabel: 'Stays linear',
      holdsWhen: '$S$ and $T$ share domain and codomain',
      note: 'Defined pointwise, and linear because both summands are. This is what supplies the addition on $\\mathcal{L}(V, W)$ \u2014 the operation exists because linear maps are closed under it, not by stipulation.',
    },
    {
      name: 'Scalar multiple',
      anchor: '#8',
      statement: '$(cT)(\\mathbf{v}) = c \\cdot T(\\mathbf{v})$',
      verdict: 'holds',
      requires: 'any scalar $c$',
      note: 'Scaling a linear map keeps it linear, including at $c = 0$, which gives the zero map \u2014 the additive identity of $\\mathcal{L}(V, W)$. Together with the sum above, this is the second of the two operations a vector space needs.',
    },
    {
      name: 'The space of linear maps',
      anchor: '#8',
      statement: '$\\dim \\mathcal{L}(V, W) = \\dim(V) \\cdot \\dim(W)$',
      verdict: 'conditional',
      holdsWhen: 'both $V$ and $W$ finite-dimensional',
      note: 'Closure under sum and scalar multiple makes $\\mathcal{L}(V, W)$ a [vector space](!/linear-algebra/vector-spaces/axioms) whose elements happen to be functions. Its dimension is the product of the two, which is the same count as the entries of an $m \\times n$ matrix \u2014 the [matrix representation](!/linear-algebra/transformations/matrix-representation) is a basis for it.',
    },
    {
      name: 'The zero vector is fixed',
      anchor: '#5',
      statement: '$T(\\mathbf{0}) = \\mathbf{0}$',
      verdict: 'holds',
      requires: '$T$ linear',
      commonError: 'treating this as a definition \u2014 it is a consequence, and its failure is a fast disproof',
      note: 'Follows from $T(0 \\cdot \\mathbf{v}) = 0 \\cdot T(\\mathbf{v})$. Useful in the contrapositive: any map sending $\\mathbf{0}$ somewhere else is not linear, no further work needed. This is why an affine map $\\mathbf{x} \\mapsto A\\mathbf{x} + \\mathbf{b}$ with $\\mathbf{b} \\neq \\mathbf{0}$ fails immediately.',
      witness: {
        label: 'Fast disproof',
        lines: [
          'T(x, y) = (x + 1, y)',
  ...
  const genericSections=[
    ...
    },
    {
        ...
        content:[
          ...
          <DiagramFrame
            key={'obj10-diagram'}
            id="linear-map-combinations"
            title="How linear maps combine"
            source="/linear-algebra/transformations/properties"
          >
            <PropertyLawCard data={combinationProperties} theme="navy" />
          </DiagramFrame>,
        ]
    },
]
