...
// obj10 — summary capstone: matrix structure → canonical form achievable via similarity
// obj10 — how simple a matrix can be made, sorted by what it must satisfy
const canonicalForms = {
  kicker: 'Transformations \u00B7 canonical forms',
  title: 'How simple the matrix can be made',
  tallyLabel: 'cases',
  intro: 'Choosing a basis means choosing $P$, and the question is how simple $P^{-1}AP$ can be made. The answer depends entirely on what $A$ satisfies \u2014 and the list is ordered from the strongest structure to no structure at all.',
  footnote: 'Every row is the same operation with a different guarantee: conjugate by $P$ and read the result. Similarity preserves eigenvalues, trace, determinant, rank and characteristic polynomial, so nothing about the transformation is lost in any of these \u2014 only the coordinates change, and the entries below say how much simplicity that buys.',
  groups: [
    {
      heading: 'Diagonal \u2014 the best case',
      identities: [
        {
          name: 'Distinct eigenvalues',
          anchor: '#5',
          formula: '$D = P^{-1}AP$',
          condition: '$n$ distinct $\\lambda_i$',
          note: 'Eigenvectors for distinct eigenvalues are automatically independent, so $P$ is invertible without further checking. Sufficient but not necessary \u2014 the row below covers the repeated case, and $I$ diagonalizes trivially with one eigenvalue.',
        },
        {
          name: 'Multiplicities agree',
          anchor: '#5',
          formula: '$D = P^{-1}AP$',
          condition: '$m_g(\\lambda_i) = m_a(\\lambda_i)$ for every $i$',
          key: true,
          note: 'The [necessary and sufficient condition](!/linear-algebra/eigen/diagonalization). Columns of $P$ are $n$ independent eigenvectors; the question is only whether that many exist, and the multiplicity comparison answers it eigenvalue by eigenvalue.',
        },
        {
          name: 'Real symmetric',
          anchor: '#7',
          formula: '$D = P^{\\mathsf{T}}AP$',
          condition: '$A = A^{\\mathsf{T}}$ \u2014 always succeeds',
          key: true,
          note: 'The [spectral theorem](!/linear-algebra/decompositions/spectral) guarantees not just diagonalizability but an orthonormal eigenbasis, so $P^{-1} = P^{\\mathsf{T}}$ and nothing needs inverting. The only row where the hypothesis is checkable at a glance.',
        },
      ],
    },
    {
      heading: 'As close as the matrix allows',
      identities: [
        {
          name: 'Defective',
          anchor: '#6',
          formula: '$J = P^{-1}AP$',
          condition: '$m_g(\\lambda) < m_a(\\lambda)$ for some $\\lambda$',
          strict: true,
          note: 'Block diagonal with ones on the superdiagonal. The columns of $P$ are eigenvectors plus generalized eigenvectors, and the number of ones counts exactly how many eigenvectors are missing \u2014 the Jordan form measures the failure rather than working around it.',
        },
        {
          name: 'Any complex square matrix',
          anchor: '#3',
          formula: '$T = U^{*}AU$, upper triangular',
          condition: 'no hypotheses; $U$ unitary',
          key: true,
          note: 'Schur decomposition. Triangular rather than diagonal, but it exists for every square matrix over $\\mathbb{C}$ and $U$ is unitary, which makes it numerically stable where the Jordan form is not \u2014 the reason libraries compute Schur and not Jordan.',
        },
      ],
    },
  ],
    ...
    },
    {
        ...
        content:[
          ...
          <DiagramFrame
            ...
            id="canonical-forms"
            title="How simple the matrix can be made"
            source="/linear-algebra/transformations/basis-change"
          >
