...
// obj4 — aggregation: projection formulas by basis type (consolidates obj1, obj3, obj4)
// obj4 — four projection formulas, ordered by how much the basis is assumed
const projectionFormulas = {
  kicker: 'Orthogonality \u00B7 projection formulas',
  title: 'Four formulas, one operation',
  tallyLabel: 'cases',
  intro: 'All four compute the same thing \u2014 the closest point of the target to $\\mathbf{b}$. They differ only in what is assumed about the basis, and the formula gets simpler as the assumption gets stronger.',
  footnote: 'Reading upward is the argument for [Gram\u2013Schmidt](!/linear-algebra/orthogonality/gram-schmidt). The general formula needs a matrix inverse; orthogonality removes the coupling between coefficients; normalisation removes the denominators. Each assumption bought by preprocessing is a computation avoided on every projection afterwards.',
  slots: [
    { key: 'formula',  label: 'formula' },
    { key: 'requires', label: 'requires' },
    { key: 'cost',     label: 'cost per projection' },
  ],
  groups: [
    {
      heading: 'Onto a single vector',
      types: [
        {
          name: 'A line',
          anchor: '#1',
          shape: 'diagonal',
          condition: 'the target is $\\operatorname{Span}\\{\\mathbf{a}\\}$',
          properties: {
            formula: '$\\dfrac{\\mathbf{b} \\cdot \\mathbf{a}}{\\mathbf{a} \\cdot \\mathbf{a}}\\,\\mathbf{a}$',
            requires: '$\\mathbf{a} \\neq \\mathbf{0}$',
            cost: 'two dot products',
          },
          note: 'The base case, and the one every formula below is built from. The denominator is what normalisation removes \u2014 for a unit vector it is $1$ and disappears.',
        },
      ],
    },
    {
      heading: 'Onto a subspace \u2014 basis assumed orthogonal',
      types: [
        {
          name: 'Orthogonal basis',
          anchor: '#3',
          shape: 'block',
          condition: '$\\mathbf{u}_i \\cdot \\mathbf{u}_j = 0$ for $i \\neq j$',
          properties: {
            formula: '$\\sum \\dfrac{\\mathbf{b} \\cdot \\mathbf{u}_i}{\\mathbf{u}_i \\cdot \\mathbf{u}_i}\\,\\mathbf{u}_i$',
            requires: 'pairwise orthogonal, nonzero',
            cost: '$2k$ dot products',
          },
          note: 'A sum of independent line projections. Orthogonality is what makes the terms independent \u2014 each coefficient can be computed without knowing the others, and adding a basis vector does not disturb them.',
        },
        {
          name: 'Orthonormal basis',
          anchor: '#3',
          shape: 'identity',
          condition: 'orthogonal **and** $\\|\\mathbf{q}_i\\| = 1$',
          properties: {
            formula: '$\\sum (\\mathbf{b} \\cdot \\mathbf{q}_i)\\,\\mathbf{q}_i$',
            requires: 'pairwise orthogonal, unit length',
            cost: '$k$ dot products',
          },
          note: 'The denominators vanish because each is $1$. This is the form worth arranging for \u2014 see [orthonormal operations](!/linear-algebra/orthogonality/orthogonal-sets), where every operation simplifies the same way.',
        },
      ],
    },
    {
      heading: 'Onto a subspace \u2014 nothing assumed',
      types: [
        {
          name: 'Arbitrary basis',
          anchor: '#4',
          shape: 'dense',
          condition: 'columns of $A$ span the target',
          properties: {
            formula: '$A(A^{\\mathsf{T}}A)^{-1}A^{\\mathsf{T}}\\mathbf{b}$',
            requires: 'columns of $A$ independent',
            cost: 'a $k \\times k$ inverse',
          },
          note: 'The general case, and the expensive one. $(A^{\\mathsf{T}}A)^{-1}$ exists exactly when the columns are independent \u2014 and when they happen to be orthonormal, $A^{\\mathsf{T}}A = I$ and this collapses to the row above.',
        },
      ],
    },
  ],
  ...
  notation: {
    ...
    entries: [
      ...
      },
      {
        ...
        means: `The letter $P$'s third job on this site: [change-of-basis](!/linear-algebra/vector-spaces/basis#notation) matrix, [permutation](!/linear-algebra/matrix/types#12) matrix, and now projection — built in **The Projection Matrix** below. The equation $P^2 = P$ is idempotence written as notation: project twice, land in the same place.`,
    ...
    },
    {
        ...
        content:[
          ...
          <DiagramFrame
            ...
            source="/linear-algebra/orthogonality/projections"
          >
            <ObjectTypeProfile data={projectionFormulas} theme="navy" variant="stack" />
          </DiagramFrame>,
          `Ordered this way the four are a single formula losing assumptions. Start at the bottom with an arbitrary basis and the projection needs a $k \\times k$ matrix inverse. Assume the basis orthogonal and the inverse disappears, because $A^{\\mathsf{T}}A$ becomes diagonal and the coefficients stop depending on each other. Assume it orthonormal as well and the denominators go too, since each is one.`,
        ]
    },
    {
        id:'5',
