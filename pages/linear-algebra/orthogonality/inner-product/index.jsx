...
// obj8 — aggregation: the three axioms of an inner product alongside their dot-product instances
// obj8 — the three axioms an inner product must satisfy, and what each one buys
const innerProductAxioms = {
  kicker: 'Orthogonality \u00B7 general inner products',
  title: 'The three axioms',
  tallyLabel: 'axioms',
  intro: 'The [dot product](!/linear-algebra/vectors/dot-product) is one inner product among many. These three conditions are what any candidate must satisfy \u2014 and each one is responsible for a specific piece of geometry that would otherwise not exist.',
  footnote: 'Every geometric notion on this page is derived from these three and nothing else. That is why the same machinery works on function spaces, matrix spaces and polynomial spaces \u2014 [Gram\u2013Schmidt](!/linear-algebra/orthogonality/gram-schmidt) and [projection](!/linear-algebra/orthogonality/projections) never look at what the vectors are, only at the axioms they obey.',
  laws: [
    {
      name: 'Symmetry',
      anchor: '#8',
      statement: '$\\langle \\mathbf{u}, \\mathbf{v} \\rangle = \\langle \\mathbf{v}, \\mathbf{u} \\rangle$',
      verdict: 'conditional',
      holdsWhen: 'real scalars',
      failsWhen: 'complex scalars \u2014 becomes $\\langle \\mathbf{u}, \\mathbf{v} \\rangle = \\overline{\\langle \\mathbf{v}, \\mathbf{u} \\rangle}$',
      note: 'Order of the arguments is irrelevant over $\\mathbb{R}$. Over $\\mathbb{C}$ the axiom weakens to conjugate symmetry, and it has to \u2014 without the conjugate, positive definiteness below would fail, since $\\langle i\\mathbf{v}, i\\mathbf{v} \\rangle$ would come out negative.',
    },
    {
      name: 'Linearity in the first argument',
      anchor: '#8',
      statement: '$\\langle c\\mathbf{u} + d\\mathbf{w}, \\mathbf{v} \\rangle = c\\langle \\mathbf{u}, \\mathbf{v} \\rangle + d\\langle \\mathbf{w}, \\mathbf{v} \\rangle$',
      verdict: 'holds',
      requires: 'any scalars $c, d$',
      note: 'Combined with symmetry this gives linearity in both arguments \u2014 the form is bilinear, so an inner product distributes across [linear combinations](!/linear-algebra/vectors/linear-combinations) on either side. Only one argument needs stating; symmetry supplies the other.',
    },
    {
      name: 'Positive definiteness',
      anchor: '#8',
      statement: '$\\langle \\mathbf{v}, \\mathbf{v} \\rangle \\geq 0$',
      verdict: 'conditional',
      holdsWhen: 'equality iff $\\mathbf{v} = \\mathbf{0}$',
      note: 'The axiom that supplies length. It makes $\\|\\mathbf{v}\\| = \\sqrt{\\langle \\mathbf{v}, \\mathbf{v} \\rangle}$ real, non-negative, and zero only at the origin \u2014 see [norm properties](!/linear-algebra/vectors/magnitude). Drop the strictness and nonzero vectors could have zero length, which breaks normalization and every distance argument built on it.',
    },
    {
      name: 'Cauchy\u2013Schwarz',
      anchor: '#6',
      statement: '$|\\langle \\mathbf{u}, \\mathbf{v} \\rangle| \\leq \\|\\mathbf{u}\\| \\, \\|\\mathbf{v}\\|$',
      verdict: 'conditional',
      verdictLabel: 'Derived',
      holdsWhen: 'equality iff $\\mathbf{u}$ and $\\mathbf{v}$ are parallel',
      note: 'Not an axiom \u2014 a consequence of the three above. It is what makes [angle](#5) definable at all: without it, $\\langle \\mathbf{u}, \\mathbf{v} \\rangle / (\\|\\mathbf{u}\\|\\|\\mathbf{v}\\|)$ could fall outside $[-1, 1]$ and have no arccosine.',
    },
    {
      ...
      verdict: 'conditional',
      verdictLabel: 'Derived',
      holdsWhen: 'equality iff $\\mathbf{u}, \\mathbf{v}$ point the same way',
      note: 'Follows from Cauchy\u2013Schwarz by squaring both sides. This is what makes the induced norm a genuine norm and the induced distance a genuine metric \u2014 the three axioms give geometry, but only through this chain.',
    },
    {
      name: 'Pythagorean theorem',
      anchor: '#10',
      statement: '$\\mathbf{u} \\perp \\mathbf{v} \\Rightarrow \\|\\mathbf{u} + \\mathbf{v}\\|^2 = \\|\\mathbf{u}\\|^2 + \\|\\mathbf{v}\\|^2$',
      verdict: 'conditional',
      verdictLabel: 'Derived',
      holdsWhen: '$\\langle \\mathbf{u}, \\mathbf{v} \\rangle = 0$',
      note: 'Expand $\\|\\mathbf{u}+\\mathbf{v}\\|^2 = \\langle \\mathbf{u}+\\mathbf{v}, \\mathbf{u}+\\mathbf{v} \\rangle$ by bilinearity and the cross terms vanish. The classical theorem in $\\mathbb{R}^2$ turns out to be a statement about any inner product space \u2014 it holds for functions and matrices just as well.',
    },
  ],
}
  ...
  const genericSections=[
    ...
    },
    {
        ...
        content:[
          ...
          <DiagramFrame
            key={'obj8-diagram'}
            id="inner-product-axioms"
            title="The three axioms"
            source="/linear-algebra/orthogonality/inner-product"
          >
