...
const faqQuestions = {
  obj1: {
    question: "Why is the range harder to find than the domain?",
    answer: "Because the domain is read off the formula directly by excluding what breaks it, while the range asks what values the function actually reaches, which depends on its whole behaviour. There is no single mechanical rule. You usually have to reason from the graph, from known shapes, or by solving for the input.",
    sectionId: "1"
  },
  obj2: {
    question: "What is the range of a quadratic function?",
    answer: "Everything from the vertex upward if the parabola opens up, and everything from the vertex downward if it opens down. The vertex supplies the single boundary value, and the sign of the leading coefficient decides which side is kept. This is why finding the vertex is the whole job for quadratics.",
    sectionId: "7"
  },
  obj3: {
    question: "Is the range of a function always all real numbers?",
    answer: "No. Only certain families reach every real value, such as odd-degree polynomials and the logarithm. Squaring never produces a negative, exponentials never reach zero, and sine stays between −1 and 1. The range depends entirely on the shape of the function, so it must be worked out rather than assumed.",
    sectionId: "7"
  },
  obj4: {
    question: "How does shifting a graph vertically change its range?",
    answer: "It moves every output by the same amount, so the range shifts by exactly that amount while keeping its shape. A horizontal shift leaves the range untouched, since it changes which inputs produce the outputs, not the outputs themselves. Vertical stretches scale the range; reflections across the horizontal axis flip it.",
    sectionId: "8"
  }
}
...
export default function RangePage({
  ...
  />
   ...
   <br/>
   <br/>
   <h1 className='title' style={{marginTop:'0px',marginBottom:'10px'}}>Range of a Function</h1>
   <br/>
   <br/>
   <SectionTableOfContents sections={genericSections}
    showSecondaryNav={true}
