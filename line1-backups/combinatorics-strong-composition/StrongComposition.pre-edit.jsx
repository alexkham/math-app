// /components/visual-tools/scenes/StrongComposition.jsx
// Scenario #9 — Strong Composition, C(n−1, k−1).
// Distribute n identical items into k distinct bins, each bin holding ≥ 1 item.
// Visual: n items in cells with (n−1) gap drop-zones between them; (k−1)
// bars fly in from a holding row to insert into chosen gaps. Groups by x₁
// (items in the first bin) ∈ {1..n−k+1}; per-group ROW_H since group sizes
// vary as C(n−j−1, k−2). Mirrors WeakComposition-v2 in style and controls.
...
// Strong requires n ≥ k. Caps keep totals manageable; max C(6,3) = 20.
const K_MAX_BY_N = { 3: 3, 4: 4, 5: 4, 6: 4, 7: 4 };
...
// Strong compositions of n into k positive parts: xᵢ ≥ 1, Σxᵢ = n.
// Ordered lex (x₁ ascending, then x₂, …), grouping naturally by first part.
function strongCompositions(n, k) {
  if (k === 1) return n >= 1 ? [[n]] : [];
  if (n < k) return [];
  const out = [];
  for (let j = 1; j <= n - (k - 1); j++) {
    for (const rest of strongCompositions(n - j, k - 1)) {
      out.push([j, ...rest]);
    }
  }
  return out;
...
// Gap positions (0-indexed in 0..n−2) where bars sit for a given composition.
// Bar i is in the gap just after the (x₁+…+xᵢ)-th item.
function gapPositionsFromStrongComp(comp) {
  const positions = [];
  let acc = 0;
  for (let i = 0; i < comp.length - 1; i++) {
    acc += comp[i];
    positions.push(acc - 1);
  }
  return positions;
}

// Item range [first, last] (0-indexed item indices) for each bin.
function binItemRanges(comp) {
  const ranges = [];
  let acc = 0;
  for (const x of comp) {
    ranges.push({ first: acc, last: acc + x - 1 });
    acc += x;
  }
  return ranges;
}
...
// Item strip + gap drop-zones
const STRIP_CY = 200;
const CELL_W = 48;        // item cell width
const CELL_H = 56;
const GAP_W = 24;         // gap (drop-zone) width between items

const BALL_R = 16;
const BAR_BUILD_H = 38;
const BAR_BUILD_W = 7;

const GAP_INDICATOR_W = 12;
const GAP_INDICATOR_H = 32;

const BAR_FILL = COLORS.accent;

// Vertical offsets below cellBottom (parallel to Weak v2)
...
export default function StrongComposition() {
  ...
  // Clamp k whenever n drops below current k's max (and enforce k ≤ n).
  useEffect(() => {
    const maxK = Math.min(K_MAX_BY_N[n] || K_MAX, n);
    if (k > maxK) setK(maxK);
  }, [n, k]);
  ...
  const outcomes = useMemo(() => strongCompositions(n, k), [n, k]);
  const totalCount = outcomes.length;

  const distinctFirsts = useMemo(
    () => Array.from({ length: n - k + 1 }, (_, j) => j + 1),
    [n, k]
  );
  const groupSizes = useMemo(
    () => distinctFirsts.map((j) => binomial(n - j - 1, k - 2)),
    [distinctFirsts, n, k]
  );

  const currentComp = outcomes[outcomeIdx] || [];
  const currentGapPositions = useMemo(
    () => gapPositionsFromStrongComp(currentComp),
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [outcomes, outcomeIdx]
  );
  const currentItemRanges = useMemo(
    () => binItemRanges(currentComp),
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [outcomes, outcomeIdx]
  );
  ...
  const stepForward = useCallback(() => {
    ...
    if (animState === "idle") {
      if (itemsPerOutcome === 0) {
        // k = 1 case (single outcome [n]); jump straight to complete
  ...
  const stepBack = useCallback(() => {
    ...
    if (animState === "complete") {
      setAnimState(itemsPerOutcome === 0 ? "idle" : "landed");
      return;
    }
    if (animState === "landed") {
  ...
  // ── Card layout (adaptive by numItems) ────────────────
  const cardLayout = useMemo(() => {
    ...
    return {
      MINI_CW, MINI_CH, MINI_GW, MINI_BALL_R, MINI_BAR_H, MINI_BAR_W,
  ...
  const placedGaps = currentGapPositions.slice(0, slotsFilled);
  const placedGapSet = new Set(placedGaps);

  // xᵢ known once bar min(i, k−2) has landed → slotsFilled >= min(i+1, k−1)
  const partKnown = (i) => slotsFilled >= Math.min(i + 1, k - 1);

  // ── Header texts ──────────────────────────────────────
  const formulaShort = `C(${n - 1}, ${k - 1}) = ${totalCount}`;
  const numerator = factorial(n - 1);
  const denomA = factorial(k - 1);
  const denomB = factorial(n - k);
  const formulaFull =
  ...
  const narrationFor = (gi) => {
    ...
    if (j === n - k + 1) {
      // Maximum x₁: leaves exactly 1 item per remaining bin → only 1 outcome
      return (
        <>
          When the first bin holds <b>{j}</b> item{j === 1 ? "" : "s"}{" "}
          (<b>x₁ = {j}</b>), the remaining <b>{nRem}</b> items are split
          among <b>{kRem}</b> bin{kRem === 1 ? "" : "s"} with each holding
          exactly 1 — just <b>1</b> outcome.
        </>
      );
    }
    if (j === 1) {
      return (
        <>
          When the first bin holds just <b>1</b> item (<b>x₁ = 1</b>), the
          remaining <b>n − 1 = {nRem}</b> items must be split among the
          other <b>k − 1 = {kRem}</b> bin{kRem === 1 ? "" : "s"} (each still
          needing ≥ 1). That&apos;s{" "}
          <b>C(n−2, k−2) = C({n - 2}, {k - 2}) = {sz}</b>{" "}
          outcome{sz === 1 ? "" : "s"}.
        </>
      );
    }
    return (
      <>
        When the first bin holds <b>{j}</b> items (<b>x₁ = {j}</b>), the
        remaining <b>n − {j} = {nRem}</b> items are split among the other{" "}
        <b>k − 1 = {kRem}</b> bin{kRem === 1 ? "" : "s"} (each ≥ 1), giving{" "}
        <b>C({nRem - 1}, {kRem - 1}) = {sz}</b>{" "}
        outcome{sz === 1 ? "" : "s"}.
      </>
    );
  };
  ...
  function renderStripCard(comp, cx, cy) {
    ...
    const compGaps = gapPositionsFromStrongComp(comp);
    const gapBarSet = new Set(compGaps);
    ...
    for (let c = 0; c < numItems; c++) {
      const ccx = cx + CARD_PAD_X + c * (MINI_CW + MINI_GW) + MINI_CW / 2;
      elems.push(
        <Ball
          key={`it-${c}`}
          item={itemForStar}
  ...
  const kMaxForN = Math.min(K_MAX_BY_N[n] || K_MAX, n);
  ...
  return (
    <VTRoot>
      ...
      <PageWrap>
        <HelpBanner>
          Press <b>▶ Play</b> to build all <b>C(n−1, k−1)</b> strong
          compositions of <b>n</b> identical items into <b>k</b> bins, where
          every bin must hold at least one item. A <b>bin</b> is a run of
          consecutive items between two chosen gaps (or before the first /
          after the last); the count of items in bin <b>i</b> is{" "}
          <b>xᵢ ≥ 1</b>. Each outcome = a choice of <b>k − 1</b> gap
          positions out of the <b>n − 1</b> gaps between adjacent items.
        </HelpBanner>
        ...
        <ControlBar>
          ...
          <NStepper
            ...
            min={N_MIN_SC}
            max={N_MAX_SC}
            tip="Number of identical items (must satisfy n ≥ k)."
          />
          <Divider />
          <KStepper
            value={k}
            ...
            min={K_MIN}
            max={kMaxForN}
            tip="Number of bins (each must hold at least one item)."
          />
          <Divider />
          <TransportButtons
        ...
        <SceneGrid>
          ...
          <SceneSvgWrap>
            ...
            >
              ...
              <text
                ...
                fontFamily="'JetBrains Mono',monospace" letterSpacing="2"
              >
                ITEMS (n = {n}) · GAPS (n − 1 = {numGaps})
              </text>

              {/* Gap labels above gaps (1-indexed; highlight when bar lands) */}
              {Array.from({ length: numGaps }, (_, g) => {
                const hasBar = placedGapSet.has(g);
                return (
                  <text
                    key={`glbl-${g}`}
                    x={gapX(g)} y={cellTop - 6}
                    textAnchor="middle"
                    fontSize={hasBar ? 12 : 11}
                    fontWeight={hasBar ? 700 : 500}
                    fontFamily="'JetBrains Mono',monospace"
                    fill={hasBar ? COLORS.accentDeep : COLORS.textFaint}
              ...
              {/* Items (Balls/Letters) at every item position */}
              {Array.from({ length: numItems }, (_, c) => (
                <Ball
                  key={`item-${c}`}
                  item={itemForStar}
                  cx={itemX(c)} cy={STRIP_CY}
                  r={BALL_R} mode={mode}
                />
              ))}

              {/* Gap drop-zone indicators (dashed) at gaps without a bar */}
              {Array.from({ length: numGaps }, (_, g) => {
                if (placedGapSet.has(g)) return null;
                // Skip if this is the currently flying bar's target
                if (g === currentGapPositions[flyingBarIdx] && isFlying) return null;
                return (
                  <rect
                    key={`gap-${g}`}
                    x={gapX(g) - GAP_INDICATOR_W / 2}
                    y={STRIP_CY - GAP_INDICATOR_H / 2}
                    width={GAP_INDICATOR_W} height={GAP_INDICATOR_H} rx="3"
                    fill="none"
                    stroke={COLORS.textFaint} strokeWidth="1"
                    strokeDasharray="3 3" opacity="0.65"
                  />
                );
              })}

              {/* Dotted guide for flying bar */}
              {flyingBarIdx >= 0 &&
               currentGapPositions[flyingBarIdx] !== undefined && (() => {
                const targetG = currentGapPositions[flyingBarIdx];
              ...
              {/* Placed bars (at chosen gaps) */}
              {placedGaps.map((g, i) => (
                <BarGlyph
                  key={`placed-${outcomeIdx}-${i}`}
                  cx={gapX(g)} cy={STRIP_CY}
              ...
              {/* Bin brackets below items (cell-only, progressive labels) */}
              {currentItemRanges.map((r, i) => {
                ...
                const padX = 4;
                const lineX1 = xLeft + padX, lineX2 = xRight - padX;
                return (
                  <g key={`bracket-${i}`}>
                    <line
                      x1={lineX1} y1={bracketLineY}
                      x2={lineX2} y2={bracketLineY}
                      stroke={COLORS.borderStrong} strokeWidth="1.5"
                    />
                    <line
                      x1={lineX1} y1={bracketLineY}
                      x2={lineX1} y2={bracketLineY - 4}
                      stroke={COLORS.borderStrong} strokeWidth="1.5"
                    />
                    <line
                      x1={lineX2} y1={bracketLineY}
                      x2={lineX2} y2={bracketLineY - 4}
                      stroke={COLORS.borderStrong} strokeWidth="1.5"
                    />
                    <text
                      x={xMid} y={bracketLabelY} textAnchor="middle"
                      fontSize="12" fontWeight="700"
                      fontFamily="'JetBrains Mono',monospace"
                      fill={known ? COLORS.accentDeep : COLORS.textFaint}
                    >
                      x{SUBSCRIPTS[i]} = {known ? currentComp[i] : "?"}
                    </text>
                  </g>
                );
              })}
              ...
              {/* Chosen-gaps readout (right) — progressive */}
              <text
                x={SVG_W / 2 + 110} y={compReadoutY} textAnchor="middle"
                fontSize="13" fontWeight="700"
                fontFamily="'JetBrains Mono',monospace"
                fill={COLORS.text}
              >
                <tspan fill={COLORS.textDim}>chosen gaps: {"{"}</tspan>
          ...
          <RightPanel>
            <InfoPanelHeader
              ...
              title="Strong compositions (positive parts)"
              formula={formulaText}
              intro={
                <>
                  Distribute <b>n</b> identical items into <b>k</b> distinct
                  bins, with every bin holding at least one item. Count the
                  solutions to <b>x₁ + x₂ + … + xₖ = n</b> in positive
                  integers (<b>xᵢ ≥ 1</b>). Equivalently: choose <b>k − 1</b>{" "}
                  of the <b>n − 1</b> gaps between adjacent items to place
                  dividers — no two at the same gap, none at the ends.
                </>
              }
            />
