...
//
// Both are element-wise: pair entries by position, add or
// subtract, write the result. The vector case is the matrix
// case with a single row.
//
// Controls:
//   - Operation toggle: addition / subtraction (both scenarios)
//   - Scenario pills: vectors / matrices (only in mode='both')
//   - Dimensions: vector length, or rows×cols (linked A,B,C)
//
// MODE prop:
...
const ADDITION_INFO =
  'Addition and subtraction are element-wise: pair each entry with its ' +
  'counterpart at the same position, combine them, and write the result. ' +
  'The same operation applies to vectors (length must match) and to ' +
  'matrices (shape must match) — only the shape of the operands differs.';

const SAME_SHAPE_INFO =
  'A and B must have the same dimensions — the same number of rows and the same number of columns. The result C has that same shape, and each cell of C is computed from the corresponding pair of cells in A and B.';

const SAME_LENGTH_INFO =
  'u and v must have the same length. Each entry of u is paired with the ' +
  'entry at the same position in v; the result w has that same length.';
...
function buildVectorScenes(n, operation) {
  ...
  scenes.push({
    title: `Vector ${opWord}`,
    formula:
      `u and v both have length ${n}. To compute w = u ${opSymbol} v, ` +
      `pair up each entry of u with its counterpart in v and ${opVerb} them.`,
  ...
  for (let j = 0; j < n; j++) {
    scenes.push({
      title:
        `w<sub>${j + 1}</sub> = ` +
        `u<sub>${j + 1}</sub> ${opSymbol} v<sub>${j + 1}</sub>`,
      formula:
        `Take entry ${j + 1} from each of u and v, ` +
        `and write their ${opNoun} into w[${j + 1}].`,
  ...
  scenes.push({
    ...
    formula:
      ...
      `u<sub>k</sub> ${opSymbol} v<sub>k</sub>. ` +
      `${isAdd ? 'Addition' : 'Subtraction'} is element-wise: ` +
      `the result has the same length as the inputs, and each entry ` +
      `depends only on its own pair.`,
...
.aw-pill:hover { border-color: #94a3b8; }
.aw-pill-active:hover { border-color: #2563eb; }
...
function Pill({ active, onClick, children }) {
  return (
    <button
      className={active ? 'aw-pill aw-pill-active' : 'aw-pill'}
      onClick={onClick}
      style={{
        fontSize: '13px',
        padding: '6px 12px',
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        {!locked && (
          <div>
            <FieldLabel info={ADDITION_INFO}>Scenario</FieldLabel>
        ...
        {scenario === 'vectors' ? (
          <div>
            <FieldLabel info={SAME_LENGTH_INFO}>
