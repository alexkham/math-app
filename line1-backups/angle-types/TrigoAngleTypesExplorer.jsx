...
// // ═══════════════════════════════════════════════════════════════════════════════
// // CONTAINER WIDTH HOOK (for responsive layout)
// // ═══════════════════════════════════════════════════════════════════════════════
...
// // ═══════════════════════════════════════════════════════════════════════════════
// // SHARED LAYOUT WRAPPER (now responsive)
// // ═══════════════════════════════════════════════════════════════════════════════

// function CardLayout({ dark, scene, explanation }) {
//   const { border, sceneBg } = useTheme(dark);
//   const [ref, width] = useContainerWidth();
//   const stack = width > 0 && width < 560;

//   return (
//     <div ref={ref} style={{
//       display: "flex",
//       flexDirection: stack ? "column" : "row",
//       minHeight: stack ? "auto" : "480px",
//     }}>
//       <div style={{
//         flex: stack ? "0 0 auto" : "0 0 360px",
//         background: sceneBg,
//         display: "flex",
...
//         touchAction: "none",
//         userSelect: "none",
//         borderRight: stack ? "none" : `1.5px solid ${border}`,
//         borderBottom: stack ? `1.5px solid ${border}` : "none",
//       }}>
//         {scene}
//       </div>
//       <div style={{
...
// function MiniCard({ label, sublabel, value, color, panelBg }) {
//   return (
//     <div style={{ flex: 1, minWidth: 0, background: panelBg, borderRadius: "10px", border: `1.5px solid ${color}22`, padding: "10px", textAlign: "center" }}>
//       <div style={{ fontSize: "10px", fontWeight: "600", color, textTransform: "uppercase", letterSpacing: "0.07em", marginBottom: sublabel ? "2px" : "4px" }}>{label}</div>
//       {sublabel && <div style={{ fontSize: "10px", color, opacity: 0.6, marginBottom: "4px" }}>{sublabel}</div>}
//       <div style={{ fontSize: "26px", fontWeight: "900", color, lineHeight: 1, letterSpacing: "-0.03em" }}>{value}°</div>
//     </div>
...
//       scene={(
//         <div ref={svgRef} onPointerDown={onPD} style={{ cursor:"crosshair", touchAction:"none", display:"flex", flexDirection:"column", alignItems:"center", gap:"12px" }}>
//           <svg width={SZ} height={SZ} viewBox={`0 0 ${SZ} ${SZ}`} style={{ display:"block", touchAction:"none", maxWidth:"100%", height:"auto" }}>
//             {!isFull && delta > 0 && <path d={secPath(VX, VY, ARC, 0, a2)} fill={bg} stroke="none" />}
//             {isFull && <circle cx={VX} cy={VY} r={ARC} fill="none" stroke={c} strokeWidth={2.5} />}
//             {!isFull && delta > 0 && (isRight
//               ? <path d={boxPath(VX, VY, 0, a2)} fill="none" stroke={c} strokeWidth={2.5} strokeLinejoin="miter" />
...
//             </g>
//           </svg>
//           <div style={{ display:"flex", background:dark?"#0f172a":"#f1f5f9", border:`1.5px solid ${border}`, borderRadius:"9px", padding:"3px", gap:"3px", width:"260px", maxWidth:"100%" }}>
//             {Object.entries(CMODES).map(([k, m]) => {
//               const act = mode === k;
//               return <button key={k} onClick={() => handleMode(k)} style={{ flex:1, padding:"5px 8px", borderRadius:"6px", border:"none", background:act?BLUE:"transparent", color:act?"#fff":textSec, fontSize:"11px", fontWeight:act?"700":"400", fontFamily:"system-ui,sans-serif", cursor:"pointer" }}>{m.label}</button>;
//             })}
...
//           <p style={{ margin:0, fontSize:"12px", lineHeight:"1.65", color:textSec }}>These appear in trig via co-function identities: sin(θ) = cos(90°−θ).</p>
//           <Divider border={border} />
//           <div style={{ display:"flex", alignItems:"baseline", gap:"6px", fontSize:"17px", fontWeight:"700", justifyContent:"center", flexWrap:"wrap" }}>
//             <span style={{ color:BLUE }}>{alpha}°</span><span style={{ color:textSec, opacity:0.4 }}>+</span><span style={{ color:AMBER }}>{beta}°</span><span style={{ color:textSec, opacity:0.4 }}>=</span><span style={{ color:textPri }}>{md.sum}</span>
//           </div>
//           <div style={{ display:"flex", gap:"8px" }}>
//             <MiniCard label="α" value={alpha} color={BLUE}  panelBg={panelBg} />
...
//           <div style={{ padding:"10px 12px", borderRadius:"10px", background:panelBg, border:`1.5px solid ${border}`, fontSize:"12px", color:textSec, lineHeight:"1.7" }}>
//             <div style={{ display:"flex", alignItems:"center", gap:"7px", marginBottom:"4px" }}>
//               <span style={{ width:9, height:9, borderRadius:"50%", background:BLUE,  display:"inline-block", flexShrink:0 }} />
//               <span>α = α <strong style={{ color:BLUE }}>( vertical pair )</strong></span>
//             </div>
//             <div style={{ display:"flex", alignItems:"center", gap:"7px" }}>
...
//           <p style={{ margin:0, fontSize:"12px", lineHeight:"1.65", color:textSec }}>Their sum has no fixed total. They appear in the angle addition identities.</p>
//           <Divider border={border} />
//           <div style={{ display:"flex", alignItems:"baseline", gap:"5px", fontSize:"16px", fontWeight:"700", justifyContent:"center", flexWrap:"wrap" }}>
//             <span style={{ color:BLUE }}>{alpha}°</span><span style={{ color:textSec, opacity:0.4 }}>+</span><span style={{ color:AMBER }}>{beta}°</span><span style={{ color:textSec, opacity:0.4 }}>=</span><span style={{ color:PURPLE }}>{total}°</span>
//           </div>
//           <div style={{ display:"flex", gap:"8px" }}>
//             <MiniCard label="α" value={alpha} color={BLUE}  panelBg={panelBg} />
...
//           <Divider border={border} />
//           <div style={{ fontSize:"40px", fontWeight:"900", lineHeight:1, color:arcCol, letterSpacing:"-0.04em" }}>{angle}<span style={{ fontSize:"16px" }}>°</span></div>
//           <div style={{ display:"inline-block", padding:"3px 10px", borderRadius:"999px", fontSize:"11px", fontWeight:"700", color:qi.color, background:`${qi.color}18`, border:`1.5px solid ${qi.color}33`, alignSelf:"flex-start" }}>{qi.label}</div>
//           <div style={{ display:"flex", gap:"6px" }}>
//             {[["sin",qi.sin],["cos",qi.cos],["tan",qi.tan]].map(([fn, sign]) => {
//               const col = sign==="+"?GREEN : sign==="−"?RED : "#64748b";
//               return <div key={fn} style={{ flex:1, minWidth:0, display:"flex", alignItems:"center", justifyContent:"center", gap:"4px", padding:"6px", borderRadius:"8px", background:panelBg, border:`1.5px solid ${col}22` }}><span style={{ fontSize:"11px", fontWeight:"600", color:"#64748b", fontFamily:"monospace" }}>{fn}</span><span style={{ fontSize:"14px", fontWeight:"900", color:col, fontFamily:"monospace" }}>{sign}</span></div>;
//             })}
//           </div>
//           <Divider border={border} />
...
//             <MiniCard label="ref" value={Math.round(ri.ref)} color={AMBER} panelBg={panelBg} />
//           </div>
//           {ri.roman !== "—" && <div style={{ padding:"8px 12px", borderRadius:"8px", background:`${ri.color}12`, border:`1.5px solid ${ri.color}33`, fontFamily:"monospace", fontSize:"11px", color:ri.color, wordBreak:"break-word" }}>{ri.formula}</div>}
//           <Divider border={border} />
//           {formulas.map(row => { const act=ri.roman===row.q; const col=qColors[row.q]||textSec; return <div key={row.q} style={{ display:"flex", alignItems:"center", gap:"8px", padding:"6px 10px", borderRadius:"7px", marginBottom:"3px", background:act?`${col}12`:"transparent", border:`1.5px solid ${act?col+"33":"transparent"}` }}><span style={{ fontSize:"11px", fontWeight:"700", color:col, minWidth:"22px" }}>{row.q}</span><span style={{ fontSize:"11px", fontFamily:"monospace", color:act?col:textSec, fontWeight:act?"700":"400" }}>{row.f}</span></div>; })}
//         </>
//       )}
...
//             </svg>
//           </div>
//           <div style={{ display:"flex", alignItems:"center", gap:"12px", maxWidth:"100%", flexWrap:"wrap", justifyContent:"center" }}>
//             <button onClick={() => setOffset(o => Math.max(o-1,-3))} disabled={offset <= -3} style={{ width:32, height:32, borderRadius:"7px", border:`1.5px solid ${offset<=-3?"transparent":border}`, background:"transparent", color:offset<=-3?"transparent":textSec, fontSize:"17px", cursor:offset<=-3?"default":"pointer", fontFamily:"inherit", display:"flex", alignItems:"center", justifyContent:"center" }}>−</button>
//             <div style={{ textAlign:"center", minWidth:"160px" }}>
//               <div style={{ fontSize:"11px", color:textSec, marginBottom:"2px" }}>n = <strong style={{ color:AMBER }}>{offset}</strong></div>
//               <div style={{ fontSize:"13px", fontWeight:"700", color:AMBER }}>{base} + ({offset}×360°) = {cot}°</div>
...
//       scene={(
//         <div style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:"12px" }}>
//           <svg width={SZ} height={SZ} viewBox={`0 0 ${SZ} ${SZ}`} style={{ display:"block", maxWidth:"100%", height:"auto" }}>
//             <line x1={12} y1={CY} x2={SZ-12} y2={CY} stroke={sub} strokeWidth={1.5} /><polygon points={`${SZ-8},${CY} ${SZ-16},${CY-4} ${SZ-16},${CY+4}`} fill={sub} />
//             <line x1={CX} y1={SZ-12} x2={CX} y2={12} stroke={sub} strokeWidth={1.5} /><polygon points={`${CX},8 ${CX-4},16 ${CX+4},16`} fill={sub} />
//             <circle cx={CX} cy={CY} r={R} fill="none" stroke={sub} strokeWidth={1.5} opacity={0.5} />
//             {a.deg > 0 && a.deg < 360 && <path d={secPath(CX,CY,30,0,a.deg)} fill={BBG} stroke="none" />}
...
//           <Divider border={border} />
//           <div style={{ display:"flex", gap:"8px" }}>
//             <div style={{ flex:1, minWidth:0, background:panelBg, borderRadius:"10px", border:`1.5px solid ${col}22`, padding:"10px", textAlign:"center" }}><div style={{ fontSize:"10px", fontWeight:"600", color:col, textTransform:"uppercase", letterSpacing:"0.07em", marginBottom:"3px" }}>Degrees</div><div style={{ fontSize:"24px", fontWeight:"900", color:col, lineHeight:1 }}>{a.deg}°</div></div>
//             <div style={{ flex:1, minWidth:0, background:panelBg, borderRadius:"10px", border:`1.5px solid ${col}22`, padding:"10px", textAlign:"center" }}><div style={{ fontSize:"10px", fontWeight:"600", color:col, textTransform:"uppercase", letterSpacing:"0.07em", marginBottom:"3px" }}>Radians</div><div style={{ fontSize:"18px", fontWeight:"900", color:col, lineHeight:1, fontFamily:"monospace", paddingTop:"3px" }}>{a.rad}</div></div>
//           </div>
//           {[["sin",a.sin],["cos",a.cos],["tan",a.tan]].map(([fn, val]) => {
//             const pos = !val.startsWith("−") && val !== "0" && val !== "undef.";
//             const c = val === "undef." ? "#64748b" : pos ? GREEN : val === "0" ? "#64748b" : RED;
//             return <div key={fn} style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"6px 10px", borderRadius:"8px", background:panelBg, border:`1.5px solid ${border}`, gap:"8px" }}><span style={{ fontSize:"12px", fontWeight:"600", color:c, fontFamily:"monospace" }}>{fn}({a.deg}°)</span><span style={{ fontSize:"12px", fontWeight:"700", color:c, fontFamily:"monospace" }}>{val}</span></div>;
//           })}
//           <Divider border={border} />
//           <div style={{ overflowY:"auto", maxHeight:"160px", borderRadius:"8px", border:`1.5px solid ${border}` }}>
...
//           </div>
//           <div style={{ display:"flex", gap:"8px" }}>
//             <div style={{ flex:1, minWidth:0, background:panelBg, borderRadius:"9px", border:`1.5px solid ${RED}22`, padding:"8px", textAlign:"center" }}><div style={{ fontSize:"9px", fontWeight:"600", color:RED, textTransform:"uppercase", marginBottom:"3px", fontFamily:"monospace" }}>sin({angle}°)</div><div style={{ fontSize:"18px", fontWeight:"900", color:RED, lineHeight:1, fontFamily:"monospace" }}>{sinV.toFixed(2)}</div></div>
//             <div style={{ flex:1, minWidth:0, background:panelBg, borderRadius:"9px", border:`1.5px solid ${GREEN}22`, padding:"8px", textAlign:"center" }}><div style={{ fontSize:"9px", fontWeight:"600", color:GREEN, textTransform:"uppercase", marginBottom:"3px", fontFamily:"monospace" }}>cos({angle}°)</div><div style={{ fontSize:"18px", fontWeight:"900", color:GREEN, lineHeight:1, fontFamily:"monospace" }}>{cosV.toFixed(2)}</div></div>
//           </div>
//           <Divider border={border} />
//           <div style={{ padding:"9px 12px", borderRadius:"9px", background:panelBg, border:`1.5px solid ${border}`, fontFamily:"monospace", fontSize:"11px", lineHeight:"1.8", color:textSec, overflowWrap:"break-word" }}>
//             <div><span style={{ color:RED }}>sin({angle}°)</span> = <span style={{ color:RED, fontWeight:"700" }}>{sinV.toFixed(3)}</span></div>
//             <div><span style={{ color:RED }}>sin({-angle}°)</span> = <span style={{ color:RED, fontWeight:"700" }}>{sinN.toFixed(3)}</span> ✓ negated</div>
//             <div style={{ marginTop:"4px" }}><span style={{ color:GREEN }}>cos({angle}°)</span> = <span style={{ color:GREEN, fontWeight:"700" }}>{cosV.toFixed(3)}</span></div>
//             <div><span style={{ color:GREEN }}>cos({-angle}°)</span> = <span style={{ color:GREEN, fontWeight:"700" }}>{cosN.toFixed(3)}</span> ✓ same</div>
...
//   const current = CONCEPTS.find((c) => c.id === active);
//   const { surface, border, textPri, textSec, panelBg } = useTheme(dark);
//   const narrow = hubWidth > 0 && hubWidth < 760;
...
//           gap: "12px",
//         }}>
//           <div style={{ display: "flex", alignItems: "baseline", gap: "10px", minWidth: 0, overflow: "hidden" }}>
//             <span style={{
//               fontSize: "16px",
//               fontWeight: "800",
//               color: textPri,
...
//               whiteSpace: "nowrap",
//             }}>Angle Types Explorer</span>
//             {current && !narrow && <span style={{
//               fontSize: "12px",
//               color: textSec,
//               whiteSpace: "nowrap",
//               overflow: "hidden",
...
function CardLayout({ dark, scene, explanation }) {
  const { border, sceneBg } = useTheme(dark);
  const [ref, width] = useContainerWidth();
  const stack = width > 0 && width < 672;

  return (
    <div ref={ref} style={{
      display: "flex",
      flexDirection: stack ? "column" : "row",
      minHeight: stack ? "auto" : "576px",
    }}>
      <div style={{
        flex: stack ? "0 0 auto" : "0 0 432px",
        background: sceneBg,
        display: "flex",
        flexDirection: "column",
        alignItems: "center",
        ...
        touchAction: "none",
        userSelect: "none",
        borderRight: stack ? "none" : `1.8px solid ${border}`,
        borderBottom: stack ? `1.8px solid ${border}` : "none",
      }}>
        {scene}
      </div>
      <div style={{
...
function MiniCard({ label, sublabel, value, color, panelBg }) {
  return (
    <div style={{ flex: 1, minWidth: 0, background: panelBg, borderRadius: "12px", border: `1.8px solid ${color}22`, padding: "12px", textAlign: "center" }}>
      <div style={{ fontSize: "12px", fontWeight: "600", color, textTransform: "uppercase", letterSpacing: "0.07em", marginBottom: sublabel ? "2px" : "5px" }}>{label}</div>
      {sublabel && <div style={{ fontSize: "12px", color, opacity: 0.6, marginBottom: "5px" }}>{sublabel}</div>}
      <div style={{ fontSize: "31px", fontWeight: "900", color, lineHeight: 1, letterSpacing: "-0.03em" }}>{value}°</div>
    </div>
  );
}
...
function BasicAngles({ dark }) {
  ...
  return (
    <CardLayout dark={dark}
      scene={(
        <div ref={svgRef} onPointerDown={onPD} style={{ cursor:"crosshair", touchAction:"none", display:"flex", flexDirection:"column", alignItems:"center", gap:"14px" }}>
          <svg width={SZ} height={SZ} viewBox={`0 0 ${SZ} ${SZ}`} style={{ display:"block", touchAction:"none", maxWidth:"100%", height:"auto" }}>
            {!isFull && delta > 0 && <path d={secPath(VX, VY, ARC, 0, a2)} fill={bg} stroke="none" />}
            {isFull && <circle cx={VX} cy={VY} r={ARC} fill="none" stroke={c} strokeWidth={2.5} />}
            {!isFull && delta > 0 && (isRight
            ...
            <line x1={VX} y1={VY} x2={a1e.x} y2={a1e.y} stroke={c} strokeWidth={2.5} strokeLinecap="round" />
            <line x1={VX} y1={VY} x2={a2e.x} y2={a2e.y} stroke={c} strokeWidth={2.5} strokeLinecap="round" />
            <circle cx={VX} cy={VY} r={5.4} fill={c} />
            {!isFull && delta > 0 && !isRight && <text x={lp.x} y={lp.y} textAnchor="middle" dominantBaseline="central" fontSize={16} fontWeight="700" fill={c} style={FS}>{angle}°</text>}
            <g style={{ cursor:"grab" }}>
              <circle cx={a2e.x} cy={a2e.y} r={19} fill="transparent" />
              <circle cx={a2e.x} cy={a2e.y} r={10} fill={c} opacity={0.92} stroke="white" strokeWidth={2} />
            </g>
          </svg>
          <div style={{ display:"flex", flexWrap:"wrap", gap:"6px", justifyContent:"center" }}>
            {BASIC_TYPES.map(t => (
              <button key={t.name} onClick={() => setAngle(t.preset)} style={{ padding:"5px 12px", borderRadius:"8px", border:`1.8px solid ${meta.name===t.name?t.color:border}`, background:meta.name===t.name?t.bg:"transparent", color:meta.name===t.name?t.color:textSec, fontSize:"13px", fontWeight:meta.name===t.name?"700":"400", fontFamily:"system-ui,sans-serif", cursor:"pointer" }}>
                {t.name}
              </button>
            ))}
          </div>
    ...
    <CardLayout dark={dark}
      scene={(
        <div ref={svgRef} onPointerDown={onPD} style={{ cursor:"crosshair", touchAction:"none", display:"flex", flexDirection:"column", alignItems:"center", gap:"14px" }}>
          <svg width={SZ} height={SZ} viewBox={`0 0 ${SZ} ${SZ}`} style={{ display:"block", touchAction:"none", maxWidth:"100%", height:"auto" }}>
            ...
            <line x1={VX} y1={VY} x2={a1e.x} y2={a1e.y} stroke={sub} strokeWidth={2.5} strokeLinecap="round" />
            <line x1={VX} y1={VY} x2={a2e.x} y2={a2e.y} stroke={BLUE} strokeWidth={2.5} strokeLinecap="round" />
            <circle cx={VX} cy={VY} r={5} fill={BLUE} />
            {alpha > 0 && <text x={alp.x} y={alp.y} textAnchor="middle" dominantBaseline="central" fontSize={14} fontWeight="700" fill={BLUE}  style={FS}>{alpha}°</text>}
            {beta  > 0 && <text x={blp.x} y={blp.y} textAnchor="middle" dominantBaseline="central" fontSize={14} fontWeight="700" fill={AMBER} style={FS}>{beta}°</text>}
            <g style={{ cursor:"grab" }}>
              <circle cx={a2e.x} cy={a2e.y} r={19} fill="transparent" />
              <circle cx={a2e.x} cy={a2e.y} r={10} fill={BLUE} opacity={0.92} stroke="white" strokeWidth={2} />
            </g>
          </svg>
          <div style={{ display:"flex", background:dark?"#0f172a":"#f1f5f9", border:`1.8px solid ${border}`, borderRadius:"11px", padding:"4px", gap:"4px", width:"312px", maxWidth:"100%" }}>
            {Object.entries(CMODES).map(([k, m]) => {
              const act = mode === k;
              return <button key={k} onClick={() => handleMode(k)} style={{ flex:1, padding:"6px 10px", borderRadius:"7px", border:"none", background:act?BLUE:"transparent", color:act?"#fff":textSec, fontSize:"13px", fontWeight:act?"700":"400", fontFamily:"system-ui,sans-serif", cursor:"pointer" }}>{m.label}</button>;
            })}
          </div>
        </div>
      )}
      explanation={(
        <>
          ...
          <Divider border={border} />
          <div style={{ padding:"11px 14px", borderRadius:"11px", background:panelBg, border:`1.8px solid ${border}`, fontFamily:"monospace", fontSize:"13px", color:textPri, lineHeight:"1.8" }}>
            <div>sin(θ) = cos(90°−θ)</div>
            <div>cos(θ) = sin(90°−θ)</div>
            <div>tan(θ) = cot(90°−θ)</div>
          </div>
    ...
    <CardLayout dark={dark}
      scene={(
        <div ref={svgRef} style={{ touchAction:"none", userSelect:"none" }}>
          <svg width={SZ} height={SZ} viewBox={`0 0 ${SZ} ${SZ}`} style={{ display:"block", touchAction:"none", maxWidth:"100%", height:"auto" }}>
            ...
            <line x1={aW.x} y1={aW.y} x2={aE.x} y2={aE.y} stroke={sub}  strokeWidth={2.5} strokeLinecap="round" />
            <line x1={bS.x} y1={bS.y} x2={bN.x} y2={bN.y} stroke={BLUE} strokeWidth={2.5} strokeLinecap="round" />
            <circle cx={VX} cy={VY} r={5.4} fill={BLUE} />
            {!isPerp && <>
              <text x={lA1.x} y={lA1.y} textAnchor="middle" dominantBaseline="central" fontSize={13} fontWeight="700" fill={BLUE}  style={FS}>{alpha}°</text>
              <text x={lB1.x} y={lB1.y} textAnchor="middle" dominantBaseline="central" fontSize={13} fontWeight="700" fill={AMBER} style={FS}>{beta}°</text>
              <text x={lA2.x} y={lA2.y} textAnchor="middle" dominantBaseline="central" fontSize={13} fontWeight="700" fill={BLUE}  style={FS}>{alpha}°</text>
              <text x={lB2.x} y={lB2.y} textAnchor="middle" dominantBaseline="central" fontSize={13} fontWeight="700" fill={AMBER} style={FS}>{beta}°</text>
            </>}
            {[bN, bS].map((pt, i) => (
              <g key={i} onPointerDown={(e) => onDrag(e, i === 0)} style={{ cursor:"grab", touchAction:"none" }}>
                <circle cx={pt.x} cy={pt.y} r={19} fill="transparent" />
                <circle cx={pt.x} cy={pt.y} r={10}  fill={BLUE} opacity={0.92} stroke="white" strokeWidth={2} />
              </g>
            ))}
          </svg>
        </div>
      ...
      explanation={(
        <>
          ...
          <div style={{ padding:"12px 14px", borderRadius:"12px", background:panelBg, border:`1.8px solid ${border}`, fontSize:"14px", color:textSec, lineHeight:"1.7" }}>
            <div style={{ display:"flex", alignItems:"center", gap:"8px", marginBottom:"5px" }}>
              <span style={{ width:11, height:11, borderRadius:"50%", background:BLUE,  display:"inline-block", flexShrink:0 }} />
              <span>α = α <strong style={{ color:BLUE }}>( vertical pair )</strong></span>
            </div>
            <div style={{ display:"flex", alignItems:"center", gap:"8px" }}>
              <span style={{ width:11, height:11, borderRadius:"50%", background:AMBER, display:"inline-block", flexShrink:0 }} />
              <span>β = β <strong style={{ color:AMBER }}>( vertical pair )</strong></span>
            </div>
          </div>
        </>
    ...
    <CardLayout dark={dark}
      scene={(
        <div ref={svgRef} style={{ touchAction:"none", userSelect:"none" }}>
          <svg width={SZ} height={SZ} viewBox={`0 0 ${SZ} ${SZ}`} style={{ display:"block", touchAction:"none", maxWidth:"100%", height:"auto" }}>
            ...
            <line x1={VX} y1={VY} x2={a3e.x} y2={a3e.y} stroke={sub}    strokeWidth={2.5} strokeLinecap="round" />
            <line x1={VX} y1={VY} x2={a2e.x} y2={a2e.y} stroke={PURPLE} strokeWidth={2.5} strokeLinecap="round" strokeDasharray="6 3" />
            <circle cx={VX} cy={VY} r={5.4} fill={PURPLE} />
            <text x={alp.x} y={alp.y} textAnchor="middle" dominantBaseline="central" fontSize={14} fontWeight="700" fill={BLUE}  style={FS}>{alpha}°</text>
            <text x={blp.x} y={blp.y} textAnchor="middle" dominantBaseline="central" fontSize={14} fontWeight="700" fill={AMBER} style={FS}>{beta}°</text>
            <g onPointerDown={onA2} style={{ cursor:"grab", touchAction:"none" }}>
              <circle cx={a2e.x} cy={a2e.y} r={19} fill="transparent" />
              <circle cx={a2e.x} cy={a2e.y} r={10}  fill={BLUE} opacity={0.92} stroke="white" strokeWidth={2} />
            </g>
            <g onPointerDown={onA3} style={{ cursor:"grab", touchAction:"none" }}>
              <circle cx={a3e.x} cy={a3e.y} r={19} fill="transparent" />
              <circle cx={a3e.x} cy={a3e.y} r={10}  fill={AMBER} opacity={0.92} stroke="white" strokeWidth={2} />
            </g>
          </svg>
        </div>
      )}
      explanation={(
        <>
          ...
          <div style={{ padding:"11px 14px", borderRadius:"11px", background:panelBg, border:`1.8px solid ${border}`, fontFamily:"monospace", fontSize:"13px", color:textPri }}>
            sin(α+β) = sinα cosβ + cosα sinβ
          </div>
        </>
      )}
  ...
  return (
    <CardLayout dark={dark}
      scene={(
        <div ref={svgRef} onPointerDown={onPD} style={{ cursor:"crosshair", touchAction:"none", userSelect:"none" }}>
          <svg width={SZ} height={SZ} viewBox={`0 0 ${SZ} ${SZ}`} style={{ display:"block", touchAction:"none", maxWidth:"100%", height:"auto" }}>
            <line x1={14} y1={CY} x2={SZ-14} y2={CY} stroke={sub} strokeWidth={1.5} />
            <polygon points={`${SZ-10},${CY} ${SZ-19},${CY-5} ${SZ-19},${CY+5}`} fill={sub} />
            <line x1={CX} y1={SZ-14} x2={CX} y2={14} stroke={sub} strokeWidth={1.5} />
            <polygon points={`${CX},10 ${CX-6},19 ${CX+6},19`} fill={sub} />
            <text x={SZ-12} y={CY-10} textAnchor="middle" fontSize={13} fontStyle="italic" fill={sub} style={FS}>x</text>
            <text x={CX+10}  y={17}   textAnchor="start"  fontSize={13} fontStyle="italic" fill={sub} style={FS}>y</text>
            <circle cx={CX} cy={CY} r={R} fill="none" stroke={sub} strokeWidth={1} strokeDasharray="3 4" opacity={0.45} />
            {showArc && <path d={isNeg ? secPathCW(CX,CY,ARC,0,norm) : secPath(CX,CY,ARC,0,norm)} fill={isNeg ? ABG : BBG} stroke="none" />}
            {showArc && <path d={isNeg ? arcPathCW(CX,CY,ARC,0,norm) : arcPath(CX,CY,ARC,0,norm)} fill="none" stroke={arcCol} strokeWidth={2.5} strokeLinecap="round" />}
            {showArc && Math.abs(angle) > 20 && Math.abs(angle) < 355 && <text x={lp.x} y={lp.y} textAnchor="middle" dominantBaseline="central" fontSize={13} fontWeight="700" fill={arcCol} style={FS}>{angle}°</text>}
            {["I","II","III","IV"].map((q, i) => { const qa=[45,135,225,315]; const qp=polar(CX,CY,R*0.55,qa[i]); return <text key={q} x={qp.x} y={qp.y} textAnchor="middle" dominantBaseline="central" fontSize={13} fontWeight="600" fill={qi.roman===q?qi.color:sub} opacity={qi.roman===q?0.9:0.2} style={FS}>{q}</text>; })}
            <line x1={CX} y1={CY} x2={CX+ARM} y2={CY} stroke={sub} strokeWidth={2} strokeLinecap="round" />
            <line x1={CX} y1={CY} x2={termEnd.x} y2={termEnd.y} stroke={arcCol} strokeWidth={2.5} strokeLinecap="round" />
            <circle cx={polar(CX,CY,R,norm).x} cy={polar(CX,CY,R,norm).y} r={5} fill={arcCol} opacity={0.7} />
            <circle cx={CX} cy={CY} r={5.4} fill={arcCol} />
            <g style={{ cursor:"grab" }}>
              <circle cx={termEnd.x} cy={termEnd.y} r={19} fill="transparent" />
              <circle cx={termEnd.x} cy={termEnd.y} r={10} fill={arcCol} opacity={0.92} stroke="white" strokeWidth={2} />
            </g>
          </svg>
        </div>
      )}
      explanation={(
        <>
          ...
          <div style={{ display:"flex", gap:"7px" }}>
            {[["sin",qi.sin],["cos",qi.cos],["tan",qi.tan]].map(([fn, sign]) => {
              const col = sign==="+"?GREEN : sign==="−"?RED : "#64748b";
              return <div key={fn} style={{ flex:1, minWidth:0, display:"flex", alignItems:"center", justifyContent:"center", gap:"5px", padding:"7px", borderRadius:"10px", background:panelBg, border:`1.8px solid ${col}22` }}><span style={{ fontSize:"13px", fontWeight:"600", color:"#64748b", fontFamily:"monospace" }}>{fn}</span><span style={{ fontSize:"17px", fontWeight:"900", color:col, fontFamily:"monospace" }}>{sign}</span></div>;
            })}
          </div>
          <Divider border={border} />
          <table style={{ width:"100%", borderCollapse:"collapse", fontSize:"13px", tableLayout:"fixed" }}>
            <thead><tr>{["Q","sin","cos","tan"].map(h => <th key={h} style={{ padding:"5px 7px", textAlign:"center", color:textSec, fontWeight:"600", borderBottom:`1px solid ${border}`, fontSize:"12px", textTransform:"uppercase" }}>{h}</th>)}</tr></thead>
            <tbody>{QT.map(row => { const act=qi.roman===row.q; return <tr key={row.q} style={{ background:act?`${qi.color}12`:"transparent" }}><td style={{ padding:"5px 7px", textAlign:"center", color:act?qi.color:textSec, fontWeight:act?"700":"400" }}>{row.q}</td>{["sin","cos","tan"].map(fn => <td key={fn} style={{ padding:"5px 7px", textAlign:"center", fontWeight:"700", color:row[fn]==="+"?GREEN:row[fn]==="−"?RED:textSec }}>{row[fn]}</td>)}</tr>; })}</tbody>
          </table>
        </>
      )}
    />
  ...
  return (
    <CardLayout dark={dark}
      scene={(
        <div ref={svgRef} onPointerDown={onPD} style={{ cursor:"crosshair", touchAction:"none", userSelect:"none" }}>
          <svg width={SZ} height={SZ} viewBox={`0 0 ${SZ} ${SZ}`} style={{ display:"block", touchAction:"none", maxWidth:"100%", height:"auto" }}>
            <line x1={14} y1={CY} x2={SZ-14} y2={CY} stroke={sub} strokeWidth={1.5} /><polygon points={`${SZ-10},${CY} ${SZ-19},${CY-5} ${SZ-19},${CY+5}`} fill={sub} />
            <line x1={CX} y1={SZ-14} x2={CX} y2={14} stroke={sub} strokeWidth={1.5} /><polygon points={`${CX},10 ${CX-6},19 ${CX+6},19`} fill={sub} />
            <circle cx={CX} cy={CY} r={R} fill="none" stroke={sub} strokeWidth={1} strokeDasharray="3 4" opacity={0.4} />
            {showFull && <path d={secPath(CX,CY,ARCB,0,norm)} fill={BBG} stroke="none" />}
            {showFull && <path d={arcPath(CX,CY,ARCB,0,norm)} fill="none" stroke={BLUE} strokeWidth={1.5} strokeLinecap="round" opacity={0.4} />}
            {showFull && norm > 20 && <text x={fullLp.x} y={fullLp.y} textAnchor="middle" dominantBaseline="central" fontSize={12} fill={BLUE} opacity={0.55} fontWeight="600" style={FS}>{Math.round(norm)}°</text>}
            {showRef  && <path d={secPath(CX,CY,ARCA,ri.refFrom,ri.refTo)} fill={ABG} stroke="none" />}
            {showRef  && <path d={arcPath(CX,CY,ARCA,ri.refFrom,ri.refTo)} fill="none" stroke={AMBER} strokeWidth={2.5} strokeLinecap="round" />}
            {showRef  && ri.ref > 15 && <text x={refLp.x} y={refLp.y} textAnchor="middle" dominantBaseline="central" fontSize={13} fontWeight="700" fill={AMBER} style={FS}>{Math.round(ri.ref)}°</text>}
            {ri.axisAngle === 180 && <line x1={CX} y1={CY} x2={polar(CX,CY,ARM+10,180).x} y2={CY} stroke={AMBER} strokeWidth={1.5} strokeDasharray="5 4" opacity={0.5} strokeLinecap="round" />}
            <line x1={CX} y1={CY} x2={CX+ARM} y2={CY} stroke={sub} strokeWidth={2} strokeLinecap="round" />
            <line x1={CX} y1={CY} x2={termEnd.x} y2={termEnd.y} stroke={ri.color} strokeWidth={2.5} strokeLinecap="round" />
            {ri.roman !== "—" && (() => { const qa={I:45,II:135,III:225,IV:315}; const qp=polar(CX,CY,R*0.6,qa[ri.roman]); return <text x={qp.x} y={qp.y} textAnchor="middle" dominantBaseline="central" fontSize={14} fontWeight="700" fill={ri.color} opacity={0.7} style={FS}>{ri.roman}</text>; })()}
            <circle cx={CX} cy={CY} r={5.4} fill={ri.color} />
            <circle cx={polar(CX,CY,R,norm).x} cy={polar(CX,CY,R,norm).y} r={5} fill={ri.color} opacity={0.7} />
            <g style={{ cursor:"grab" }}>
              <circle cx={termEnd.x} cy={termEnd.y} r={19} fill="transparent" />
              <circle cx={termEnd.x} cy={termEnd.y} r={10} fill={ri.color} opacity={0.92} stroke="white" strokeWidth={2} />
            </g>
          </svg>
        </div>
      )}
      explanation={(
        <>
          ...
          {ri.roman !== "—" && <div style={{ padding:"10px 14px", borderRadius:"10px", background:`${ri.color}12`, border:`1.8px solid ${ri.color}33`, fontFamily:"monospace", fontSize:"13px", color:ri.color, wordBreak:"break-word" }}>{ri.formula}</div>}
          <Divider border={border} />
          {formulas.map(row => { const act=ri.roman===row.q; const col=qColors[row.q]||textSec; return <div key={row.q} style={{ display:"flex", alignItems:"center", gap:"10px", padding:"7px 12px", borderRadius:"8px", marginBottom:"4px", background:act?`${col}12`:"transparent", border:`1.8px solid ${act?col+"33":"transparent"}` }}><span style={{ fontSize:"13px", fontWeight:"700", color:col, minWidth:"26px" }}>{row.q}</span><span style={{ fontSize:"13px", fontFamily:"monospace", color:act?col:textSec, fontWeight:act?"700":"400" }}>{row.f}</span></div>; })}
        </>
      )}
    />
  );
        ...
        <div style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:"14px" }}>
          <div ref={svgRef} onPointerDown={onPD} style={{ cursor:"crosshair", touchAction:"none", userSelect:"none" }}>
            <svg width={SZ} height={SZ} viewBox={`0 0 ${SZ} ${SZ}`} style={{ display:"block", touchAction:"none", maxWidth:"100%", height:"auto" }}>
              ...
              {offset !== 0 && showArc && <path d={secPath(CX,CY,ARCB,0,norm)} fill={ABG} stroke="none" />}
              {offset !== 0 && showArc && <path d={arcPath(CX,CY,ARCB,0,norm)} fill="none" stroke={AMBER} strokeWidth={2.5} strokeLinecap="round" />}
              {offset !== 0 && norm > 20 && <text x={cLp.x} y={cLp.y} textAnchor="middle" dominantBaseline="central" fontSize={13} fontWeight="700" fill={AMBER} style={FS}>{cot}°</text>}
              {showArc && <path d={secPath(CX,CY,ARCA,0,norm)} fill={BBG} stroke="none" />}
              {showArc && <path d={arcPath(CX,CY,ARCA,0,norm)} fill="none" stroke={BLUE} strokeWidth={2.5} strokeLinecap="round" />}
              {norm > 20 && <text x={bLp.x} y={bLp.y} textAnchor="middle" dominantBaseline="central" fontSize={13} fontWeight="700" fill={BLUE} style={FS}>{base}°</text>}
              {offset !== 0 && showArc && <path d={spiralPath(ARCA,ARCB,0,norm)} fill="none" stroke={AMBER} strokeWidth={1} strokeDasharray="3 4" opacity={0.35} />}
              <line x1={CX} y1={CY} x2={CX+ARM} y2={CY} stroke={sub} strokeWidth={2} strokeLinecap="round" />
              <line x1={CX} y1={CY} x2={termEnd.x} y2={termEnd.y} stroke={BLUE} strokeWidth={2.5} strokeLinecap="round" />
              <circle cx={CX} cy={CY} r={5.4} fill={BLUE} />
              <g style={{ cursor:"grab" }}>
                <circle cx={termEnd.x} cy={termEnd.y} r={19} fill="transparent" />
                <circle cx={termEnd.x} cy={termEnd.y} r={10} fill={BLUE} opacity={0.92} stroke="white" strokeWidth={2} />
              </g>
            </svg>
          </div>
          <div style={{ display:"flex", alignItems:"center", gap:"14px", maxWidth:"100%", flexWrap:"wrap", justifyContent:"center" }}>
            <button onClick={() => setOffset(o => Math.max(o-1,-3))} disabled={offset <= -3} style={{ width:38, height:38, borderRadius:"8px", border:`1.8px solid ${offset<=-3?"transparent":border}`, background:"transparent", color:offset<=-3?"transparent":textSec, fontSize:"20px", cursor:offset<=-3?"default":"pointer", fontFamily:"inherit", display:"flex", alignItems:"center", justifyContent:"center" }}>−</button>
            <div style={{ textAlign:"center", minWidth:"192px" }}>
              <div style={{ fontSize:"13px", color:textSec, marginBottom:"2px" }}>n = <strong style={{ color:AMBER }}>{offset}</strong></div>
              <div style={{ fontSize:"16px", fontWeight:"700", color:AMBER }}>{base} + ({offset}×360°) = {cot}°</div>
            </div>
            <button onClick={() => setOffset(o => Math.min(o+1,3))}  disabled={offset >= 3}  style={{ width:38, height:38, borderRadius:"8px", border:`1.8px solid ${offset>=3?"transparent":border}`, background:"transparent", color:offset>=3?"transparent":textSec, fontSize:"20px", cursor:offset>=3?"default":"pointer", fontFamily:"inherit", display:"flex", alignItems:"center", justifyContent:"center" }}>+</button>
          </div>
        </div>
      )}
      explanation={(
        <>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:"7px", padding:"8px 12px", borderRadius:"10px", background:"rgba(22,163,74,0.08)", border:"1.8px solid rgba(22,163,74,0.2)", fontSize:"13px", color:GREEN, fontWeight:"600" }}>
            <span>✓</span><span>Same terminal side at {norm}°</span>
          </div>
          <div style={{ padding:"11px 14px", borderRadius:"10px", background:panelBg, border:`1.8px solid ${border}`, fontFamily:"monospace", fontSize:"13px", color:textPri, lineHeight:"1.8" }}>
            <div style={{ color:textSec }}>{base}° + n×360°</div>
            <div style={{ color:AMBER, fontWeight:"700" }}>n = {offset} → {cot}°</div>
          </div>
        </>
      ...
      scene={(
        <div style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:"14px" }}>
          <svg width={SZ} height={SZ} viewBox={`0 0 ${SZ} ${SZ}`} style={{ display:"block", maxWidth:"100%", height:"auto" }}>
            ...
            {SPECIAL.map((sp, i) => {
              ...
              return (
                <g key={i} onClick={() => setSel(i)} style={{ cursor:"pointer" }}>
                  <circle cx={pt.x} cy={pt.y} r={isSel?9:5} fill={c} opacity={isSel?1:0.55} stroke={isSel?"white":"none"} strokeWidth={2} />
                  <circle cx={pt.x} cy={pt.y} r={17} fill="transparent" />
                  <text x={lp.x} y={lp.y} textAnchor={anchor} dominantBaseline="central" fontSize={isSel?11:8} fontWeight={isSel?"700":"400"} fill={isSel?c:sub} opacity={isSel?1:0.7} style={FS}>{lbl}</text>
                  {mode === "both" && <text x={lp.x} y={lp.y + (lp.y > CY ? 11 : -11)} textAnchor={anchor} dominantBaseline="central" fontSize={isSel?10:7} fontWeight={isSel?"700":"400"} fill={isSel?c:sub} opacity={isSel?0.85:0.5} style={FS}>{sp.rad}</text>}
                </g>
              );
          ...
          <div style={{ display:"flex", background:dark?"#0f172a":"#f1f5f9", border:`1.8px solid ${border}`, borderRadius:"11px", padding:"4px", gap:"4px", width:"312px", maxWidth:"100%" }}>
            {[["degrees","Degrees"],["radians","Radians"],["both","Both"]].map(([m, lbl]) => {
              const act = mode === m;
              return <button key={m} onClick={() => setMode(m)} style={{ flex:1, padding:"6px 7px", borderRadius:"7px", border:"none", background:act?BLUE:"transparent", color:act?"#fff":textSec, fontSize:"12px", fontWeight:act?"700":"400", fontFamily:"system-ui,sans-serif", cursor:"pointer" }}>{lbl}</button>;
            })}
          </div>
        </div>
      )}
      explanation={(
        <>
          ...
          <div style={{ display:"flex", gap:"10px" }}>
            ...
            <div style={{ flex:1, minWidth:0, background:panelBg, borderRadius:"12px", border:`1.8px solid ${col}22`, padding:"12px", textAlign:"center" }}><div style={{ fontSize:"12px", fontWeight:"600", color:col, textTransform:"uppercase", letterSpacing:"0.07em", marginBottom:"4px" }}>Radians</div><div style={{ fontSize:"22px", fontWeight:"900", color:col, lineHeight:1, fontFamily:"monospace", paddingTop:"4px" }}>{a.rad}</div></div>
          </div>
          {[["sin",a.sin],["cos",a.cos],["tan",a.tan]].map(([fn, val]) => {
            const pos = !val.startsWith("−") && val !== "0" && val !== "undef.";
            const c = val === "undef." ? "#64748b" : pos ? GREEN : val === "0" ? "#64748b" : RED;
            return <div key={fn} style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"7px 12px", borderRadius:"10px", background:panelBg, border:`1.8px solid ${border}`, gap:"10px" }}><span style={{ fontSize:"14px", fontWeight:"600", color:c, fontFamily:"monospace" }}>{fn}({a.deg}°)</span><span style={{ fontSize:"14px", fontWeight:"700", color:c, fontFamily:"monospace" }}>{val}</span></div>;
          })}
          <Divider border={border} />
          <div style={{ overflowY:"auto", maxHeight:"192px", borderRadius:"10px", border:`1.8px solid ${border}` }}>
            <table style={{ width:"100%", borderCollapse:"collapse", fontSize:"12px", fontFamily:"monospace", tableLayout:"fixed" }}>
              <thead><tr style={{ background:dark?"#1e293b":"#f8fafc" }}>{["°","rad","sin","cos","tan"].map(h => <th key={h} style={{ padding:"5px 6px", textAlign:"center", color:textSec, fontWeight:"600", borderBottom:`1px solid ${border}`, fontSize:"11px", textTransform:"uppercase", fontFamily:"system-ui,sans-serif" }}>{h}</th>)}</tr></thead>
              <tbody>{SPECIAL.map((sp, i) => { const act = i === sel; const c = QC[sp.q]; return <tr key={i} onClick={() => setSel(i)} style={{ background:act?`${c}14`:"transparent", cursor:"pointer" }}><td style={{ padding:"4px 6px", textAlign:"center", color:act?c:textSec, fontWeight:act?"700":"400" }}>{sp.deg}</td><td style={{ padding:"4px 6px", textAlign:"center", color:act?c:textSec, fontWeight:act?"700":"400" }}>{sp.rad}</td><td style={{ padding:"4px 6px", textAlign:"center", color:act?c:textSec }}>{sp.sin}</td><td style={{ padding:"4px 6px", textAlign:"center", color:act?c:textSec }}>{sp.cos}</td><td style={{ padding:"4px 6px", textAlign:"center", color:act?c:textSec }}>{sp.tan}</td></tr>; })}</tbody>
            </table>
          </div>
        </>
      )}
      ...
      scene={(
        <div ref={svgRef} onPointerDown={onPD} style={{ cursor:"crosshair", touchAction:"none", userSelect:"none" }}>
          <svg width={SZ} height={SZ} viewBox={`0 0 ${SZ} ${SZ}`} style={{ display:"block", touchAction:"none", maxWidth:"100%", height:"auto" }}>
            ...
            {showArc && <path d={isNeg ? secPathCW(CX,CY,ARC,0,norm) : secPath(CX,CY,ARC,0,norm)} fill={isNeg?ABG:BBG} stroke="none" />}
            {showArc && <path d={isNeg ? arcPathCW(CX,CY,ARC,0,norm) : arcPath(CX,CY,ARC,0,norm)} fill="none" stroke={col} strokeWidth={2.5} strokeLinecap="round" />}
            {showArc && absAngle > 20 && absAngle < 355 && <text x={lp.x} y={lp.y} textAnchor="middle" dominantBaseline="central" fontSize={13} fontWeight="700" fill={col} style={FS}>{angle}°</text>}
            {showArc && <text x={polar(CX,CY,ARC-14,arcMid/2).x} y={polar(CX,CY,ARC-14,arcMid/2).y} textAnchor="middle" dominantBaseline="central" fontSize={10} fill={col} opacity={0.6} style={FS}>{isNeg?"CW":"CCW"}</text>}
            <line x1={CX} y1={unitPt.y} x2={unitPt.x} y2={unitPt.y} stroke={GREEN} strokeWidth={1.5} strokeDasharray="3 3" opacity={0.5} />
            <line x1={unitPt.x} y1={CY} x2={unitPt.x} y2={unitPt.y} stroke={RED} strokeWidth={1.5} strokeDasharray="3 3" opacity={0.5} />
            <line x1={CX} y1={CY} x2={CX+ARM} y2={CY} stroke={sub} strokeWidth={2} strokeLinecap="round" />
            <line x1={CX} y1={CY} x2={termEnd.x} y2={termEnd.y} stroke={col} strokeWidth={2.5} strokeLinecap="round" />
            <circle cx={unitPt.x} cy={unitPt.y} r={6} fill={col} opacity={0.85} stroke="white" strokeWidth={1.5} />
            <circle cx={CX} cy={CY} r={5.4} fill={col} />
            <g style={{ cursor:"grab" }}>
              <circle cx={termEnd.x} cy={termEnd.y} r={19} fill="transparent" />
              <circle cx={termEnd.x} cy={termEnd.y} r={10} fill={col} opacity={0.92} stroke="white" strokeWidth={2} />
            </g>
          </svg>
        </div>
      )}
      explanation={(
        <>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:"10px", flexWrap:"wrap" }}>
            ...
            <span style={{ display:"inline-block", padding:"4px 12px", borderRadius:"7px", fontSize:"13px", fontWeight:"700", color:col, background:`${col}15`, border:`1.8px solid ${col}33` }}>{isNeg?"CW ↻":"CCW ↺"}</span>
          </div>
          <div style={{ display:"flex", gap:"10px" }}>
            <div style={{ flex:1, minWidth:0, background:panelBg, borderRadius:"11px", border:`1.8px solid ${RED}22`, padding:"10px", textAlign:"center" }}><div style={{ fontSize:"11px", fontWeight:"600", color:RED, textTransform:"uppercase", marginBottom:"4px", fontFamily:"monospace" }}>sin({angle}°)</div><div style={{ fontSize:"22px", fontWeight:"900", color:RED, lineHeight:1, fontFamily:"monospace" }}>{sinV.toFixed(2)}</div></div>
            <div style={{ flex:1, minWidth:0, background:panelBg, borderRadius:"11px", border:`1.8px solid ${GREEN}22`, padding:"10px", textAlign:"center" }}><div style={{ fontSize:"11px", fontWeight:"600", color:GREEN, textTransform:"uppercase", marginBottom:"4px", fontFamily:"monospace" }}>cos({angle}°)</div><div style={{ fontSize:"22px", fontWeight:"900", color:GREEN, lineHeight:1, fontFamily:"monospace" }}>{cosV.toFixed(2)}</div></div>
          </div>
          <Divider border={border} />
          <div style={{ padding:"11px 14px", borderRadius:"11px", background:panelBg, border:`1.8px solid ${border}`, fontFamily:"monospace", fontSize:"13px", lineHeight:"1.8", color:textSec, overflowWrap:"break-word" }}>
            <div><span style={{ color:RED }}>sin({angle}°)</span> = <span style={{ color:RED, fontWeight:"700" }}>{sinV.toFixed(3)}</span></div>
            <div><span style={{ color:RED }}>sin({-angle}°)</span> = <span style={{ color:RED, fontWeight:"700" }}>{sinN.toFixed(3)}</span> ✓ negated</div>
            <div style={{ marginTop:"5px" }}><span style={{ color:GREEN }}>cos({angle}°)</span> = <span style={{ color:GREEN, fontWeight:"700" }}>{cosV.toFixed(3)}</span></div>
            <div><span style={{ color:GREEN }}>cos({-angle}°)</span> = <span style={{ color:GREEN, fontWeight:"700" }}>{cosN.toFixed(3)}</span> ✓ same</div>
          </div>
          <div style={{ padding:"11px 14px", borderRadius:"11px", background:panelBg, border:`1.8px solid ${border}`, fontFamily:"monospace", fontSize:"13px", color:textPri, lineHeight:"1.8" }}>
            <div>sin(−θ) = −sin(θ) <span style={{ color:textSec, fontSize:"12px" }}>odd</span></div>
            <div>cos(−θ) = cos(θ) <span style={{ color:textSec, fontSize:"12px" }}>even</span></div>
          </div>
        </>
      )}
    />
...
export default function TrigoAngleTypesExplorer({ defaultActive = "standard" } = {}) {
  ...
  const current = CONCEPTS.find((c) => c.id === active);
  const { surface, border, textPri, textSec, panelBg } = useTheme(dark);
  const narrow = hubWidth > 0 && hubWidth < 912;
  ...
  return (
    <div style={{
      ...
      <div ref={hubRef} style={{
        ...
        boxShadow: dark ? "0 5px 48px rgba(0,0,0,0.45)" : "0 5px 48px rgba(0,0,0,0.07)",
        overflow: "hidden",
        display: "flex",
        flexDirection: "column",
      }}>
        ...
        <div style={{
          ...
          alignItems: "center",
          padding: "14px 24px",
          borderBottom: `1.8px solid ${border}`,
          background: surface,
          flexShrink: 0,
          gap: "14px",
        }}>
        ...
        <div style={{
          ...
          {narrow ? (
            ...
            <div style={{
              ...
              flexShrink: 0,
              background: panelBg,
              borderRight: `1.8px solid ${border}`,
              padding: "14px 10px",
              display: "flex",
              flexDirection: "column",
              overflowY: "auto",
