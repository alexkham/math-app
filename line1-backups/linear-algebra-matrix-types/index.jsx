...
export async function getStaticProps() {
  ...
  const sectionsContent = {
    ...
    obj0: {
      title: `Key Terms`,
      content: `**Square matrix** — a matrix with as many rows as columns, $n \\times n$. Every type on this page is square; the main diagonal only makes sense when it is.
...
**Identity matrix** — $I_n$, ones on the diagonal and zeros elsewhere; the matrix that changes nothing under multiplication.
...
**Symmetric matrix** — equal to its transpose, $a_{i,j} = a_{j,i}$; a mirror across the diagonal.
...
• **Applying these types to non-square matrices** — the diagonal, and every rule built on it, needs a square matrix`,
    ...
    obj12: {
      ...
      content: `**Transpose** — the operation behind the symmetric and skew-symmetric definitions and the mirror between upper and lower triangular.

**Determinant** — the product of the diagonal for diagonal and triangular matrices; zero for the zero matrix and for odd skew-symmetric matrices.

**Eigenvalues and eigenvectors** — readable from the diagonal for diagonal and triangular matrices; real and orthogonal for symmetric ones.

**Diagonalisation** — the change of basis that turns a matrix into a diagonal one.

**Matrix inverse** — exists for diagonal and triangular matrices exactly when the diagonal has no zeros.

**Gaussian elimination and LU decomposition** — the routes to triangular form.

**Orthogonal matrices** — the other classic type, defined by $A^T A = I$, which the generator does not cover.

**Positive definite matrices** — symmetric matrices with positive eigenvalues, the type behind least squares and optimisation.`,
      before: ``,
      after: ``,
      link: '',
    },
...
export default function MatrixTypesPage({ seoData, sectionsContent, faqQuestions, schemas, matrixTypesExplanations }) {
  ...
  const genericSections = [
    ...
    plain('obj3', 'identity-and-zero'),
    plain('obj4', 'scalar-and-diagonal'),
    plain('obj5', 'triangular-matrices'),
    plain('obj6', 'symmetric-and-skew-symmetric'),
    plain('obj7', 'random-matrices'),
