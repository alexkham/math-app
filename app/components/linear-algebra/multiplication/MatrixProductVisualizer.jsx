...
export default function MatrixProductVisualizer({
  ...
  autoPlayMs = 1900,
  layout = 'stacked',
  /* Line 1: optional per-state sentences, keyed by the state keys below
     (idleBoth, idleSingle, buildColumns, buildRows, entry, genColumns,
     genRows, doneColumns, doneRows). Each normally ends in an on-page
     anchor. Additive — nothing passed, nothing appended, and every
  ...
  /* Line 1: which state the figure is in, as one key. Same switch the
     narration uses, lifted out so the explanations object can be keyed
     by it. */
  const stateKey = useMemo(() => {
    switch (phase.p) {
      case 'idle': return reading === 'both' ? 'idleBoth' : 'idleSingle';
      case 'build': return reading === 'columns' ? 'buildColumns' : 'buildRows';
      case 'entry': return 'entry';
      case 'gen': return reading === 'columns' ? 'genColumns' : 'genRows';
      default: return reading === 'columns' ? 'doneColumns' : 'doneRows';
    }
  }, [phase, reading]);
  ...
  }, [copy, phase, reading, say, explanations, stateKey]);
