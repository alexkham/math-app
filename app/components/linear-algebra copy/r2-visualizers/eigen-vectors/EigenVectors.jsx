...
// //  SECTION 2  ::  SVGRender
// // =====================================================================
// const DEFAULT_GEOM = { size: 480, scale: 50, gridR: 5 };
...
//   background:var(--bg);
//   background-image:
//     radial-gradient(circle at 10% 8%,rgba(147,197,253,.25) 0%,transparent 35%),
//     radial-gradient(circle at 90% 92%,rgba(191,219,254,.22) 0%,transparent 40%);
// }

// .ev-lede{display:flex;align-items:baseline;gap:12px;font-size:12.5px;color:var(--text-dim);line-height:1.4}
// .ev-crumb{font-family:var(--font-mono);font-size:10px;letter-spacing:.18em;text-transform:uppercase;color:var(--text-faint)}
...
// .ev-lede code{font-family:var(--font-mono);font-size:11.5px;background:var(--accent-subtle);color:var(--accent-hover);padding:1px 5px;border-radius:3px}
...
// }
// .ev-chip input{width:11px;height:11px;margin:0;accent-color:var(--accent);cursor:pointer}
// .ev-chip:hover,.ev-chip:has(input:checked){color:var(--text);background:var(--accent-subtle)}
// .ev-chip .ev-sw{display:inline-block;width:10px;height:3px;border-radius:1px}

// .ev-main{display:grid;grid-template-columns:minmax(0,1fr) 380px;gap:16px;min-height:0;align-items:stretch}

// .ev-canvas-col{display:flex;flex-direction:column;gap:8px;min-width:0;min-height:0;align-items:center}
// .ev-canvas-wrap{flex:1;display:flex;align-items:center;justify-content:center;min-height:0;width:100%}
// .ev-canvas{
//   height:100%;aspect-ratio:1/1;max-height:100%;max-width:100%;
//   background:linear-gradient(135deg,#fff 0%,#f0f7fc 100%);
//   border:1px solid var(--border-soft);border-radius:8px;box-shadow:var(--shadow-canvas);
//   display:block;touch-action:none;
// }
// .ev-canvas.dragging{cursor:grabbing}
// .ev-canvas.dragging .ev-tip-hit,.ev-canvas.dragging .ev-v-handle{cursor:grabbing}

// .ev-readouts{display:flex;gap:10px;width:100%}
// .ev-readout{
...
// }
// .ev-readout .ev-lab{font-family:var(--font-display);font-style:italic;font-size:17px;font-weight:500}
// .ev-readout .ev-eq{color:var(--text-faint);font-weight:400}
// .ev-readout .ev-val{font-variant-numeric:tabular-nums}
// .ev-readout.ev-v{color:var(--v);background:var(--vSoft);border-color:var(--vBorder)}
// .ev-readout.ev-av{color:var(--av);background:var(--avSoft);border-color:var(--avBorder)}

// .ev-info-col{display:flex;flex-direction:column;gap:10px;min-width:0;min-height:0;overflow-y:auto}
// .ev-info-col::-webkit-scrollbar{width:6px}
// .ev-info-col::-webkit-scrollbar-thumb{background:var(--border);border-radius:3px}

// .ev-card{
//   background:var(--surface);border:1px solid var(--border-soft);border-radius:8px;
//   box-shadow:var(--shadow-card);padding:12px 14px;min-height:0;
// }
// .ev-card h2{
...
// }
// .ev-badge{color:var(--accent);margin-right:6px}
// .ev-card h2 .ev-note{font-weight:500;letter-spacing:.05em;text-transform:none;font-size:10px;color:var(--text-dim)}

// /* Explanation */
// .ev-ex-card{flex:0 0 auto}
// .ev-ex-header{display:flex;align-items:baseline;justify-content:space-between;gap:10px;margin-bottom:8px;padding-bottom:8px;border-bottom:1px solid var(--border-soft)}
// .ev-ex-header h3{font-family:var(--font-display);font-weight:500;font-size:17px;color:var(--text);margin:0;letter-spacing:-.01em;line-height:1.15}
// .ev-ex-tag{font-family:var(--font-mono);font-size:9px;letter-spacing:.1em;text-transform:uppercase;color:var(--text-faint);white-space:nowrap}
// .ev-ex-body{color:var(--text-soft);font-size:12.5px;line-height:1.55;margin:0}
...
// .ev-ex-body code{font-family:var(--font-mono);font-size:11px;background:var(--accent-subtle);color:var(--accent-hover);padding:1px 4px;border-radius:3px}
...
// .ev-live-val{font-family:var(--font-mono);font-size:12px;color:var(--text);font-weight:500;text-align:right;font-variant-numeric:tabular-nums}
...
// .ev-snap-btn:disabled{background:var(--surface-2);color:var(--text-faint);cursor:not-allowed;border:1px solid var(--border-soft)}
// .ev-snap-btn svg{width:13px;height:13px;fill:currentColor}

// .ev-char-poly{
//   font-family:var(--font-mono);font-size:11px;color:var(--text-dim);
//   text-align:center;padding:6px 0;border-top:1px solid var(--border-soft);
//   margin-top:4px;letter-spacing:.02em;
// }
// .ev-char-poly code{color:var(--text);font-weight:500}

// /* Scenarios */
// .ev-scenarios-card{padding:12px 18px}
// .ev-scenarios-card h2{margin-bottom:8px;display:flex;font-size:10px}
// .ev-scen-sections{display:grid;grid-template-columns:repeat(4,1fr);gap:14px;align-items:start}
...
// .ev-scen-section-label .ev-tag.repeated{background:var(--accent-soft);color:var(--accent-hover);border-color:rgba(37,99,235,.35)}
...
// .ev-preset-grid{display:flex;flex-direction:column;gap:5px}
// .ev-preset-btn{
//   background:var(--surface-2);border:1px solid var(--border-soft);color:var(--text-soft);
//   padding:7px 11px;font-family:var(--font-body);font-size:12.5px;font-weight:500;
//   cursor:pointer;border-radius:5px;text-align:left;transition:all .12s;
//   display:flex;justify-content:space-between;align-items:baseline;gap:8px;
// }
// .ev-preset-btn:hover{background:var(--accent-subtle);border-color:var(--accent);color:var(--accent)}
// .ev-preset-btn.active{background:var(--accent-soft);border-color:var(--accent);color:var(--accent-hover);font-weight:600}
...
// .ev-preset-btn.distinct{border-left:3px solid var(--eigen)}
// .ev-preset-btn.distinct:hover{border-color:var(--eigen);color:var(--eigen);background:var(--eigenSoft)}
// .ev-preset-btn.distinct.active{background:var(--eigenSoft);border-color:var(--eigen);color:var(--eigen)}
// .ev-preset-btn.defective{border-left:3px solid var(--defective)}
// .ev-preset-btn.defective:hover{border-color:var(--defective);color:var(--defective);background:var(--defectiveSoft)}
// .ev-preset-btn.defective.active{background:var(--defectiveSoft);border-color:var(--defective);color:var(--defective)}
// .ev-preset-btn.complex{border-left:3px solid var(--complex)}
// .ev-preset-btn.complex:hover{border-color:var(--complex);color:var(--complex);background:var(--complexSoft)}
// .ev-preset-btn.complex.active{background:var(--complexSoft);border-color:var(--complex);color:var(--complex)}

// /* SVG styles */
// .ev-grid-line{stroke:var(--grid);stroke-width:1;fill:none}
// .ev-grid-axis{stroke:var(--grid-axis);stroke-width:1.4;fill:none}
...
//   .ev-app{padding:8px 18px;gap:7px}
//   .ev-card{padding:10px 12px}
//   .ev-ex-header h3{font-size:15px}
//   .ev-ex-body{font-size:12px}
// }
// @media (max-width:1200px){
//   .ev-main{grid-template-columns:1fr 340px}
//   .ev-scen-sections{grid-template-columns:repeat(2,1fr)}
// }
// `;
...
//   // Hit area for v handle (rendered as JSX so React handles event)
//   const tx = SVGRender._proj(geom);
...
//       <h2><span><span className="ev-badge">03</span>Scenarios</span></h2>
...
//   initialA, initialPreset, initialLayers, initialV, scenarios,
//   // slots
//   layerChips, canvas, explanation, liveCard, eigenStructure, scenariosPanel,
//   // atomization
...
//               {ledeBody && <span className="ev-body-text" dangerouslySetInnerHTML={{ __html: ledeBody }} />}
//             </div>
//           );
//         };
...
//   LiveCard, EigenStructureCard, ExplanationCard, ScenariosPanel,
//   // Hooks
//   useEigenState,
//   // Helpers
//   Math2D, SVGRender,
