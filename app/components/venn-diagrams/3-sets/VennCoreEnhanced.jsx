...
// Internal 2-set / generic implementation. All hooks live here so they're
// always called in the same order. The exported VennCoreEnhanced below is a
// thin dispatcher that picks between this and ThreeSetVenncore based on the
// shape of `circles`, with no hooks of its own.
const TwoSetVennCoreImpl = (props) => {
  const {
    circles = [],
    regions: regionsProp = null,
    highlight = null,
...
// Public dispatcher — no hooks, just picks the right implementation.
export const VennCoreEnhanced = (props) => {
  const { circles, regions: regionsProp } = props;
  if (
    !regionsProp &&
    circles && !Array.isArray(circles) &&
    circles.A && circles.B && circles.C
  ...
  return <TwoSetVennCoreImpl {...props} />;
};
