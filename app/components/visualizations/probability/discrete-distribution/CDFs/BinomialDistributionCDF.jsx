...
export default function BinomialDistributionCDF({ explanation }) {
  const [binomialN, setBinomialN] = useState(10);
  const [binomialP, setBinomialP] = useState(0.5);

  const defaultExplanation = 'The binomial CDF is $F(k) = P(X \\leq k) = \\sum_{i=0}^{k} \\binom{n}{i} p^i (1-p)^{n-i}$. This represents the probability of getting $k$ or fewer successes in $n$ trials. The CDF starts at $F(0) = (1-p)^n$ (the probability of zero successes) and increases with each additional possible outcome, reaching 1.0 at $k = n$. The shape of the CDF reflects the underlying binomial distribution: symmetric when $p = 0.5$, right-skewed when $p < 0.5$, and left-skewed when $p > 0.5$.';
  ...
  return (
    <div className="container">
      ...
      `}</style>
{/* 
      <h1>Binomial Distribution CDF</h1>
      <p className="subtitle">Interactive visualization of the cumulative distribution function</p> */}

      <div className="main-layout">
        <div className="content">
          <div className="distribution-header">
            <h2 className="distribution-title">Binomial Distribution CDF</h2>
            <p className="distribution-description">CDF for successes in n independent trials</p>
          </div>
