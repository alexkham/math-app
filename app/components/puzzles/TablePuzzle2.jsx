...
const TablePuzzle2 = ({ items, theme }) => {
  // Normalize props BEFORE any hooks so every hook sees a stable shape.
  // Early-return for empty/invalid `items` happens AFTER all hooks below.
  ...
  const [slots, setSlots] = useState(() => safeItems.map((_, i) => i));
  const [prevCorrect, setPrevCorrect] = useState(() => new Set(safeItems.map((_, i) => i)));
  ...
  const isSolved = useMemo(
    () => slots.length > 0 && slots.every((tileIdx, slotIdx) => tileIdx === slotIdx),
    [slots]
  );
  ...
  const renderPair = (tileIdx, slotIdx) => {
    const slotItem = safeItems[slotIdx];
    const tileItem = safeItems[tileIdx];
        ...
        <span style={S.puzzleProgress}>
          {isSolved ? (
            <strong style={{ color: C.primaryDark }}>
              &#10003; Solved! All {safeItems.length} matched.
            </strong>
          ) : (
            <>
              <strong style={{ color: C.primaryDark }}>{correctCount}</strong>
              {' '}/ {safeItems.length} in place
            </>
          )}
        </span>
      </div>
