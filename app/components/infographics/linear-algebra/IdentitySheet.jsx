...
//  * an entry called 'Does the set span $\\mathbb{R}^n$?' has to typeset rather
//  * than print its own source. */
// function nameNode(entry) {
//   if (!entry.name) return null;

//   const label = processContent(entry.name);
//   if (!entry.anchor) return label;
...
export default function IdentitySheet(props) {
  ...
  const masthead = (
    <div className="ids-mast">
      {data.kicker ? (
        <div className="ids-kicker">{processContent(data.kicker)}</div>
      ) : null}
      {data.title ? (
        <h3 className="ids-title">{processContent(data.title)}</h3>
      ) : null}
      {data.intro ? <p className="ids-intro">{processContent(data.intro)}</p> : null}
      <div className="ids-tally">
        <b>{total}</b>
