...
//  * say where it goes. A law with no anchor renders as plain text. */
// function nameNode(law) {
//   /* The name is prose like every other field, so it runs through the parser —
//    * a law called 'Rank of $A^{\\mathsf{T}}$' has to typeset rather than print
//    * its own source. */
//   const label = processContent(law.name);

//   if (!law.anchor) return label;
...
//                   >
//                     <span className="plc-item-i">{romanise(l.index + 1)}</span>
//                     <span className="plc-item-nm">{processContent(l.raw.name)}</span>
//                   </button>
//                 );
//               })}
//             </React.Fragment>
...
export default function PropertyLawCard(props) {
  ...
  const masthead = (
    <div className="plc-mast">
      {data.kicker ? (
        <div className="plc-kicker">{processContent(data.kicker)}</div>
      ) : null}
      {data.title ? (
        <h3 className="plc-title">{processContent(data.title)}</h3>
      ) : null}
      {data.intro ? <p className="plc-intro">{processContent(data.intro)}</p> : null}
      ...
      <div className="plc-mast-r">
        <div className={'plc-tally' + (missingAnchors > 0 ? ' is-gap' : '')}>
          <b>{padIndex(laws.length)}</b>
          <span>{processContent(data.tallyLabel || 'entries')}</span>
        </div>
    ...
    <div className="plc-view plc-view-detail">
      <div className="plc-list">
        {groups.map(function (g) {
          return (
            <React.Fragment key={g.key}>
              <div className="plc-list-grp">{processContent(g.heading)}</div>
              {g.items.map(function (l) {
                return (
                  <button
                    key={l.key}
