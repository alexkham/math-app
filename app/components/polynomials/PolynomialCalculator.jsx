...
import PolynomialInputDisplay from './PolynomialInputDisplay';
import ExplanationDetails from '../ExplanationDetails';
import './PolynomialCalculator.css';
import { processContent } from '@/app/utils/contentProcessor';

const PolynomialCalculator = ({instructions=[], operationExplanations={}}) => {
  const [operation, setOperation] = useState('addition');
  const [polynomial1Coefficients, setPolynomial1Coefficients] = useState([]);
  const [polynomial2Coefficients, setPolynomial2Coefficients] = useState([]);
  const [result, setResult] = useState([]);
  ...
  const performOperation = () => {
    ...
    const normalized1 = new Array(maxDegree + 1).fill('');
    const normalized2 = new Array(maxDegree + 1).fill('');
  ...
  return (
    <div className="main-container">
      ...
      {/* <div className="explanation-section">
        {result.length > 0 && operationExplanations[operation] && (
          <>
            <p>{operationExplanations[operation].text}</p>
            <BlockMath math={operationExplanations[operation].example} />
          </>
        )}
      </div> */}

{/* <div className="explanation-section">
  {result && operationExplanations[operation] && (
    <>
      <p>{operationExplanations[operation].text}</p>
      <BlockMath math={operationExplanations[operation].example} />
      <p className="note">{operationExplanations[operation].note}</p>
    </>
  )}
</div> */}
...
{/* <div className="explanation-section">
  {result && operationExplanations[operation] && (
    <>
      <p>{operationExplanations[operation].text}</p>
      {processContent(operationExplanations[operation].example)}
      <p>{processContent(operationExplanations[operation].note)}</p>
    </>
  )}
</div> */}
...
<div className="explanation-section">
  {result.length>0 && operationExplanations[operation] && (
    <div className="explanation-content">
      <p className="explanation-text">{operationExplanations[operation].text}</p>
      <div className="explanation-sections">
        {operationExplanations[operation].sections.map((section, index) => (
          <div key={index} className="explanation-section-item">
            <h4>{section.title}</h4>
            {processContent(section.content)}
          </div>
        ))}
      </div>
      <p className="explanation-note">{processContent(operationExplanations[operation].note)}</p>
    </div>
  )}
</div>
    </div>
