...
/**
 * FunctionTransformations — v4
 *
 * Changes from v2 (user's current working copy):
 *   1. Animation is now time-based (units per second) instead of
 *      step-per-frame. Frame rate variations no longer affect speed.
 *   2. Default speed reduced significantly: full range traversal takes
 *      ~6 seconds at 1× (was ~2 seconds at 60 fps).
 *   3. New speed preset control in the Auto controls row:
 *      0.5× / 1× / 2× / 4×. Defaults to 1×.
 *   4. Step buttons still use animStep (small discrete nudge), unaffected.
 *
 *   PARAM_DEFS now has both:
 *     - animStep  : the step-button nudge (kept from v2)
 *     - baseSpeed : units per second at 1× speed (new)
 */
...
const PARAM_DEFS = {
  a: { label: 'vertical scale a',   short: 'V. scale', min: -3, max: 3, step: 0.05, def: 1, animStep: 0.05, baseSpeed: 1.0 },
  k: { label: 'vertical shift k',   short: 'V. shift', min: -6, max: 6, step: 0.1,  def: 0, animStep: 0.1,  baseSpeed: 2.0 },
  b: { label: 'horizontal scale b', short: 'H. scale', min: -3, max: 3, step: 0.05, def: 1, animStep: 0.05, baseSpeed: 1.0 },
  h: { label: 'horizontal shift h', short: 'H. shift', min: -6, max: 6, step: 0.1,  def: 0, animStep: 0.1,  baseSpeed: 2.0 },
};
...
export default function FunctionTransformations({
  ...
  const startAnim = (key) => {
    ...
    const tick = (ts) => {
      ...
      const dt = (ts - lastTimeRef.current) / 1000;     // seconds since last frame
      lastTimeRef.current = ts;

      const cur = paramsRef.current[key];
      let v = cur + def.baseSpeed * speedRef.current * dt * animDirRef.current;
      if (v > def.max) { v = def.max; animDirRef.current = -1; }
      if (v < def.min) { v = def.min; animDirRef.current = 1; }
      setParams(p => {
        const next = { ...p, [key]: v };
  ...
  /* speed-preset chip group, shown only in Auto mode */
