...
// // // ============================================================
// // // DEFAULT STYLES (refined aesthetic pass)
// // // ============================================================
...
// //     shadedRegions.forEach(region => {
// //       const { type, functionIndex = 0, functionIndex1 = 0, functionIndex2 = 1, xStart = xMin, xEnd = xMax, y = 0,
...
// //         const pts = [];
// //         for (let i = 0; i <= sampleN; i++) { const x = x1 + i * step; try { const yv = fn1(x); if (typeof yv === 'number' && isFinite(yv)) pts.push({ x, y: clamp(yv) }); } catch {} }
...
// //         const pts1 = [], pts2 = [];
// //         for (let i = 0; i <= sampleN; i++) { const x = x1 + i * step; try { const a = fn1(x), b = fn2(x); if (isFinite(a)) pts1.push({ x, y: clamp(a) }); if (isFinite(b)) pts2.push({ x, y: clamp(b) }); } catch {} }
...
// //   }), [width, height, padding, coordSys.viewport, coordSys.gridStep, normFns, styles, showGrid, showMinorGrid, showAxes, showAxisLabels, shadedRegions, asymptotes, tangentLine, secantLine, specialPoints, labelMode, legendPosition, samples, effectiveDefaultStroke]);
...
// //   }, [coordSys, interaction, normFns, specialPoints, asymptotes, tangentLine, secantLine, shadedRegions, showGrid, showMinorGrid, showAxes, showAxisLabels, showCrosshair, showCrosshairCoordinates, showCurveTooltip, labelMode, legendPosition, specialPointLabelMode, width, height, padding, styles, samples, effectiveDefaultStroke]);
...
// //   return (
// //     <div style={{ position: 'fixed', inset: 0, background: 'rgba(15, 23, 42, 0.55)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 10000 }} onClick={onClose}>
// //       <div style={{ background: 'white', borderRadius: 12, padding: 24, maxWidth: 500, width: '90%', maxHeight: '80vh', overflow: 'auto', boxShadow: '0 20px 60px rgba(15, 23, 42, 0.25)' }} onClick={e => e.stopPropagation()}>
...
// // // FIX: maximize layout — toolbar now anchored to top, canvas height accounts for chrome
// // // ============================================================
...
// //   // FIX: subtract realistic chrome (toolbar ~48 + label ~24 + padding 40 + breathing room ~8 = 120)
// //   const cur = isMax ? { width: winSize.width - 40, height: winSize.height - 120 } : size;
...
// //   return (
// //     <div style={isMax
// //       ? { position: 'fixed', inset: 0, background: '#0f172a', zIndex: 9999, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'flex-start', padding: 20, overflow: 'auto' }
...
// //       <div style={{ marginTop: 8, fontSize: 11, color: '#94a3b8', fontVariantNumeric: 'tabular-nums', flexShrink: 0 }}>{cur.width} × {cur.height}</div>
...
// // // ============================================================
// // // DEMO
// // // New: foldable sidebar + hidden scrollbar (still scrolls)
// // // ============================================================
...
// //   const [curveStroke, setCurveStroke] = useState(1.75);
...
// //   const functions = [
// //     { fn: x => 0.1 * x * x - 2, color: '#3b82f6', label: 'f', formula: 'f(x) = 0.1x² − 2', visible: true },
// //     { fn: x => Math.sin(x) * 2, color: '#10b981', label: 'g', formula: 'g(x) = 2 sin(x)',  visible: true }
// //   ];

// //   const shadedRegions = showUnderCurve
// //     ? [{ type: 'underCurve', functionIndex: 0, xStart: underCurveStart, xEnd: underCurveEnd, color: 'rgba(59, 130, 246, 0.18)' }]
// //     : [];
...
// //   // Collapse/expand button — floats at top-right of expanded sidebar, or fills the rail when collapsed
// //   const collapseBtn = (
// //     <button
...
// //       {/* Hidden-scrollbar CSS — applied to .vc-no-scrollbar */}
// //       <style>{`
// //         .vc-no-scrollbar { scrollbar-width: none; -ms-overflow-style: none; }
// //         .vc-no-scrollbar::-webkit-scrollbar { width: 0; height: 0; display: none; }
// //       `}</style>

// //       {/* Sidebar — animated width, foldable, hidden scrollbar */}
// //       <div
// //         style={{
// //           width: sidebarCollapsed ? 40 : 280,
// //           transition: 'width 220ms cubic-bezier(0.4, 0, 0.2, 1)',
// //           background: 'white',
// //           padding: sidebarCollapsed ? 6 : 18,
// //           borderRadius: 10,
...
// //         }}
// //         className="vc-no-scrollbar"
// //       >
// //         {sidebarCollapsed ? (
// //           // ---- Collapsed rail: just the chevron ----
...
// //                 <h3 style={{ margin: '0 0 4px', color: '#0f172a', fontSize: 16, letterSpacing: '-0.01em' }}>VisualizerCore</h3>
// //                 <p style={{ fontSize: 11, color: '#94a3b8', marginBottom: 16 }}>v2 &mdash; foldable sidebar &amp; layout fixes</p>
// //               </div>
// //               {collapseBtn}
// //             </div>

// //             <div style={sec}>
// //               <h4 style={{ margin: '0 0 8px', fontSize: 10.5, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.05em', fontWeight: 600 }}>Display</h4>
// //               <label style={chk}><input type="checkbox" checked={showGrid} onChange={e => setShowGrid(e.target.checked)} /> Major grid</label>
// //               <label style={chk}><input type="checkbox" checked={showMinorGrid} onChange={e => setShowMinorGrid(e.target.checked)} /> Minor grid</label>
...
// //             <div style={sec}>
// //               <h4 style={{ margin: '0 0 8px', fontSize: 10.5, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.05em', fontWeight: 600 }}>Curve stroke</h4>
// //               <label style={{ display: 'block', fontSize: 12, color: '#334155', fontVariantNumeric: 'tabular-nums' }}>
// //                 Width: {curveStroke.toFixed(2)}
// //                 <input type="range" min="0.5" max="4" step="0.25" value={curveStroke} onChange={e => setCurveStroke(+e.target.value)} style={sld} />
// //               </label>
// //             </div>

// //             <div style={sec}>
// //               <h4 style={{ margin: '0 0 8px', fontSize: 10.5, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.05em', fontWeight: 600 }}>Special Points</h4>
...
// //             <div style={sec}>
// //               <h4 style={{ margin: '0 0 8px', fontSize: 10.5, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.05em', fontWeight: 600 }}>Shaded Region</h4>
// //               <label style={chk}><input type="checkbox" checked={showUnderCurve} onChange={e => setShowUnderCurve(e.target.checked)} /> Under f(x)</label>
// //               {showUnderCurve && <>
// //                 <label style={{ display: 'block', fontSize: 12, marginTop: 8, color: '#334155', fontVariantNumeric: 'tabular-nums' }}>
// //                   x start: {underCurveStart}
// //                   <input type="range" min="-10" max="10" step="0.5" value={underCurveStart} onChange={e => setUnderCurveStart(+e.target.value)} style={sld} />
// //                 </label>
// //                 <label style={{ display: 'block', fontSize: 12, color: '#334155', fontVariantNumeric: 'tabular-nums' }}>
// //                   x end: {underCurveEnd}
// //                   <input type="range" min="-10" max="10" step="0.5" value={underCurveEnd} onChange={e => setUnderCurveEnd(+e.target.value)} style={sld} />
// //                 </label>
// //               </>}
// //             </div>

// //             <div style={{ padding: 12, background: '#eff6ff', borderRadius: 6, fontSize: 12, marginBottom: 12, color: '#1e3a8a', border: '1px solid #dbeafe' }}>
// //               <strong>Note</strong><br />
// //               Click the chevron to fold this panel. Scroll works without a visible scrollbar.
// //             </div>

// //             <div style={{ padding: 10, background: '#f8fafc', borderRadius: 6, fontSize: 11, color: '#475569', border: '1px solid #f1f5f9', fontVariantNumeric: 'tabular-nums' }}>
// //               <strong style={{ color: '#0f172a' }}>Viewport</strong><br />
...
// const ChevronLeftIcon = () => <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M15 18l-6-6 6-6" /></svg>;
// const ChevronRightIcon = () => <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M9 18l6-6-6-6" /></svg>;
// // NEW: larger chevron for the folded sidebar CTA
...
// // Backdrop click closes (was already wired; kept and verified).
// // Modal panel now has < 100% opacity via rgba background + backdrop blur.
// // ============================================================
// function ExportModal({ isOpen, onClose, pngDataUrl, svgContent }) {
//   const [copied, setCopied] = useState(null);
//   if (!isOpen) return null;
...
// // ============================================================
// // WRAPPER WITH CONTROLS  --- ISSUE #2 ---
// // Maximize overlay now uses padding '80px 20px 20px' so the toolbar
// // (with the red Exit button) lands well below any host-page navbar.
// // Canvas height in maximize subtracts 180 instead of 120.
// // ============================================================
// function VisualizerWithControls(props) {
//   const { defaultWidth = 700, defaultHeight = 550, minWidth = 400, minHeight = 300, ...rest } = props;
//   const visualizerRef = useRef(null);
...
//   // Maximize: top padding 80, plus toolbar + label + breathing ≈ 100 → subtract 180.
...
//   return (
//     <div style={isMax
//       ? { position: 'fixed', inset: 0, background: '#0f172a', zIndex: 9999, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'flex-start', padding: '80px 20px 20px', overflow: 'auto' }
//       : { background: 'white', borderRadius: 10, boxShadow: '0 1px 3px rgba(15,23,42,0.06), 0 8px 24px rgba(15,23,42,0.06)', padding: 16, display: 'flex', flexDirection: 'column', alignItems: 'center', border: '1px solid #f1f5f9' }}>
//       <div style={{ display: 'flex', gap: 8, marginBottom: 12, alignItems: 'center', flexWrap: 'wrap', justifyContent: 'center', flexShrink: 0 }}>
//         {!isMax && <div style={{ display: 'flex', gap: 4, marginRight: 8 }}>{presets.map(p => <button key={p.l} onClick={() => setSize({ width: p.w, height: p.h })} style={size.width === p.w && size.height === p.h ? actStyle : btnStyle}>{p.l}</button>)}</div>}
//         {!isMax && <div style={{ display: 'flex', gap: 4, alignItems: 'center', fontSize: 12, color: '#64748b' }}>
...
// // ============================================================
// // DEMO  --- ISSUE #1 ---
// // Sidebar defaults to FOLDED. Folded bar is wider (56px), shows a
// // large chevron in a soft badge, has vertical "Show Controls" CTA,
// // and the entire bar is one big click-to-expand button with a
...
//   // Collapse button used in the EXPANDED panel header (folds the panel).
//   const collapseBtn = (
//     <button
...
//         .vc-no-scrollbar { scrollbar-width: none; -ms-overflow-style: none; }
//         .vc-no-scrollbar::-webkit-scrollbar { width: 0; height: 0; display: none; }
//         .vc-folded-cta:hover .vc-folded-badge { background: #dbeafe; border-color: #93c5fd; }
//         .vc-folded-cta:hover .vc-folded-text { color: #1e293b; }
//       `}</style>
...
//       >
//         {sidebarCollapsed ? (
//           // ---- Collapsed: full-bar click-to-expand button with CTA ----
//           <button
//             type="button"
//             className="vc-folded-cta"
//             onClick={() => setSidebarCollapsed(false)}
//             title="Click to expand the controls panel"
...
//               <div style={{ minWidth: 0 }}>
//                 <h3 style={{ margin: '0 0 4px', color: '#0f172a', fontSize: 16, letterSpacing: '-0.01em' }}>VisualizerCore</h3>
//                 <p style={{ fontSize: 11, color: '#94a3b8', marginBottom: 16 }}>v3 &mdash; folded by default, max spacing, modal opacity</p>
//               </div>
//               {collapseBtn}
//             </div>
...
// ============================================================
// WRAPPER WITH CONTROLS
// FIX: Maximize overlay now closes when clicking the dark area.
// - Outer dark wrapper has onClick={() => setIsMax(false)}
// - Inner content (toolbar + canvas + label) wrapped in a div
...
function VisualizerWithControls(props) {
  ...
  // Inner content (everything inside the dark area when maximized).
  const innerContent = (
    <div
        ...
        {sidebarCollapsed ? (
          ...
          <>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 4, gap: 8 }}>
              <div style={{ minWidth: 0 }}>
                <h3 style={{ margin: '0 0 4px', color: '#0f172a', fontSize: 16, letterSpacing: '-0.01em' }}>VisualizerCore</h3>
                <p style={{ fontSize: 11, color: '#94a3b8', marginBottom: 16 }}>v4 &mdash; maximize click-to-exit</p>
              </div>
              {collapseBtn}
            </div>
            ...
            <div style={{ padding: 12, background: '#eff6ff', borderRadius: 6, fontSize: 12, marginBottom: 12, color: '#1e3a8a', border: '1px solid #dbeafe' }}>
              <strong>Note</strong><br />
              When maximized, click outside the canvas to exit fullscreen.
            </div>
