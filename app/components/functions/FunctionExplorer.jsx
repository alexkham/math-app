...
// Main component, math pipeline, state, design tokens.
// Imports: ./VisualizerCore (engine), ./ExplorerAtoms, ./ExplorerPanels
// ============================================================
...
// ============================================================
// DESIGN TOKENS  (blue-dominant, exported for atoms + panels)
// ============================================================
export const T = {
  bg: {
    app:       '#f4f5f7',
...
// Passes blue + slate theme to VisualizerCore; engine deep-merges.
// ============================================================
...
// ============================================================
// MATH PIPELINE
// Re-implemented numeric solvers (the engine has its own but
// they aren&apos;t exported). Used for filling accordion text content.
// ============================================================
function safe(f, x) {
  try { const y = f(x); return (typeof y === 'number' && isFinite(y)) ? y : null; }
  catch { return null; }
}
...
// ============================================================
// FAMILY DETECTION
// Quick heuristic from the AST shape; used for the display label only.
// ============================================================
function detectFamily(node) {
  ...
  if (has(/\//) && has(/x/)) {
    // crude rational detection: contains a division and x on both sides
    return 'Rational';
  }
  if (has(/\babs\b|\|/)) return 'Absolute value';
  // Polynomial degree by examining the AST
...
function polynomialDegree(node) {
  // Walk the AST. For OperatorNode '^' with base = SymbolNode 'x' and integer exponent, record it.
  let max = 0;
  const walk = (n) => {
    if (!n) return;
    if (n.type === 'OperatorNode' && n.op === '^') {
      ...
      if (base.type === 'SymbolNode' && base.name === 'x' &&
          exp.type === 'ConstantNode' && Number.isInteger(exp.value)) {
        if (exp.value > max) max = exp.value;
      }
    }
    // a stand-alone x is degree 1
...
// ============================================================
// PARITY DETECTION
// Sample-based: even if f(-x) ≈ f(x); odd if f(-x) ≈ -f(x); neither otherwise.
// ============================================================
...
// ============================================================
// MAIN PIPELINE BUILD
// Given an expression string, return everything the panels need.
// ============================================================
export function buildPipeline(expression, opts = {}) {
  ...
  const asymptotes = findVerticalAsymptotes(fn, xMin, xMax);
  ...
  // End behavior
  const farRight = safe(fn, xMax * 10);
  const farLeft  = safe(fn, xMin * 10);

  // Continuity: if no asymptotes and missing samples are empty, declare continuous on viewport
  const continuous = (asymptotes.length === 0) && (missingAt.length === 0);

  // Concavity intervals (using inflections to split)
  ...
  const bounds = { lo, hi, bounded };

  // Injectivity: rough check — if any horizontal line hits the curve twice, not injective.
  // Approximate by: not strictly monotonic across the domain.
  ...
  return {
    ...
    domain: missingAt.length === 0 ? '\u211D' : `\u211D \\ {${missingAt.slice(0, 3).map(x => x.toFixed(2)).join(', ')}...}`,
    range: bounded ? `[${lo.toFixed(3)}, ${hi.toFixed(3)}]` : 'unbounded',
    codomain: '\u211D',
    parity,
    roots, extrema, inflections, asymptotes,
    yIntercept: yInt,
    sign, mono, concavity,
    bounded, bounds,
    continuous,
...
// ============================================================
// STATE HOOK
// Centralized: tabs, density, rail toggles, cursor, view mode,
// pinned insights.
// ============================================================
function useExplorerState(initial = {}) {
  ...
  const [overlays, setOverlays] = useState({
    fp: true, fpp: false, anti: false, inv: false,
  });
  const [annotations, setAnnotations] = useState({
    roots: true, extrema: true, inflect: false, asymp: false, tangent: true, area: true,
  });

  const [cursor, setCursor] = useState({ x: 2.34, y: -1.45 });
  const [viewport, setViewport] = useState({ xMin: -10, xMax: 10, yMin: -10, yMax: 10 });

  const [pinned, setPinned] = useState([
    { id: 'fp-at-cursor', label: "f'(2.34)", value: '0.468', sub: 'slope at cursor' },
  ]);
...
export function FunctionExplorer(props) {
  ...
  // Build the pipeline. Memoize so we don&apos;t recompute on every render.
  const data = useMemo(
  ...
  // Build the engine&apos;s functions array: f + active overlay curves.
  const engineFunctions = useMemo(() => {
    ...
    if (state.overlays.anti && data.fn) {
      // Numeric cumulative integral over the visible window
    ...
    if (state.overlays.inv && data.fn) {
      // Inverse via reflection across y=x; this works graphically by swapping coordinates.
      // For a non-injective f, this draws a relation rather than a function. Engine still renders.
      // Provide as a special-cased function only if we can express it; here we skip the math
      // and let the panels mention the restriction. The graph stays unchanged for v1.
    }
    return list;
  }, [data, state.overlays, state.viewport.xMin, state.viewport.xMax]);

  // Map rail annotation toggles to engine props.
  const engineAnnotations = useMemo(() => ({
    ...
    shadedRegions:   state.annotations.area
      ? [{ type: 'underCurve', functionIndex: 0, xStart: -3, xEnd: 3 }]
      : [],
  }), [state.annotations, state.cursor.x]);
      ...
      >
        ...
        <div style={{ position: 'relative', background: T.bg.graph, overflow: 'hidden' }}>
          ...
          {state.view === 'table' && !data.error && (
            <NumericTable fn={data.fn} xMin={-4} xMax={4} step={0.5} />
          )}

          {state.view === 'map' && !data.error && (
            <MappingDiagram fn={data.fn} domain={[-4, -3, -2, -1, 0, 1, 2, 3, 4]} />
          )}
...
function defaultInsightsFor(data) {
  ...
  cards.push({ id: 'parity', label: 'Symmetry', value: data.parity, sub: '' });
  cards.push({ id: 'continuity', label: 'Continuity', value: data.continuous ? 'continuous' : 'has gaps', sub: '' });
  return cards;
}

// Numerical cumulative integral (trapezoidal) over the visible window.
