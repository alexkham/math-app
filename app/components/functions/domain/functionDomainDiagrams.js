// Static SVG diagrams for the function-domain page sections.
// Each diagram freezes the FunctionDomain visualizer in one family at default
// parameters, replicating the component's rendering: curve #3b82f6, x-axis
// domain highlight - green #10b981 band where inputs are accepted, red
// #ef4444 band where they are not, with open/closed endpoint markers.
...
// Base functions at default parameters, copied from FAMILIES in FunctionDomain.jsx.
// domain: list of [from, to, inDomain]; endpoints: [x, 'open'|'closed']
const FNS = {
  identity:    { f: (x) => x,               eq: 'f(x) = x',    bands: [[-10, 10, true]], pts: [] },
  linearScale: { f: (x) => 2 * x,           eq: 'f(x) = 2x',   bands: [[-10, 10, true]], pts: [] },
  quadratic:   { f: (x) => x * x,           eq: 'f(x) = x²', bands: [[-10, 10, true]], pts: [] },
  cubic:       { f: (x) => x * x * x,       eq: 'f(x) = x³', bands: [[-10, 10, true]], pts: [] },
  exponential: { f: (x) => Math.exp(x),     eq: 'f(x) = eˣ', bands: [[-10, 10, true]], pts: [] },
  sine:        { f: (x) => Math.sin(x),     eq: 'f(x) = sin(x)', bands: [[-10, 10, true]], pts: [] },
  cosine:      { f: (x) => Math.cos(x),     eq: 'f(x) = cos(x)', bands: [[-10, 10, true]], pts: [] },
  absolute:    { f: (x) => Math.abs(x),     eq: 'f(x) = |x|',  bands: [[-10, 10, true]], pts: [] },
  logarithmic: { f: (x) => (x > 0 ? Math.log(x) : NaN), eq: 'f(x) = ln(x)',
                 bands: [[-10, 0, false], [0, 10, true]], pts: [[0, 'open']] },
  sqrt:        { f: (x) => (x >= 0 ? Math.sqrt(x) : NaN), eq: 'f(x) = √x',
                 bands: [[-10, 0, false], [0, 10, true]], pts: [[0, 'closed']] },
  reciprocal:  { f: (x) => (x === 0 ? NaN : 1 / x), eq: 'f(x) = 1/x',
                 bands: [[-10, 10, true]], pts: [[0, 'open']] },
};
...
function diagram(key) {
  ...
  for (const [from, to, ok] of spec.bands) {
    out += `<rect x="${sx(from)}" y="${axisY - 5}" width="${(sx(to) - sx(from)).toFixed(2)}" height="10" rx="3" ` +
           `fill="${ok ? '#10b981' : '#ef4444'}" opacity="${ok ? 0.4 : 0.28}"/>`;
  }
  out += `<path d="${curvePath(spec.f)}" fill="none" stroke="#3b82f6" stroke-width="1.75"/>`;
  for (const [x, kind] of spec.pts) {
    out += `<circle cx="${sx(x)}" cy="${axisY}" r="5" fill="${kind === 'closed' ? '#10b981' : '#fff'}" ` +
...
const functionDomainDiagrams = {};
for (const key of Object.keys(FNS)) functionDomainDiagrams[key] = diagram(key);

export default functionDomainDiagrams;
