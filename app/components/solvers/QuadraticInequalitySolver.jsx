...
/* =====================================================
   ...
   Two exports:
   - QuadraticInequalitySolverEngine: Standalone solver component
   - QuadraticInequalitySolver: Full educational wrapper (default)
   ...
   - Sign chart / parabola method
   - Compound intervals, union notation
   - All reals / no solution cases
   ===================================================== */
...
function tokenize(input) {
  ...
  while (i < input.length) {
    ...
    if (char === '\u00B2') { tokens.push({ type: TokenType.POWER }); tokens.push({ type: TokenType.NUMBER, value: 2 }); i++; continue; }
    if (char === '\u00B3') { tokens.push({ type: TokenType.POWER }); tokens.push({ type: TokenType.NUMBER, value: 3 }); i++; continue; }
...
function parse(tokens) {
  ...
  const parseInequality = () => {
    ...
    if (t?.type === TokenType.EQUALS) { consume(); return { type: 'EQUATION', left, right: parseExpression() }; }
    return left;
  };
...
function collectQuadraticTerms(node, varName) {
  ...
  switch (node.type) {
    case 'NUMBER': return { a: 0, b: 0, c: node.value };
    case 'VARIABLE': return node.name === varName ? { a: 0, b: 1, c: 0 } : { a: 0, b: 0, c: 0 };
    case 'NEGATE': { const inner = collectQuadraticTerms(node.operand, varName); if (!inner) return null; return { a: -inner.a, b: -inner.b, c: -inner.c }; }
    case 'ADD': { const l = collectQuadraticTerms(node.left, varName); const r = collectQuadraticTerms(node.right, varName); if (!l || !r) return null; return { a: l.a + r.a, b: l.b + r.b, c: l.c + r.c }; }
    case 'SUBTRACT': { const l = collectQuadraticTerms(node.left, varName); const r = collectQuadraticTerms(node.right, varName); if (!l || !r) return null; return { a: l.a - r.a, b: l.b - r.b, c: l.c - r.c }; }
    case 'POWER': {
    ...
    case 'MULTIPLY': {
      ...
      if (lConst !== null && containsVariable(node.right)) { const r = collectQuadraticTerms(node.right, varName); if (!r) return null; return { a: lConst * r.a, b: lConst * r.b, c: lConst * r.c }; }
      if (rConst !== null && containsVariable(node.left)) { const l = collectQuadraticTerms(node.left, varName); if (!l) return null; return { a: rConst * l.a, b: rConst * l.b, c: rConst * l.c }; }
      if (lConst !== null && rConst !== null) return { a: 0, b: 0, c: lConst * rConst };
      if (node.left?.type === 'VARIABLE' && node.left.name === varName && node.right?.type === 'VARIABLE' && node.right.name === varName) return { a: 1, b: 0, c: 0 };
      return null;
    }
    case 'DIVIDE': {
      const divisor = evaluateConstant(node.right);
      if (divisor !== null && divisor !== 0) { const l = collectQuadraticTerms(node.left, varName); if (l) return { a: l.a / divisor, b: l.b / divisor, c: l.c / divisor }; }
      return null;
    }
    default: return null;
  }
...
function solveQuadraticInequality(ast) {
  const steps = [];
  ...
  if (!leftTerms || !rightTerms) {
    throw new Error('Could not parse as a quadratic inequality. Ensure the highest power is 2.');
  }

  let a = leftTerms.a - rightTerms.a;
  let b = leftTerms.b - rightTerms.b;
  let c = leftTerms.c - rightTerms.c;
  let currentOp = operator;

  if (Math.abs(a) < 1e-10) {
    throw new Error('This is not a quadratic inequality (the coefficient of ' + varName + '\u00B2 is 0). Try the linear inequality solver.');
  }

  // If a < 0, multiply by -1 and flip — simplifies analysis
  if (a < 0) {
    a = -a; b = -b; c = -c;
    currentOp = flipOperator(currentOp);
    steps.push({
      rule: 'Multiply by \u22121',
      description: `Multiply both sides by \u22121 and flip the inequality sign so the leading coefficient is positive.`,
      before: ast,
      after: {
        type: 'INEQUALITY',
        operator: currentOp,
        left: buildQuadAST(a, b, c, varName),
    ...
    steps.push({
      rule: 'Standard Form',
      description: `Rewrite with all terms on the left: ${formatNumber(a)}${varName}\u00B2 ${b >= 0 ? '+ ' + formatNumber(b) : '\u2212 ' + formatNumber(-b)}${varName} ${c >= 0 ? '+ ' + formatNumber(c) : '\u2212 ' + formatNumber(-c)} ${currentOp} 0.`,
      before: ast,
      after: {
        type: 'INEQUALITY',
        operator: currentOp,
  ...
  steps.push({
    ...
    description: `\u0394 = b\u00B2 \u2212 4ac = (${formatNumber(b)})\u00B2 \u2212 4(${formatNumber(a)})(${formatNumber(c)}) = ${formatNumber(disc)}.`,
    before: null,
    after: null
  });

  // Case 1: Discriminant < 0 — no real roots, parabola doesn't cross x-axis
  // Since a > 0 (we ensured), the parabola opens up → always positive
  if (disc < 0) {
    // a > 0, no roots: quadratic is always positive
    const alwaysPositive = true;
    const wantLess = isLessThan(currentOp);

    if (wantLess) {
      // Want < 0 or ≤ 0 but it's always > 0 → no solution
      steps.push({
        rule: 'No Real Roots',
        description: `Since \u0394 < 0 and a > 0, the parabola opens upward and never touches the x-axis. The expression is always positive, so the inequality has no solution.`,
        before: null,
        after: null
      });
      return {
        ...
        graphData: { type: 'quadratic', a, b, c, variable: varName, roots: [], vertex: { x: -b / (2 * a), y: c - b * b / (4 * a) }, inequality: { operator: currentOp } }
      };
    } else {
      // Want > 0 or ≥ 0 and it's always positive → all reals
      steps.push({
        rule: 'No Real Roots',
        description: `Since \u0394 < 0 and a > 0, the parabola opens upward and never touches the x-axis. The expression is always positive, so the inequality is true for all real numbers.`,
        before: null,
        after: null
      });
      return {
  ...
  if (Math.abs(disc) < 1e-10) {
    ...
    steps.push({
      rule: 'Repeated Root',
      description: `\u0394 = 0, so there is one repeated root: ${varName} = ${formatNumber(root)}. The expression touches zero only at this point.`,
      before: null,
      after: null
    });
    ...
    if (wantLess) {
      if (strict) {
        // < 0: expression is always ≥ 0 with equality only at root → no solution
        steps.push({
          rule: 'Analyze Sign',
          description: `The expression equals zero only at ${varName} = ${formatNumber(root)} and is positive everywhere else. Since we need strictly less than zero, there is no solution.`,
          before: null, after: null
        });
        return { steps, solution: { type: 'none', variable: varName }, graphData: { type: 'quadratic', a, b, c, variable: varName, roots: [root], vertex: { x: root, y: 0 }, inequality: { operator: currentOp } } };
      } else {
        // ≤ 0: only at the root itself
        steps.push({
          rule: 'Analyze Sign',
          description: `The expression equals zero only at ${varName} = ${formatNumber(root)} and is positive everywhere else. The inequality holds only at that single point.`,
          before: null, after: null
        });
        return { steps, solution: { type: 'point', variable: varName, value: root, exact: Number.isInteger(root) }, graphData: { type: 'quadratic', a, b, c, variable: varName, roots: [root], vertex: { x: root, y: 0 }, inequality: { operator: currentOp } } };
      }
    } else {
      if (strict) {
        // > 0: all reals except the root
        steps.push({
          rule: 'Analyze Sign',
          description: `The expression is positive everywhere except at ${varName} = ${formatNumber(root)} where it equals zero. The solution is all reals except ${varName} = ${formatNumber(root)}.`,
          before: null, after: null
        });
        return { steps, solution: { type: 'all_except', variable: varName, value: root, exact: Number.isInteger(root) }, graphData: { type: 'quadratic', a, b, c, variable: varName, roots: [root], vertex: { x: root, y: 0 }, inequality: { operator: currentOp } } };
      } else {
        // ≥ 0: always true
        steps.push({
          rule: 'Analyze Sign',
          description: `The expression is always \u2265 0 (equals zero at ${varName} = ${formatNumber(root)}, positive elsewhere). The inequality is true for all real numbers.`,
          before: null, after: null
        });
        return { steps, solution: { type: 'all', variable: varName }, graphData: { type: 'quadratic', a, b, c, variable: varName, roots: [root], vertex: { x: root, y: 0 }, inequality: { operator: currentOp } } };
      }
    }
  }
  ...
  const isExact = isPerfectSquare(disc) && Number.isInteger(a) && Number.isInteger(b) && Number.isInteger(c);

  steps.push({
    rule: 'Find Roots',
    description: `The roots are ${varName}\u2081 = ${formatNumber(root1)} and ${varName}\u2082 = ${formatNumber(root2)}.`,
    before: null,
    after: null
  });

  // Sign chart: a > 0 parabola opens up
  // Positive outside roots, negative between roots
  steps.push({
    rule: 'Sign Chart',
    description: `Since a > 0, the parabola opens upward. The expression is negative between the roots (${formatNumber(root1)} < ${varName} < ${formatNumber(root2)}) and positive outside (${varName} < ${formatNumber(root1)} or ${varName} > ${formatNumber(root2)}).`,
    before: null,
    after: null
  });
  ...
  if (wantLess) {
    // Want negative region: between the roots
    const intervalStr = `${openL}${formatNumber(root1)}, ${formatNumber(root2)}${closeR}`;
    steps.push({
      rule: 'Solution Interval',
      description: `The expression is ${strict ? 'negative' : 'non-positive'} between the roots: ${intervalStr}.`,
      before: null,
      after: null
    });
    return {
      steps,
      solution: { type: 'compound', variable: varName, root1, root2, operator: currentOp, exact: isExact, region: 'between' },
      graphData: { type: 'quadratic', a, b, c, variable: varName, roots: [root1, root2], vertex: { x: -b / (2 * a), y: c - b * b / (4 * a) }, inequality: { operator: currentOp } }
    };
  } else {
    // Want positive region: outside the roots
    const r1Str = formatNumber(root1);
    const r2Str = formatNumber(root2);
    const intervalStr = `(\u2212\u221E, ${r1Str}${closeR} \u222A ${openL}${r2Str}, \u221E)`;
    steps.push({
      rule: 'Solution Interval',
      description: `The expression is ${strict ? 'positive' : 'non-negative'} outside the roots: ${intervalStr}.`,
      before: null,
      after: null
    });
    return {
      steps,
      solution: { type: 'union', variable: varName, root1, root2, operator: currentOp, exact: isExact, region: 'outside' },
      graphData: { type: 'quadratic', a, b, c, variable: varName, roots: [root1, root2], vertex: { x: -b / (2 * a), y: c - b * b / (4 * a) }, inequality: { operator: currentOp } }
    };
  }
}

function buildQuadAST(a, b, c, v) {
  const parts = [];
  if (a === 1) parts.push({ type: 'POWER', base: { type: 'VARIABLE', name: v }, exponent: { type: 'NUMBER', value: 2 } });
  else parts.push({ type: 'MULTIPLY', left: { type: 'NUMBER', value: a }, right: { type: 'POWER', base: { type: 'VARIABLE', name: v }, exponent: { type: 'NUMBER', value: 2 } } });

  let result = parts[0];
  if (b !== 0) {
    const bTerm = Math.abs(b) === 1
      ? { type: 'VARIABLE', name: v }
      : { type: 'MULTIPLY', left: { type: 'NUMBER', value: Math.abs(b) }, right: { type: 'VARIABLE', name: v } };
    result = b > 0 ? { type: 'ADD', left: result, right: bTerm } : { type: 'SUBTRACT', left: result, right: bTerm };
  }
  if (c !== 0) {
    result = c > 0 ? { type: 'ADD', left: result, right: { type: 'NUMBER', value: c } } : { type: 'SUBTRACT', left: result, right: { type: 'NUMBER', value: -c } };
  }
  return result;
}
...
const CURSOR_CSS = `
  @keyframes qi-blink { 0%, 100% { opacity: 1; } 50% { opacity: 0; } }
  .qi-caret { display: inline-block; width: 2px; height: 1.2em; background: #fbbf24; animation: qi-blink 1s step-end infinite; vertical-align: text-bottom; margin: 0 -1px; border-radius: 1px; }
  .qi-display:focus { outline: none; }
  .qi-display:focus .qi-caret { animation: qi-blink 1s step-end infinite; }
  .qi-display:not(:focus) .qi-caret { opacity: 0.4; animation: none; }
  .qi-char { cursor: text; position: relative; }
  .qi-char:hover { background: rgba(255,255,255,0.08); border-radius: 2px; }
  .qi-sup-group { display: inline; cursor: text; }
  .qi-form-card, .qi-form-card:visited, .qi-form-card:active, .qi-form-card:focus, .qi-form-card:focus-visible, .qi-form-card:focus-within { outline: none !important; box-shadow: none; border-color: var(--qi-border-color) !important; }
`;
...
export const QuadraticInequalitySolverEngine = forwardRef(({
  compact = false,
  style = {},
  darkMode = false,
  onResultChange = null,
  ...
  const solve = useCallback(() => {
    try {
      ...
      const solveResult = solveQuadraticInequality(ast);
  ...
  const buildDisplayElements = () => {
    ...
    if (expr.length === 0) {
      if (focused) return <span className="qi-caret" />;
    ...
    for (let i = 0; i < expr.length; i++) { if (expr[i] === '^' && i + 1 < expr.length) supStarts.add(i); }

    const renderCaret = (pos) => (cursorPos === pos && focused) ? <span key={`caret-${pos}`} className="qi-caret" /> : null;
    ...
    while (i < expr.length) {
      if (supStarts.has(i)) {
        ...
        elements.push(<span key={`ch-${i}`} className="qi-char" data-idx={i} style={{ fontSize: '0' }}>{expr[i]}</span>);
        i++;
        const supChars = [];
        while (i < expr.length && /[0-9a-zA-Z.()]/.test(expr[i]) && !supStarts.has(i)) {
          supChars.push({ ch: expr[i], idx: i }); i++;
          if (supChars.length === 1 && !/[()]/.test(supChars[0].ch)) break;
          if (supChars[supChars.length - 1]?.ch === ')') break;
        }
        elements.push(
          <span key={`sup-${supChars[0]?.idx}`} className="qi-sup-group">
            <sup style={eStyles.displaySup}>
              {supChars.map(sc => (<React.Fragment key={`sc-${sc.idx}`}>{renderCaret(sc.idx)}<span className="qi-char" data-idx={sc.idx}>{sc.ch}</span></React.Fragment>))}
      ...
      elements.push(renderCaret(i));
      elements.push(<span key={`ch-${i}`} className="qi-char" data-idx={i} style={charStyle(expr[i])}>{displayChar(expr[i])}</span>);
      i++;
    }
    if (!supStarts.has(expr.length - 1) || expr[expr.length - 1] === '^') elements.push(renderCaret(expression.length));
    return elements;
  };
  ...
  const renderSolutionInline = () => {
    ...
    if (sol.type === 'all') return <span>All real numbers</span>;
    if (sol.type === 'none') return <span>No solution</span>;
    if (sol.type === 'point') return <span><span style={styles.solVar}>{sol.variable}</span><span style={styles.solEq}> = </span><span style={styles.solNum}>{formatNumber(sol.value)}</span></span>;
    if (sol.type === 'all_except') return <span><span style={styles.solVar}>{sol.variable}</span> <span style={styles.solEq}>{'\u2260'}</span> <span style={styles.solNum}>{formatNumber(sol.value)}</span></span>;
    if (sol.type === 'compound') {
      const s = isStrict(sol.operator);
      return <span><span style={styles.solNum}>{formatNumber(sol.root1)}</span><span style={styles.solEq}> {s ? '<' : '\u2264'} </span><span style={styles.solVar}>{sol.variable}</span><span style={styles.solEq}> {s ? '<' : '\u2264'} </span><span style={styles.solNum}>{formatNumber(sol.root2)}</span></span>;
    }
    if (sol.type === 'union') {
      const s = isStrict(sol.operator);
      const op = s ? '<' : '\u2264';
      return <span><span style={styles.solVar}>{sol.variable}</span><span style={styles.solEq}> {s ? '<' : '\u2264'} </span><span style={styles.solNum}>{formatNumber(sol.root1)}</span><span style={styles.solEq}> or </span><span style={styles.solVar}>{sol.variable}</span><span style={styles.solEq}> {s ? '>' : '\u2265'} </span><span style={styles.solNum}>{formatNumber(sol.root2)}</span></span>;
    }
    return null;
  };
  ...
  return (
    <div style={{ ...styles.container, ...style }}>
      ...
      <div ref={displayRef} className="qi-display" tabIndex={0}
      ...
      <div style={{ ...styles.builder, gap: `${12 * p}px` }}>
        ...
        <div style={styles.row}><span style={styles.label}>Ineq</span><div style={styles.btnGroup}>{ineqOps.map((op) => (<button key={op} style={{ ...styles.btn, ...styles.btnOp, color: darkMode ? '#fbbf24' : '#f59e0b', fontWeight: '700' }} onClick={() => insertAt(op)}>{op}</button>))}</div></div>
      ...
      {result && result.solution && <div style={styles.solution}>{renderSolutionInline()}</div>}
    </div>
  );
});

QuadraticInequalitySolverEngine.displayName = 'QuadraticInequalitySolverEngine';
...
const inequalityForms = [
  {
    ...
    generate: (nice) => {
      if (nice) {
        ...
        const r2 = r1 + Math.floor(Math.random() * 5) + 1;
        return `x^2${-(r1 + r2) >= 0 ? '+' + (-(r1 + r2)) : (-(r1 + r2))}\u00D7x+${r1 * r2}<0`;
      }
  ...
  },
  {
    ...
    generate: (nice) => {
      ...
      if (nice) {
        ...
        const r2 = r1 + Math.floor(Math.random() * 4) + 1;
        const b = -a * (r1 + r2);
        const c = a * r1 * r2;
        return `${a}\u00D7x^2${b >= 0 ? '+' + b : b}\u00D7x${c >= 0 ? '+' + c : c}${op}0`;
      }
  ...
  },
  {
    ...
    generate: (nice) => {
      ...
      if (nice) {
        ...
        const b = r1 + r2;
        const c = -(r1 * r2);
        return `-1\u00D7x^2${b >= 0 ? '+' + b : b}\u00D7x${c >= 0 ? '+' + c : c}${op}0`;
      }
...
const QuadraticInequalitySolver = () => {
  ...
  const handleFormClick = (form) => {
    ...
    const ineq = form.generate(isNice);
    setSelectedForm(form.id);
    setSolveResult(null);
    if (engineRef.current) engineRef.current.loadEquation(ineq);
  };
    ...
    if (sol.type === 'none') {
      return (
        <div style={wStyles.finalAnswer}>
          ...
          <div style={wStyles.answerNote}>{'\u2717'} No value satisfies this inequality</div>
          <div style={wStyles.answerInterval}>{'\u2205'}</div>
        </div>
      );
    }
    if (sol.type === 'point') {
      return (
        <div style={wStyles.finalAnswer}>
          ...
          <div style={wStyles.answerNote}>Only at this single point</div>
          <div style={wStyles.answerInterval}>{`{${formatNumber(sol.value)}}`}</div>
        </div>
      );
    }
    if (sol.type === 'all_except') {
      return (
        <div style={wStyles.finalAnswer}>
          <div style={wStyles.answerLabel}>Answer</div>
          <div style={wStyles.answerValue}>
          ...
          <div style={wStyles.answerNote}>All reals except {sol.variable} = {formatNumber(sol.value)}</div>
          <div style={wStyles.answerInterval}>({'\u2212'}{'\u221E'}, {formatNumber(sol.value)}) {'\u222A'} ({formatNumber(sol.value)}, {'\u221E'})</div>
        </div>
      );
    }
    if (sol.type === 'compound') {
      ...
      return (
        <div style={wStyles.finalAnswer}>
          <div style={wStyles.answerLabel}>Answer</div>
          <div style={wStyles.answerValue}>
            <span style={wStyles.answerNum}>{formatNumber(sol.root1)}</span>
            <span style={wStyles.answerEq}> {s ? '<' : '\u2264'} </span>
            <span style={wStyles.answerVar}>{sol.variable}</span>
            <span style={wStyles.answerEq}> {s ? '<' : '\u2264'} </span>
            <span style={wStyles.answerNum}>{formatNumber(sol.root2)}</span>
          </div>
          <div style={wStyles.answerNote}>{sol.exact ? '\u2713 Exact' : '\u2248 Approximate'} &mdash; between the roots</div>
          <div style={wStyles.answerInterval}>{oL}{formatNumber(sol.root1)}, {formatNumber(sol.root2)}{cR}</div>
        </div>
      );
    }
    if (sol.type === 'union') {
      ...
      return (
        <div style={wStyles.finalAnswer}>
          ...
          <div style={wStyles.answerNote}>{sol.exact ? '\u2713 Exact' : '\u2248 Approximate'} &mdash; outside the roots</div>
          <div style={wStyles.answerInterval}>({'\u2212'}{'\u221E'}, {formatNumber(sol.root1)}{cR} {'\u222A'} {oL}{formatNumber(sol.root2)}, {'\u221E'})</div>
        </div>
      );
    }
    return null;
    ...
    <div style={wStyles.container}>
      ...
      <div style={wStyles.inner}>
        <header style={wStyles.header}>
          ...
          <h1 style={wStyles.title}>Quadratic Inequality Solver</h1>
          <p style={wStyles.subtitle}>Solve quadratic inequalities step by step</p>
        </header>
        <div style={wStyles.main}>
          <div style={wStyles.leftCol}>
            <QuadraticInequalitySolverEngine ref={engineRef} onResultChange={handleResultChange} onClear={handleClear} darkMode={darkMode} compact={true} />
                ...
                <div style={wStyles.accordionContent}>
                  ...
                  <div style={wStyles.formsGrid}>
                    {inequalityForms.map((form) => (
                      <button key={form.id} className="qi-form-card"
...
const getWrapperStyles = (darkMode) => ({
  ...
  icon: { color: '#fff', fontSize: '20px', fontWeight: '700', fontStyle: 'italic' },
  ...
  formCard: { '--qi-border-color': darkMode ? '#475569' : '#e2e8f0', background: darkMode ? '#334155' : '#f8fafc', border: darkMode ? '2px solid #475569' : '2px solid #e2e8f0', borderRadius: '10px', padding: '10px 12px', cursor: 'pointer', transition: 'all 0.2s ease', textAlign: 'left', fontFamily: 'inherit' },
  formCardSelected: { '--qi-border-color': '#3b82f6', borderColor: '#3b82f6', background: darkMode ? '#1e3a5f' : '#eff6ff', boxShadow: '0 2px 8px rgba(59, 130, 246, 0.15)' },
  ...
  answerInterval: { fontSize: '1.1rem', fontWeight: '600', marginTop: '8px', opacity: 0.9, fontFamily: 'ui-monospace, monospace', letterSpacing: '1px' },
});

export default QuadraticInequalitySolver;