...
const Base2LogarithmTable = () => {
  ...
  const handleCalculateClick = () => {
    ...
    if (!isNaN(value) && value >= 1.0 && value <= 99.9) {
      ...
      const logValue = Math.log2(value).toFixed(4);
      setCalculatedLog(logValue);
      setHighlightIndex({ baseIndex, decimalIndex, meanDiffIndex: null });
      scrollToRow(baseIndex);
    } else {
      setCalculatedLog('Input out of range');
    }
  };
  ...
  const calculateLogValues = () => {
    ...
    for (let i = 10; i <= 999; i++) {  // Adjusted for values 1.0 to 99.9 in steps of 0.1
      ...
      for (let j = 0; j <= 9; j++) {
        ...
        const logValue = Math.log2(number).toFixed(4);
        row.push(logValue);
      }
      values.push(row);
    }
  ...
  return (
    <>
      <div className='title'><h2>Binary Logarithm Table (Base 2)</h2></div>
      <br></br>
      <br></br>
      <div className="input-area sticky">
        ...
        <br></br>
        <hr></hr>
        {calculatedLog && <div><p>Base 2 Logarithm: {calculatedLog}</p></div>}

      </div>
      <div className="flex-table-container" role="table" aria-label="Base 2 Logarithm Table">
        ...
        {logTableValues.map((row, rowIndex) => (
          <div key={rowIndex} ref={el => rowRefs.current[rowIndex] = el} className="flex-table-row" role="row">
            ...
            {row.map((logValue, columnIndex) => (
              <div key={columnIndex} className={`flex-table-cell tooltip-container ${highlightIndex.baseIndex === rowIndex && highlightIndex.decimalIndex === columnIndex ? 'highlight' : ''}`} role="cell" title={`Log value for ${(1.0 + rowIndex / 10 + columnIndex / 100).toFixed(2)}: ${logValue}`}>
                {logValue}
                <span className="tooltip">Base 2 Logarithm value for {(rowIndex + 10) + columnIndex / 10}: {logValue}</span>
              </div>
            ))}
          </div>
        ))}
...
export default Base2LogarithmTable;
