...
export function CircularPermutation({ explanations }) {
  ...
  const calculate = async () => {
    ...
    try {
      ...
      if (nNum > 100) {
        try {
          const apiResult = await CombinatoricsAPI.calculate('circular-permutation', { n: nNum })
          setResult({
            n: nNum,
            result: apiResult,
            formula: `$(${nNum}-1)! = ${nNum - 1}!$`,
      ...
      const factorialInput = nNum - 1
      const memoizedResult = getFactorial(factorialInput)
