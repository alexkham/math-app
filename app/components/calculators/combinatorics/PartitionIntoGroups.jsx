...
export function PartitionIntoGroups({ explanations }) {
  ...
  const clearGroupSize = (index) => {
    updateGroupSize(index, '')
  }
  ...
  const calculate = async () => {
    ...
    try {
      ...
      if (nNum > 100) {
        try {
          const apiResult = await CombinatoricsAPI.calculate('partition-groups', { n: nNum, groups: sizes })
          const sizesStr = sizes.join(',')
          const denominatorStr = sizes.map(size => size + '!').join(' \\times ')
      ...
      for (let size of sizes) {
        const sizeFactorial = getFactorial(size)
        if (sizeFactorial === null) {
          try {
            const apiResult = await CombinatoricsAPI.calculate('partition-groups', { n: nNum, groups: sizes })
            const sizesStr = sizes.join(',')
            const denominatorStr = sizes.map(size => size + '!').join(' \\times ')
        ...
        factorials.push(sizeFactorial)
        denominator *= parseInt(sizeFactorial)
      }

      const multinomialResult = Math.floor(parseInt(nFactorial) / denominator)
      ...
      setResult({
        ...
        result: multinomialResult,
        formula: '$\\binom{' + nNum + '}{' + sizesStr + '} = \\frac{' + nNum + '!}{' + denominatorStr + '}$',
        calculation: '$\\frac{' + nFactorial + '}{' + factorials.join(' \\times ') + '}$',
        method: 'local'
      })
          ...
          <div style={{ marginBottom: '25px' }}>
            ...
            <div style={{
              ...
              <button
                ...
                disabled={loading || !n || groupSizes.every(s => s === '')}
              >
                Calculate
              </button>
