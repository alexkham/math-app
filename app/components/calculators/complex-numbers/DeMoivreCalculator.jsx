        ...
        <div style={styles.leftCol}>
          <div style={styles.planeWrap}>
            ...
            >
              ...
              {modResult > 0.2 && modResult <= range && (
                <circle cx={originSvg.x} cy={originSvg.y} r={modResult * scale} fill="none" stroke="#2A7A8C" strokeWidth="1" strokeDasharray="4,3" opacity="0.3" />
              )}
              ...
              {/* Angle arc for result */}
              {modResult > 0.1 && Math.abs(argResult) > 0.05 && (
                <path d={makeArc(1.2, 0, argResultNorm)} fill="none" stroke="#2A7A8C" strokeWidth="2.5" opacity="0.6" />
              )}
              ...
              {modResult > 0.1 && Math.abs(argResultNorm) > 0.15 && (() => {
                const mid = argResultNorm / 2;
                const lp = toSvg(1.5 * Math.cos(mid), 1.5 * Math.sin(mid));
                return <text x={lp.x} y={lp.y} textAnchor="middle" fontSize="12" fontWeight="700" fill="#2A7A8C" fontFamily="'JetBrains Mono', monospace">nθ</text>;
              })()}
              ...
              {modResult > 0.1 && (() => {
                if (resultVisible) {
                  return <line x1={originSvg.x} y1={originSvg.y} x2={rSvg.x} y2={rSvg.y} stroke="#2A7A8C" strokeWidth="3" />;
                } else {
                  const edgeDist = range - 0.3;
                  const cosA = Math.cos(argResultNorm);
                  const sinA = Math.sin(argResultNorm);
              ...
              {resultVisible && modResult > 0.1 && (
                <>
                  <circle cx={rSvg.x} cy={rSvg.y} r="9" fill="#2A7A8C" stroke="#fff" strokeWidth="2" />
                  <text x={rSvg.x + 14} y={rSvg.y - 12} fontSize="14" fontWeight="700" fill="#2A7A8C" fontFamily="'DM Sans', sans-serif" style={{ pointerEvents: 'none' }}>
                    z{toSup(n)}
                  </text>
                </>
              )}
                ...
                return (
                  <g>
                    ...
                    <g clipPath="url(#insetClipDM)">
                      ...
                      <text x={iqx + 10} y={iqy - 8} fontSize="12" fontWeight="700" fill="#2A7A8C" fontFamily="'DM Sans', sans-serif">z{toSup(n)}</text>
                      <circle cx={insetCx} cy={insetCy} r="2.5" fill="#8098b0" />
                      <text x={insetX + insetSize / 2} y={insetY + insetSize - 8} textAnchor="middle" fontSize="10" fontWeight="600" fill="#5a6480" fontFamily="'JetBrains Mono', monospace">
                        {fmtComplex(resultRe, resultIm)}
                      </text>
