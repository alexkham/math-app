...
export default function EulerFormulaExplorer() {
  ...
  return (
    <div style={styles.container}>
      ...
      <div style={styles.header}>
        <span style={styles.subtitle}>e<sup>i&theta;</sup> = cos &theta; + i sin &theta;</span>
      </div>
      ...
      <div style={styles.mainLayout}>
        <div style={styles.leftCol}>
          <div style={styles.planeWrap}>
            ...
            >
              ...
              {/* Angle arc */}
              {theta > 0.01 && (
                <path d={arcPath(0.35 * radius, 0, theta)} fill="none" stroke="#B85C2A" strokeWidth="2" />
              )}
              ...
              {theta > 0.15 && (() => {
                ...
                const lp = toSvg(labelR * Math.cos(midA), labelR * Math.sin(midA));
                return (
                  <text x={lp.x} y={lp.y} textAnchor="middle" fontSize="14" fontWeight="700" fill="#B85C2A" fontFamily="'DM Sans', sans-serif">&theta;</text>
                );
              })()}
              ...
              {Math.abs(zRe) > 0.15 && Math.abs(zIm) > 0.15 && (() => {
                ...
                return (
                  <polyline
                    points={`${projReSvg.x},${projReSvg.y + dirY * sz} ${projReSvg.x + dirX * sz},${projReSvg.y + dirY * sz} ${projReSvg.x + dirX * sz},${projReSvg.y}`}
                    fill="none" stroke="#8098b0" strokeWidth="1"
                  />
                );
              })()}
              ...
              {radius > 0.3 && (
                <text
                  x={(originSvg.x + zSvg.x) / 2 - (zIm >= 0 ? 12 : -12)}
                  y={(originSvg.y + zSvg.y) / 2 + (zIm >= 0 ? -8 : 16)}
                  textAnchor="middle" fontSize="11" fontWeight="600" fill="#304090"
                  fontFamily="'JetBrains Mono', monospace"
                >
                  r = {fmtNum(radius)}
                </text>
              ...
              {landmarks.map((lm, i) => {
                ...
                const isActive = Math.abs(theta - lm.angle) < 0.05 && radius === 1;
                return (
                  <circle key={i} cx={lp.x} cy={lp.y} r={isActive ? 5 : 3}
                    fill={isActive ? '#B85C2A' : '#4098d8'} opacity={isActive ? 1 : 0.5} />
                );
              })}
              ...
              <circle className="drag-point" cx={zSvg.x} cy={zSvg.y} r="11"
                ...
                onPointerDown={handlePointerDown} />
              <text x={zSvg.x + 16} y={zSvg.y - 10} fontSize="14" fontWeight="700"
                fill="#304090" fontFamily="'DM Sans', sans-serif" style={{ pointerEvents: 'none' }}>
                e<tspan fontSize="10" baselineShift="super">i&theta;</tspan>
              </text>
          ...
          <div style={styles.controlsBox}>
            <div style={styles.sliderRow}>
              <label style={styles.sliderLabel}>&theta;</label>
              <input type="range" min="0" max={2 * Math.PI} step="0.01" value={theta}
                onChange={(e) => setTheta(parseFloat(e.target.value))} style={styles.slider} />
              <span style={styles.sliderVal}>{(theta * 180 / Math.PI).toFixed(1)}&deg; = {fmtRad(theta)}</span>
            </div>
            <div style={styles.sliderRow}>
              <label style={styles.sliderLabel}>r</label>
              <input type="range" min="0.1" max={maxR} step="0.1" value={radius}
                onChange={(e) => setRadius(parseFloat(e.target.value))} style={styles.slider} />
              <span style={styles.sliderVal}>{fmtNum(radius)}</span>
            </div>
            <div style={styles.landmarkRow}>
              {landmarks.map((lm) => (
                <button key={lm.label} className="landmark-btn"
                  style={{ ...styles.landmarkBtn, ...(Math.abs(theta - lm.angle) < 0.05 ? styles.landmarkBtnActive : {}) }}
                  onClick={() => { setTheta(lm.angle); setRadius(1); }}>
                  {lm.label}
                </button>
              ))}
        ...
        <div style={styles.panel}>
          <div style={styles.valuesBox}>
            ...
            <div style={styles.formulaRow}>
              <span style={styles.formulaLabel}>
                {radius === 1 ? '' : `${fmtNum(radius)} \u00b7 `}e<sup>i&theta;</sup>
              </span>
              <span style={styles.formulaEquals}>=</span>
              <span style={styles.formulaResult}>{fmtComplex(zRe, zIm)}</span>
            </div>
            <div style={styles.valGrid}>
              <span style={styles.valKey}>&theta;</span>
              <span style={styles.valVal}>{fmtRad(theta)} = {(theta * 180 / Math.PI).toFixed(1)}&deg;</span>

              <span style={styles.valKey}>cos &theta;</span>
              <span style={{ ...styles.valVal, color: '#2A7A8C' }}>{fmtNum(cosT)}</span>

              <span style={styles.valKey}>sin &theta;</span>
              <span style={{ ...styles.valVal, color: '#c04040' }}>{fmtNum(sinT)}</span>

              <span style={styles.valKey}>Re(z)</span>
              <span style={{ ...styles.valVal, color: '#2A7A8C' }}>{fmtNum(zRe)}</span>
          ...
          <div style={styles.breakdownBox}>
            ...
            <div style={styles.breakdownLine}>
              <span style={styles.breakdownStep}>1</span>
              <span>Start with Euler&apos;s formula: <strong>e<sup>i&theta;</sup> = cos &theta; + i sin &theta;</strong></span>
            </div>
            {radius !== 1 && (
              <div style={styles.breakdownLine}>
                <span style={styles.breakdownStep}>&bull;</span>
                <span>Multiply by r: <strong>r &middot; e<sup>i&theta;</sup> = r cos &theta; + i &middot; r sin &theta;</strong></span>
              </div>
            )}
            <div style={styles.breakdownLine}>
              <span style={styles.breakdownStep}>2</span>
              <span>Substitute &theta; = {fmtRad(theta)}:</span>
            </div>
            <div style={styles.breakdownMath}>
              {radius === 1 ? '' : `${fmtNum(radius)} \u00b7 `}e<sup>i&middot;{fmtRad(theta)}</sup> = {radius === 1 ? '' : `${fmtNum(radius)}(`}cos({fmtRad(theta)}) + i sin({fmtRad(theta)}){radius === 1 ? '' : ')'}
            </div>
            <div style={styles.breakdownLine}>
              <span style={styles.breakdownStep}>3</span>
              <span>Evaluate:</span>
            ...
            <div style={styles.breakdownMath}>
              = {radius === 1 ? '' : `${fmtNum(radius)}(`}<span style={{ color: '#2A7A8C' }}>{fmtNum(cosT)}</span> + i &middot; <span style={{ color: '#c04040' }}>{fmtNum(sinT)}</span>{radius === 1 ? '' : ')'}
              {' '}= <strong>{fmtComplex(zRe, zIm)}</strong>
            </div>
          </div>
          ...
          <div style={styles.explainBox}>
            ...
            <div style={styles.explainItem}>
              <span style={{ ...styles.dot, backgroundColor: '#4098d8' }}></span>
              <span>
                <strong>e<sup>i&theta;</sup> traces the unit circle.</strong> As &theta; goes from 0 to 2&pi;, the point completes one full revolution. The real part is cos &theta;, the imaginary part is sin &theta;.
              </span>
            </div>
            <div style={styles.explainItem}>
              <span style={{ ...styles.dot, backgroundColor: palette.orange }}></span>
              <span>
                <strong>&theta; is the angle from the positive real axis,</strong> measured counterclockwise in radians. One full turn = 2&pi; radians = 360&deg;.
              </span>
            </div>
            <div style={styles.explainItem}>
              <span style={{ ...styles.dot, backgroundColor: palette.teal }}></span>
              <span>
                <strong>The right triangle connects trig to complex numbers.</strong> The hypotenuse is r (the modulus), the horizontal leg is r cos &theta; (real part), and the vertical leg is r sin &theta; (imaginary part).
              </span>
            </div>
            <div style={styles.explainItem}>
              <span style={{ ...styles.dot, backgroundColor: '#c04040' }}></span>
              <span>
                <strong>Why use e<sup>i&theta;</sup> instead of cos &theta; + i sin &theta;?</strong> Because multiplication becomes simple: e<sup>i&alpha;</sup> &middot; e<sup>i&beta;</sup> = e<sup>i(&alpha;+&beta;)</sup>. Multiplying complex numbers = adding angles. The exponential form makes rotation algebra trivial.
              </span>
            </div>
            <div style={styles.explainItem}>
