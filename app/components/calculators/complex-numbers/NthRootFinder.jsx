...
export default function NthRootFinder() {
  ...
  for (let k = 0; k < n; k++) {
    const angle = (arg + 2 * Math.PI * k) / n;
    roots.push({ k, angle, re: rootMod * Math.cos(angle), im: rootMod * Math.sin(angle) });
  }
      ...
      <div style={styles.mainLayout}>
        <div style={styles.leftCol}>
          <div style={styles.planeWrap}>
            ...
            >
              ...
              {/* Root circle */}
              {rootMod > 0.15 && rootMod <= range && (
                <circle cx={ox} cy={oy} r={rootMod * scale} fill="none" stroke="#2A7A8C" strokeWidth="1.5" strokeDasharray="5,3" opacity="0.4" />
              )}
              ...
              {visibleCount >= n && n >= 3 && allRootsVisible && (
                <polygon
                  points={roots.map(r => { const s = toSvg(r.re, r.im); return `${s.x},${s.y}`; }).join(' ')}
                  fill="#2A7A8C" fillOpacity="0.04"
                  stroke="#2A7A8C" strokeWidth="1.5" strokeOpacity="0.3"
                />
              )}

              {/* Angle arc between first two roots */}
              {n >= 2 && visibleCount >= 2 && rootMod > 0.2 && rootMod <= range && (
                <path d={makeArc(roots[0].angle, roots[1].angle, rootMod * scale * 0.45)}
                  fill="none" stroke="#B85C2A" strokeWidth="2.5" />
              )}
              {n >= 2 && visibleCount >= 2 && rootMod > 0.3 && (() => {
                const midAngle = (roots[0].angle + roots[1].angle) / 2;
                const labelR = rootMod * scale * 0.58;
                return (
                  <text
                    x={ox + labelR * Math.cos(midAngle)}
                    y={oy - labelR * Math.sin(midAngle)}
                    textAnchor="middle" fontSize="12" fontWeight="700" fill="#B85C2A"
                    fontFamily="'JetBrains Mono', monospace"
                  >
                    2&pi;/{n}
                  </text>
                ...
                return (
                  <g key={root.k}>
                    ...
                    <text
                      x={s.x + 14 * Math.cos(root.angle)}
                      y={s.y - 14 * Math.sin(root.angle)}
                      textAnchor="middle" fontSize={isHi ? 13 : 11} fontWeight="700"
              ...
              {/* Radius label */}
              {rootMod > 0.3 && rootMod <= range && (
                <text x={ox + rootMod * scale / 2 + 4} y={oy + 16}
                  fontSize="11" fontWeight="600" fill="#2A7A8C"
                  fontFamily="'JetBrains Mono', monospace">
                  |z|^(1/{n}) = {fmtShort(rootMod)}
                </text>
              )}
          ...
          <div style={styles.controlsBox}>
            <div style={styles.inputRow}>
              ...
              <input type="number" min="2" max="20" value={n}
                ...
                style={{ ...styles.numInput, width: '56px' }} />
            </div>
            <div style={styles.sliderRow}>
              <label style={styles.sliderLabel}>n</label>
              <input type="range" min="2" max="12" step="1" value={n}
        ...
        <div style={styles.panel}>
          ...
          <div style={styles.stepsBox}>
            ...
            <div style={styles.stepMath}>
              z = {fmtShort(mod)} &middot; e<sup>i&middot;{fmtAngleRad(arg)}</sup>
            </div>
            <div style={styles.stepLine}>
              <span style={styles.stepNum}>2</span>
              <span>The n-th root modulus is |z|<sup>1/n</sup>:</span>
            ...
            <div style={styles.stepMath}>
              {fmtShort(mod)}<sup>1/{n}</sup> = <strong style={{ color: palette.teal }}>{fmtShort(rootMod)}</strong>
            </div>
            <div style={styles.stepLine}>
              <span style={styles.stepNum}>3</span>
              <span>The k-th root has angle (arg(z) + 2&pi;k) / n:</span>
            </div>
            <div style={styles.stepMath}>
              &theta;<sub>k</sub> = ({fmtDeg(arg)} + 360&deg;&middot;k) / {n}, &nbsp; k = 0, 1, &hellip;, {n - 1}
            </div>
            <div style={styles.stepLine}>
              <span style={styles.stepNum}>4</span>
              <span>This gives {n} roots spaced <strong style={{ color: '#B85C2A' }}>{fmtShort(360 / n)}&deg;</strong> apart on a circle of radius <strong style={{ color: palette.teal }}>{fmtShort(rootMod)}</strong>:</span>
            </div>
          </div>
              ...
              <table style={styles.table}>
                ...
                <tbody>
                  {roots.map((root) => {
                    ...
                    const vIm = Math.pow(rootMod, n) * Math.sin(root.angle * n);
                    return (
                      <tr key={root.k} className="root-row"
                        style={{ ...styles.tr, opacity: isVis ? 1 : 0.3, backgroundColor: isHi ? '#e0ecff' : 'transparent' }}
                        onMouseEnter={() => setHighlightK(root.k)}
                        onMouseLeave={() => setHighlightK(null)}
                      >
                        <td style={styles.td}>
                        ...
                        <td style={styles.td}>{fmtAngleFrac(root.k)}</td>
                        <td style={{ ...styles.td, fontWeight: '600' }}>{fmtComplex(root.re, root.im)}</td>
                        <td style={{ ...styles.td, color: palette.teal, fontSize: '0.82rem' }}>
                          w{toSup(n)} = {fmtComplex(vRe, vIm)} &#x2713;
                        </td>
                      </tr>
                    );
          ...
          <div style={styles.explainBox}>
            ...
            <div style={styles.explainItem}>
              ...
              <span>
                <strong>The roots form a regular n-gon</strong> because consecutive roots differ by exactly 2&pi;/n = {fmtShort(360 / n)}&deg; in angle. The first root&apos;s angle is arg(z)/n; the rest are evenly spaced from there.
              </span>
            </div>
            <div style={styles.explainItem}>
              <span style={{ ...styles.dot, backgroundColor: palette.navy }}></span>
              <span>
                <strong>De Moivre&apos;s theorem is the engine:</strong> w<sup>n</sup> = z means w = |z|<sup>1/n</sup> &middot; e<sup>i(arg(z) + 2&pi;k)/n</sup>. Each value of k = 0, 1, &hellip;, n&minus;1 gives a different root. After that, the angles repeat.
              </span>
            </div>
            <div style={styles.explainItem}>
              <span style={{ ...styles.dot, backgroundColor: '#6A4E7A' }}></span>
              <span>
                <strong>Special case: roots of unity.</strong> When z = 1, the roots are just the n-th roots of unity &mdash; evenly spaced on the unit circle starting at angle 0. For general z, the circle&apos;s radius changes and the whole polygon rotates.
              </span>
            </div>
            <div style={styles.explainItem}>
              <span style={{ ...styles.dot, backgroundColor: palette.blue }}></span>
              <span>
                <strong>Try &radic;(&minus;1):</strong> the two square roots of &minus;1 are i and &minus;i. The polygon is a line segment through the origin on the imaginary axis &mdash; the simplest possible &ldquo;2-gon.&rdquo;
              </span>
            </div>
          </div>
        </div>
