 ...
 * @property {string} [image] - Image path (renders on left side)
 * @property {React.ComponentType} [icon] - Icon component
 * @property {string|React.ReactElement} [svg] - Raw SVG (string or JSX element)
 * @property {number} [ratio] - Flex ratio for paired cards (both cards need ratio)
 * 
 * @note Visual priority per card: image > icon > svg > text-only fallback
 * @note Each card receives id="card-{index}" for on-page linking
 * @note Works with QuickNav component for smooth scroll navigation
 * 
 * @example
 * <ModernCardsGroup items={[
 *   { title: "Tool 1", description: "...", href: "/tool-1", backgroundColor: "#4F46E5", textColor: "#fff" },
 *   { title: "Tool 2", description: "...", href: "/tool-2", svg: "<svg viewBox='0 0 24 24'>...</svg>" },
 *   { title: "Tool 3", description: "...", href: "/tool-3" } // text-only fallback
 * ]} />
 */
...
const Card = ({ item, index, widthStyle }) => {
  const [imageFailed, setImageFailed] = useState(false);

  const hasImage = isValidImagePath(item.image) && !imageFailed;
  const hasIcon = !hasImage && !!item.icon;
  const hasSvg = !hasImage && !hasIcon && !!item.svg;
  const hasVisual = hasImage || hasIcon || hasSvg;

  const svgIsString = hasSvg && typeof item.svg === 'string';
  const svgIsElement = hasSvg && React.isValidElement(item.svg);
  ...
  return (
    ...
    >
      ...
      <div className={`${styles.contentContainer} ${hasVisual ? styles.withImage : styles.fullWidth}`}>
        <div>
          <div className={styles.titleRow}>
            {hasIcon && (
              <div className={styles.iconContainer}>
            ...
            {hasSvg && svgIsString && (
              <div
                ...
                style={{ color: item.textColor || '#4F46E5' }}
  ...
  return (
    <div className={styles.grid}>
      {items.map((item, index) => {
        ...
        return <Card key={index} item={item} index={index} widthStyle={widthStyle} />;
      })}
    </div>
  );
};
