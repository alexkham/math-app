...
const SEOFriendlySectionTableOfContents = ({ 
  sections = [], 
  title = '',
    ...
    if (section.subsections) {
      return (
        <div key={section.id} className="toc-item accordion" style={{backgroundColor: `${section.background}`}}>
          {/* SEO-friendly link that's always visible */}
          <a 
            href={section.link ? section.link : `#${section.id}`}
            className="toc-link seo-link"
            onClick={section.link ? null : (e) => handleLinkClick(e, section.id)}
          >
            {section.icon && <span className="toc-icon">{renderIcon(section.icon)}</span>}
            <span className="section-title">{section.title}</span>
          </a>
          
          {/* Interactive button for expanding/collapsing */}
          <button 
  ...
  const renderSecondaryNav = () => {
    ...
    // For non-sticky mode - keep original accordion behavior but with SEO-friendly links
    return (
...
export default SEOFriendlySectionTableOfContents;