...
// /**
//  * VisualToolsPage v31 — file 1 of 3 (visualToolsCards.jsx)
//  *
//  * Change from v30:
//  *   - FadingDescription now runs processContent on string children, so card
//  *     descriptions can carry inline $...$ math and markdown. Wrapping element
//  *     changed from <p> to <div> to allow block-level output from processContent.
//  *
//  * Change from v29:
//  *   - Reverted BigB1 to fixed `height` (from `minHeight`), restored
//  *     `overflow: hidden`, removed the `alignSelf: stretch` on icon column.
...
//  *
//  * Change from v26:
//  *   - BigB1: title shrunk (1.4rem → 1.15rem, marginBottom 22 → 12).
//  *
//  * Change from v25:
//  *   - BigB1 icon column width reduced from 45% to 33.33%.
//  *
...
// /* ================================================================
//    FADING DESCRIPTION
//    - Runs string children through processContent so card descriptions
//      can carry inline $...$ math and markdown.
//    - Wrapper is <div>, not <p>, because processContent may emit block
//      elements (which would be invalid inside <p>).
//    ================================================================ */
...
//     }}
//   >
//     <div style={{ margin: 0, fontSize, lineHeight, color, opacity, paddingBottom: FADE_PX }}>
...
/**
 * VisualToolsPage v32 — file 1 of 3 (visualToolsCards.jsx)
 *
 * Change from v31:
 *   - resolveMiniVariant / resolveBigVariant now treat icon-only items as
 *     "no visual" for V1/V2 and B1/B2. An icon alone is no longer enough to
 *     justify the banner layout — those variants need a real image or SVG.
 *     Items with only `icon` set fall through to V3 / B3, which inline the
 *     icon as a small badge beside the title, leaving the full card height
 *     for the description.
 *
 * Change from v31 (prior):
 *   - FadingDescription runs processContent on string children, so card
 *     descriptions can carry inline $...$ math and markdown. Wrapping element
 *     changed from <p> to <div> to allow block-level output from processContent.
 *
 * Change from v30 (prior):
 *   - Reverted BigB1 to fixed `height` (from `minHeight`), restored
 *     `overflow: hidden`, removed the `alignSelf: stretch` on icon column.
 *   - Reverted AlgorithmicGrid to `alignItems: 'start'`.
 *
 * Change from v28 (prior):
 *   - DEFAULT_CARD_HEIGHTS.b1: 280 → 240 (matches b3 baseline).
 *
 * Change from v26 (prior):
 *   - BigB1: title shrunk (1.4rem → 1.15rem, marginBottom 22 → 12).
 *
 * Change from v25 (prior):
 *   - BigB1 icon column width reduced from 45% to 33.33%.
 *
 * Change from v24 (prior):
 *   - Visual fields read from item.* or item.seoData.* via raw* helpers.
 *
 * Change from v22 (prior):
 *   - MiniV3 + BigB3 icon badges render inline SVG via CardIconBadge.
 */
...
/* ================================================================
   VARIANT RESOLVERS
   - V1/V2 and B1/B2 are banner-image layouts. They need a real image or
     SVG to justify the banner. An icon alone is not a banner-worthy
     visual — those items degrade to V3 / B3, which inline the icon next
     to the title.
   ================================================================ */
...
export function resolveMiniVariant(requested, item) {
  ...
  const hasBannerVisual = visualType === 'image' || visualType === 'svg';
  const hasFormula = !!(item && item.formula);
  if (v === 'v4' && !hasFormula) v = 'v1';
  if ((v === 'v1' || v === 'v2') && !hasBannerVisual) v = 'v3';
  return v;
}
...
export function resolveBigVariant(requested, item) {
  ...
  const visualType = getVisualType(item);
  const hasBannerVisual = visualType === 'image' || visualType === 'svg';
  if ((v === 'b1' || v === 'b2') && !hasBannerVisual) v = 'b3';
  return v;
}
