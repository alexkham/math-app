...
// /components/visual-tools/scenes/FullPermutation.jsx  (v2)
// Changes from v1:
//   - SceneTitle removed (h1 belongs to the host page; SEO).
//   - HelpBanner added on top with a concise usage note.
//   - SVG_W trimmed 800 → 720 to match the compact shared-v2 layout.
//   - Ball sizes, font sizes, and all internal SVG geometry unchanged.
...
import {
  ...
  // helpers
  permsAll, factorial, getItems, nameOf, tint,
  // theme + shells
...
// ── SVG geometry constants (unchanged except SVG_W from shared default) ──
const SVG_W = SVG_W_DEFAULT;   // = 720 in v2
const PAD = 30;
const SRC_R = 26;
const SRC_Y = 56;
const SRC_SP = 80;
...
export default function FullPermutation() {
  ...
  const outcomes = useMemo(() => permsAll(itemIds), [itemIds]);
  ...
  }, [n, items, itemsPerOutcome, outcomesPerGroup]);
  ...
  const stepStatuses = useMemo(() => {
    ...
    return arr;
  }, [completed, outcomes, outcomeIdx, animState, n]);
  ...
  // ── Header texts ──────────────────────────────────────
  const formulaSeq = Array.from({ length: n }, (_, i) => n - i).join(" × ");
  const formulaText = `P(${n}) = ${n}! = ${formulaSeq} = ${factorial(n)}`;
  const formulaShort = `P(${n}) = ${n}! = ${outcomes.length}`;

  let statusText;
  if (animState === "done") {
    statusText = `Complete · ${outcomes.length} / ${outcomes.length}`;
  ...
  const narrationFor = (stepIdx) => {
    ...
    const seq = n > 1
      ? Array.from({ length: n - 1 }, (_, idx) => n - 1 - idx).join(" × ")
      : "1";

    if (n < 2) {
      return (
        <>
          Only <ChipAndName item={item} mode={mode} /> is available — exactly one permutation here.
        </>
      );
    }
    return (
      <>
        ...
        {" "}The remaining <b>{n - 1}</b> positions are filled with{" "}
  ...
  return (
    <VTRoot>
      ...
      <PageWrap>
        <HelpBanner>
          Press <b>▶ Play</b> to auto-build all permutations, or{" "}
          <b>Step ▶</b> to advance one ball at a time. Switch between{" "}
          <b>balls / letters</b> and adjust <b>n</b> to change the number of items.
        </HelpBanner>
        ...
        <SceneGrid>
          ...
          <SceneSvgWrap>
            ...
            >
              ...
              {/* Dotted guide line (visible during spawn/fly) */}
          ...
          <RightPanel>
            <InfoPanelHeader
              ...
              title="Building all permutations"
              formula={formulaText}
              intro={
                <>
                  Each step builds the family of permutations starting with one
                  fixed item. With <b>n</b> items there are <b>n</b> such families,
                  each containing <b>(n−1)!</b> arrangements. Steps appear here as
                  they unfold.
                </>
              }
            />
