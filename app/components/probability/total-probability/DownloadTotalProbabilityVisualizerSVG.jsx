...
export default function TotalProbabilityVisualizerSVGDownload() {
  const svgRef = useRef(null);
  const [numA, setNumA] = useState(3);
  const [numB, setNumB] = useState(3);
  
  // Probabilities for A events (must sum to 1)
  const [aProbs, setAProbs] = useState([0.33, 0.33, 0.34]);
  
  // Conditional probabilities P(Bj|Ai) - 2D array [numA][numB]
  ...
  // Get max B based on numA (inverse relationship)
  ...
  const downloadSVG = () => {
    ...
    // Remove style attributes with transitions and convert to proper SVG attributes
    ...
    link.href = url;
    link.download = 'total-probability-tree.svg';
  ...
  // Update individual A probability
  ...
  return (
    <div style={{ padding: '20px', maxWidth: '1700px', margin: '0 auto', fontFamily: 'system-ui, -apple-system, sans-serif' }}>
      <p style={{ color: '#666', marginBottom: '30px' }}>
        Visualize total probability with multiple events and outcomes. P(Bⱼ) = Σᵢ P(Aᵢ) × P(Bⱼ|Aᵢ)
      </p>
          ...
          <div style={{ background: 'white', border: '1px solid #e2e8f0', borderRadius: '8px', padding: '20px', marginBottom: '20px' }}>
            <svg ref={svgRef} width={width} height={height} style={{ display: 'block' }}>
              ...
              {paths.map((path, idx) => {
                ...
                // For second level, extract which B outcome this is to stagger labels
                ...
                if (isSecondLevel) {
                  ...
                  if (match) {
                    ...
                    verticalOffset = (bIndex - centerOffset) * 8; // Stagger by 8px per outcome
                  }
                }
