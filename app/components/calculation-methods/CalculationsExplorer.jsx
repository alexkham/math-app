...
function Chip({ label, active, onClick }) {
  ...
  return (
    <button
      ...
      style={{
        background: active ? ACCENT : SURFACE,
        border: `1px solid ${active ? ACCENT : (hover ? ACCENT : "transparent")}`,
        padding: "4px 11px",
        font: "500 12.5px inherit",
        color: active ? "white" : (hover ? ACCENT : INK),
        borderRadius: "999px",
        cursor: "pointer",
        transition: "all .12s",
        userSelect: "none",
...
function FilterBar({ filters, activeFilters, onToggle, onClear, hasActive, stickyTop }) {
  return (
    ...
    >
      <div style={{ maxWidth: MAX_W, margin: "0 auto", padding: `0 ${GUTTER}` }}>
        <div
          style={{
            background: SURFACE_2,
...
function CategorySection({ cat, numeral, methods, totalInCategory, showCount, components }) {
  return (
    <section
      id={`cat-${cat.id}`}
      data-cat-id={cat.id}
      style={{ marginBottom: "200px", scrollMarginTop: "180px" }}
    >
      <header
        style={{
          display: "flex", alignItems: "baseline", gap: "12px",
...
export default function CalculationsExplorer({ data = {} }) {
  ...
  const resolvedSidebarTop =
    sidebarStickyTop != null
      ? sidebarStickyTop
      : (filters ? filterStickyTop + 200 : filterStickyTop + 20);
