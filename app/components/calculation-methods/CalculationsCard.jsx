...
// // const ACCENT = '#4a6cf7';
// // const ACCENT_50 = '#f3f5fe';
// // const ACCENT_LIGHT = '#eef1fd';
// // const ACCENT_BORDER = '#cdd5f8';
// // const ACCENT_INK = '#2740a8';
...
// // /* Fields with no renderable content should not produce tabs */
...
// // // ─── Front-of-card icons (HEADER — unchanged) ────────────────────
...
// //         borderRadius: '6px',
// //         background: hover ? SURFACE : SURFACE_2,
// //         border: `1px solid ${hover ? FORMULA_INK : BORDER_SOFT}`,
// //         color: hover ? FORMULA_INK_HOVER : FORMULA_INK,
// //         display: 'inline-flex',
// //         alignItems: 'center',
// //         justifyContent: 'center',
...
// // // ═════════════════════════════════════════════════════════════════
// // // STATEMENT ZONE (v8 pattern — mirrors MethodsExplorer)
// // // Blue-tinted container: white statement box (formula) + split info
// // // (When to use, blue tint) + (Notation, gray tint), each with a
// // // 3px left accent bar.
// // // ═════════════════════════════════════════════════════════════════

// // function StatementZoneInner({ formula, when, notation }) {
// //   if (!formula && !when && !notation) return null;
// //   const hasSplit = !!(when || notation);
// //   const splitCols = when && notation ? '1fr 1fr' : '1fr';
...
// //       gap: '10px',
// //     }}>
// //       {formula && <StatementBox formula={formula} />}
// //       {hasSplit && (
// //         <div style={{
// //           display: 'grid',
...
// //         }}>
// //           {when && (
// //             <InfoCell variant="blue" label="When to use">
// //               {when}
// //             </InfoCell>
// //           )}
// //           {notation && (
// //             <InfoCell variant="gray" label="Notation">
// //               {notation}
// //             </InfoCell>
// //           )}
// //         </div>
...
// // function InfoCell({ variant, label, children }) {
// //   const isBlue = variant === 'blue';
// //   const bg = isBlue ? ACCENT_LIGHT : SURFACE_2;
// //   const border = isBlue ? ACCENT_BORDER : BORDER_SOFT;
// //   const bar = isBlue ? ACCENT : SLATE_500;
// //   const labelColor = isBlue ? ACCENT_INK : SLATE_700;
// //   const textColor = isBlue ? ACCENT_INK : INK;
...
// //         color: textColor,
// //       }}>
// //         {processContent(String(children))}
// //       </div>
// //     </div>
// //   );
// // }
...
// // function TabsView({ fields, components }) {
// //   const entries = Object.entries(fields).filter(([, v]) => !isEmptyField(v));
// //   const [active, setActive] = useState(0);
// //   if (entries.length === 0) return null;
// //   const safeActive = Math.min(active, entries.length - 1);
...
// //         ))}
// //       </nav>
// //       <div style={{ padding: '18px 22px 22px', background: SURFACE }}>
// //         <FieldContent value={entries[safeActive][1]} components={components} />
// //       </div>
// //     </>
// //   );
// // }
...
// //         color: active ? ACCENT : hover ? SLATE_700 : MUTED,
// //         cursor: 'pointer',
// //         borderBottom: `2px solid ${active ? ACCENT : 'transparent'}`,
// //         marginBottom: '-1px',
// //         whiteSpace: 'nowrap',
...
// // function StackedView({ when, formula, notation, fields, components }) {
// //   const entries = fields
// //     ? Object.entries(fields).filter(([, v]) => !isEmptyField(v))
// //     : [];
// //   const hasEssentials = !!(when || formula || notation);
// //   if (!hasEssentials && entries.length === 0) return null;
...
// //     <div style={{ background: SURFACE }}>
// //       {hasEssentials && (
// //         <StackedRow title="Essentials" hint={hint} defaultOpen>
// //           <StatementZoneInner formula={formula} when={when} notation={notation} />
// //         </StackedRow>
// //       )}
// //       {entries.map(([key, value]) => (
// //         <StackedRow key={key} title={key.replace(/_/g, ' ')}>
...
// // // ═════════════════════════════════════════════════════════════════
// // // MAIN — header untouched. Body below rebuilt to MethodsExplorer v8:
// // //   statement zone (formula + when-to-use + notation split)
// // //   → tabs (restyled)
// // // Stacked view: same statement zone content lives inside Essentials.
// // // ═════════════════════════════════════════════════════════════════
...
// //         {/* ─── BODY (rebuilt to MethodsExplorer v8) ───────────── */}
// //         {hasBody && (
// //           <div style={{
// //             display: 'grid',
// //             gridTemplateRows: expanded ? '1fr' : '0fr',
...
// // ═════════════════════════════════════════════════════════════════
// // STATEMENT ZONE  (formula rendered via KaTeX)
// // ═════════════════════════════════════════════════════════════════
...
export default function CalculationCard({
  ...
  // #2 — When the user expands the card, write its id to the URL hash so
  // the page can be shared / bookmarked at this specific card.
  const handleToggle = () => {
    const nextExpanded = !expanded;
    if (onToggle) onToggle();
    if (expandedProp === undefined) setInternalExpanded(nextExpanded);
    if (nextExpanded && cardId && typeof window !== 'undefined') {
