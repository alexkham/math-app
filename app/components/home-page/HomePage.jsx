...
const HomePage = ({ categorizedUrls, metaDescriptions }) => {
  ...
  useEffect(() => {
    console.log('Meta Descriptions :  ' + JSON.stringify(metaDescriptions, null, 2));
  }, [metaDescriptions]);
  ...
  const getDescription = (url) => {
    const key = Object.keys(metaDescriptions).find(k => url.includes(k));
    return metaDescriptions[key] || 'No description available at the Moment';
  };

  const NewsCard = ({ url, size = 'medium' }) => {
    const title = formatTitle(url);
    console.log(`NewsCard URL: ${url}, Description: ${getDescription(url)}`);
    return (
      <div className={`${styles.newsCard} ${styles[size]}`}>
        <div className={styles.newsCardHeader}>
          <div className={styles.newsCardIcon}>
  ...
  return (
    <div className={styles.container} id='top'>
      ...
      <main className={styles.main}>
        <section className={styles.heroSection}>
          {heroUrls.map((url, index) => {
            console.log(`Hero URL: ${url}, Description: ${getDescription(url)}`);
            return (
              <div key={index} className={`${styles.heroSlide} ${index === currentSlide ? styles.activeSlide : ''}`}>
                <a href={url} className={styles.heroLink}>
                  <div className={styles.heroImageWrapper}>
