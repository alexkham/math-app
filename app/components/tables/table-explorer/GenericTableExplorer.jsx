...
function buildCsv(items, categories) {
  const header = ['Name', 'LHS', 'RHS', 'Category', 'Tip', 'Link'].map(csvEscape).join(',');
...
.ge-quiz-skip:hover { color: ${C.primary} !important; }
.ge-action-btn:not(.copied):hover {
  border-color: ${C.primary} !important;
...
const VIEW_NOTES = {
  cards:  'Cards view: click any card to expand its explanation.',
  rows:   'Rows view: dense, print-friendly listing. Click any row to expand.',
  puzzle: 'Puzzle mode: drag tiles into the matching slot.',
};
...
const GenericExplorer = ({
  ...
  // Hash-based deep-linking. Runs on mount AND on every hashchange so
  // inline anchor links such as (!/path#categoryId) inside in-page prose
  // also activate the matching filter and scroll to the table — not just
  ...
  useEffect(() => {
    ...
    const applyHashFilter = () => {
      ...
      // Empty hash → clear any filter that was driven from a hash link.
      // Direct user clicks on filter cards are unaffected because those
      // don't touch the URL.
      if (!hash) {
  ...
  const runInstantSearch = (modeId, value) => {
    ...
    if (mode.isCategoryPicker) {
      ...
      if (viewMode === 'puzzle') setViewMode('rows');
      setActiveFilter(null);
      setDetailsId(null);
  ...
  const handleCloseDetails = () => setDetailsId(null);
  ...
  const handleCopyCsv = async () => {
    try {
      const csv = buildCsv(items, categories);
  ...
  const handleDownloadCsv = () => {
    ...
    a.href = url;
    a.download = `${slugifyFilename(meta.toolTitle || meta.name || 'table')}.csv`;
  ...
  const handleQuizSkip = (e) => {
    e.preventDefault();
    const el = sectionRefs.current['sec-quiz'];
  ...
  const showBanner = (activeFilter !== null || highlightedIds.size > 0) && viewMode !== 'puzzle';
            ...
            <section id="sec-table" ref={(el) => { sectionRefs.current['sec-table'] = el; }}>
              <div style={S.legend}>
                <div style={S.legendLeft}>
                  ...
                  <p style={S.legendNote}>{VIEW_NOTES[viewMode] || ''}</p>
                </div>
                <div style={S.viewToggleColumn}>
                  <div style={S.viewToggle}>
                    {['cards', 'rows', 'puzzle'].map((v) => {
                      const isActive = viewMode === v;
                      const label = v === 'cards' ? 'Cards' : v === 'rows' ? 'Rows' : 'Puzzle';
                      return (
                        <button
                          key={v} type="button"
                  ...
                  <div style={S.actionBar}>
                    ...
                    <button
                      type="button"
                      className={`ge-action-btn ${copyState === 'copied' ? 'copied' : ''}`}
                      style={{ ...S.actionBtn, ...(copyState === 'copied' ? S.actionBtnCopied : {}) }}
                      onClick={handleCopyCsv}
                      title="Copy all entries as CSV to clipboard"
                    >
                      <span>{copyState === 'copied' ? '\u2713' : '\u2398'}</span>
                      <span>{copyState === 'copied' ? 'Copied!' : copyState === 'error' ? 'Failed' : 'Copy CSV'}</span>
                    </button>
                    <button
                      type="button"
                      className="ge-action-btn"
                  ...
                  {typeof quizGenerator === 'function' && (
                    <a
                      href="#sec-quiz"
                      className="ge-quiz-skip"
                      style={S.quizSkipLink}
                      onClick={handleQuizSkip}
                    >
                      Or skip to the quiz &rarr;
                    </a>
                  )}
                </div>
              </div>
              ...
              {viewMode === 'cards' && (
                <CardsView {...viewProps} cols={cols} styles={S} />
              )}
              ...
              <section id="sec-patterns" ref={(el) => { sectionRefs.current['sec-patterns'] = el; }}>
                ...
                {viewMode === 'puzzle' && (
                  <div style={S.filtersDisabledNote}>
                    Filters are available in <strong>Cards</strong> and <strong>Rows</strong> views.
                  </div>
                )}
