...
/* ============================================================
   BisectedApexDemo — v8
   Changes from v7:
   - V1 styling: deeper panel bg (#f1f5f9), stronger borders
     (#cbd5e1), slightly stronger card shadow.
   - All fonts scaled by ×1.2 (SVG, panel, controls, identity bar,
     metric cards, slider, step log).
   - Legend rect dimensions and row spacing bumped to accommodate
     larger fonts.
   ============================================================ */
...
function BisectedApexCore({
  ...
  return (
    <svg viewBox={viewBox} style={{ display: 'block', width: '100%' }} role="img">
      ...
      {showLegend && (
        <g transform="translate(10, 16)">
          <rect x="0" y="0" width="184" height="260" rx="6"
                fill={COLORS.panelBg} stroke={COLORS.borderTer} strokeWidth="0.5" />
          <text x="14" y="22" fontSize="12" fontWeight="600" fill={COLORS.textMuted}
                style={{ textTransform: 'uppercase', letterSpacing: '1.2px' }}>Legend</text>
          <text x="14" y="48" fontSize="14.4" fill={COLORS.text}>
            <tspan fontStyle="italic" fontWeight="500">O</tspan>
            <tspan dx="4" fill={COLORS.textMuted}>— center</tspan>
          </text>
          <g style={{ opacity: showRadii ? 1 : 0, transition: 'opacity 400ms ease' }}>
            <text x="14" y="72" fontSize="14.4" fill={COLORS.text}>
              <tspan fontStyle="italic" fontWeight="500">A, B</tspan>
              <tspan dx="4" fill={COLORS.textMuted}>— chord ends</tspan>
            </text>
            <text x="14" y="96" fontSize="14.4" fill={COLORS.text}>
              <tspan fontStyle="italic" fontWeight="500">a, b</tspan>
              <tspan dx="4" fill={COLORS.textMuted}>— sides (= 1)</tspan>
            </text>
          </g>
          <g style={{ opacity: showApex ? 1 : 0, transition: 'opacity 400ms ease' }}>
            <text x="14" y="120" fontSize="14.4" fill={COLORS.text}>
              <tspan fontStyle="italic" fontWeight="500" fill={COLORS.red}>C</tspan>
              <tspan dx="4" fill={COLORS.textMuted}>— apex (= {lbl.apex})</tspan>
            </text>
          </g>
          <g style={{ opacity: showBisector ? 1 : 0, transition: 'opacity 400ms ease' }}>
            <text x="14" y="144" fontSize="14.4" fill={COLORS.text}>
              <tspan fontStyle="italic" fontWeight="500">M</tspan>
              <tspan dx="4" fill={COLORS.textMuted}>— bisector foot</tspan>
            </text>
            <text x="14" y="168" fontSize="14.4" fill={COLORS.text}>
              <tspan fontStyle="italic" fontWeight="500" fill={COLORS.deepBlue}>{lbl.bisector}</tspan>
              <tspan dx="4" fill={COLORS.textMuted}>— OM length</tspan>
            </text>
          </g>
          <g style={{ opacity: showSubAngles ? 1 : 0, transition: 'opacity 400ms ease' }}>
            <text x="14" y="192" fontSize="14.4" fill={COLORS.text}>
              <tspan fontStyle="italic" fontWeight="500">{lbl.subAngle}</tspan>
              <tspan dx="4" fill={COLORS.textMuted}>— half of C</tspan>
            </text>
          </g>
          <g style={{ opacity: showSinLabels ? 1 : 0, transition: 'opacity 400ms ease' }}>
            <text x="14" y="216" fontSize="14.4" fill={COLORS.text}>
              <tspan fontStyle="italic" fontWeight="500" fill={COLORS.midBlue}>{lbl.halfChord}</tspan>
              <tspan dx="4" fill={COLORS.textMuted}>— half-chord</tspan>
            </text>
          </g>
        </g>
      ...
      <text x={cx - 8} y={cy + 18} textAnchor="end"
            fontSize="14.4" fill={COLORS.textMuted} fontStyle="italic">O</text>

      <FadeGroup visible={showRadii}>
        <text x={ax + 8} y={ay - 4} fontSize="14.4" fill={COLORS.textMuted} fontStyle="italic">A</text>
        <text x={bx + 8} y={by + 16} fontSize="14.4" fill={COLORS.textMuted} fontStyle="italic">B</text>
      </FadeGroup>

      <FadeGroup visible={showBisector}>
        <text x={mx + 8} y={my + 20} fontSize="14.4" fill={COLORS.textMuted} fontStyle="italic">M</text>
      </FadeGroup>
      ...
      <FadeGroup visible={showRadii}>
        <text
          ...
          y={(cy + ay) / 2 - c * 18 + 4}
          textAnchor="middle"
          fontSize="16.8" fontWeight="500" fill={COLORS.text}
        >a = 1</text>
        <text
          x={(cx + bx) / 2 - s * 18}
          y={(cy + by) / 2 + c * 18 + 4}
      ...
      <FadeGroup visible={showApex}>
        <text x={cx + arcR + 6} y={cy + 16}
              fontSize="16.8" fontWeight="500" fontStyle="italic" fill={COLORS.red}>C = {lbl.apex}</text>
      </FadeGroup>

      <FadeGroup visible={subAnglesActuallyVisible}>
        <text x={subLblX - 4} y={subLblYU + 4} fontSize="14.4" fill={COLORS.textMuted} fontStyle="italic">{lbl.subAngle}</text>
        <text x={subLblX - 4} y={subLblYD + 4} fontSize="14.4" fill={COLORS.textMuted} fontStyle="italic">{lbl.subAngle}</text>
      </FadeGroup>

      <FadeGroup visible={showBisector}>
        <text x={(cx + mx) / 2} y={cy - 10} textAnchor="middle"
              fontSize="16.8" fontWeight="500" fontStyle="italic" fill={COLORS.deepBlue}>{lbl.bisector}</text>
      </FadeGroup>

      <FadeGroup visible={showSinLabels}>
        <text x={mx + 10} y={(ay + my) / 2 + 4}
              fontSize="16.8" fontWeight="500" fontStyle="italic" fill={COLORS.midBlue}>{lbl.halfChord}</text>
        <text x={mx + 10} y={(by + my) / 2 + 4}
              fontSize="16.8" fontWeight="500" fontStyle="italic" fill={COLORS.midBlue}>{lbl.halfChord}</text>
      </FadeGroup>
    </svg>
...
function MiniSolutionPanel({ steps, tabs = [], stepsTitle = 'Derivation', placeholder, onStepClick }) {
  const [activeTab, setActiveTab] = useState('steps');
          ...
          return (
            <div
              ...
              style={{
                ...
                animation: 'bae-fade-in 0.35s ease',
                outline: 'none',
              }}
            >
              <div style={{
  ...
  return (
    <div>
      <style>{`
        ...
        .bae-step-log::-webkit-scrollbar { display: none; width: 0; height: 0; }
      `}</style>
      ...
      {showTabBar && (
        <div style={{
          ...
          {visibleTabs.map(t => (
            <button key={t.key} onClick={() => setActiveTab(t.key)} style={{
              flex: 1, padding: '10px 18px', border: 'none', borderRadius: '6px',
              fontFamily: 'inherit', fontSize: '0.984rem', fontWeight: 500, cursor: 'pointer',
              background: effective === t.key ? COLORS.white : 'transparent',
              color: effective === t.key ? COLORS.text : COLORS.textMuted,
              boxShadow: effective === t.key ? '0 1px 3px rgba(0,0,0,0.06)' : 'none',
            }}>
...
function MetricCard({ label, value, visible }) {
  return (
    <div style={{
      ...
      <p style={{
        fontSize: '0.96rem', color: COLORS.textMuted, margin: '0 0 4px', fontStyle: 'italic',
      }}>{label}</p>
      <p style={{
...
export default function BisectedApexDemo({
  ...
  const mergedScenario = {
    ...
    steps:        scenario.steps        ?? DEFAULT_SCENARIO.steps,
    metricPairs:  scenario.metricPairs  ?? DEFAULT_SCENARIO.metricPairs,
    labels:       { ...DEFAULT_LABELS, ...(scenario.labels || {}) },
  ...
  const handlePrev   = () => { setIsPlaying(false); setCurrentStep(s => Math.max(s - 1, 0)); };
  const handleReset  = () => { setIsPlaying(false); setCurrentStep(0); };
  const handleJumpTo = (visibleIdx) => {
    setIsPlaying(false);
    setCurrentStep(visibleIdx + 1);
  };
