...
// app/utils/illustrations/arithmetic/divisibility/factorSet.js
//
// String-based SVG renderer for the "factor-set" illustration component.
// v4: customizability contract — every color reference goes through `palette`,
//     merged once per render from defaults (`C`) and `spec.style` overrides.
//     v3 visual behavior preserved exactly when no `style` is passed.
...
function chip(x, y, w, h, value, fillColor, strokeColor, fillOpacity, strokeWidth, weight, struck, fontSize, palette) {
  const cx = x + w / 2;
  const cy = y + h / 2 + 4;
  const parts = [
    `<rect x="${x}" y="${y}" width="${w}" height="${h}" rx="6" ry="6" fill="${fillColor}" fill-opacity="${fillOpacity}" stroke="${strokeColor}" stroke-width="${strokeWidth}"/>`,
    `<text x="${cx}" y="${cy}" font-family="sans-serif" font-size="${fontSize}" font-weight="${weight}" fill="${palette.text}" text-anchor="middle">${value}</text>`,
  ];
  if (struck) {
    parts.push(`<line x1="${x + 4}" y1="${y + h / 2}" x2="${x + w - 4}" y2="${y + h / 2}" stroke="${palette.negation}" stroke-width="1.5"/>`);
  }
  return parts.join('');
}
...
function renderSingleSet(spec) {
  ...
  parts.push(`<text x="30" y="58" font-family="sans-serif" font-size="13" font-weight="600" fill="${palette.primary}">factors of ${n}</text>`);

  factors.forEach((f, i) => {
    const x = startX + i * (chipW + gap);
    const isHi = highlightSet.has(f);
    const isEx = excludeSet.has(f);
    const fillColor = isHi ? palette.result : (isEx ? palette.neutral : palette.primary);
    const strokeColor = isHi ? palette.resultStroke : (isEx ? palette.neutral : palette.primary);
    const fillOp = isHi ? 0.4 : (isEx ? 0.08 : 0.15);
    const sw = isHi ? 1.6 : 1.4;
    const weight = isHi ? 600 : 400;
    parts.push(chip(x, chipY, chipW, chipH, f, fillColor, strokeColor, fillOp, sw, weight, isEx, fontSize, palette));
  });

  if (caption) {
    const cx = startX + totalW / 2;
    parts.push(`<text x="${cx}" y="138" font-family="sans-serif" font-size="11" font-style="italic" fill="${palette.textMuted}" text-anchor="middle">${caption}</text>`);
  }

  parts.push(infoPanel(info?.lines, 58, palette));
...
function renderPairs(spec) {
  ...
  const labelTitle = isPerfectSquare
    ? `factors of ${n} \u2014 found by testing 1 to \u221A${n} = ${sqrtN}`
    : `factors of ${n} \u2014 found by testing 1 to \u221A${n} \u2248 ${sqrtN.toFixed(2)}`;
  parts.push(`<text x="30" y="20" font-family="sans-serif" font-size="13" font-weight="600" fill="${palette.primary}">${labelTitle}</text>`);
  ...
  arcPairs.forEach(([a, b], i) => {
    ...
    const apexY = arcN <= 1
      ...
      : APEX_TOP + (APEX_BOTTOM - APEX_TOP) * i / (arcN - 1);
    const lbl = `${a} \u00B7 ${b} = ${n}`;
    parts.push(pairArc(x1, x2, chipY - 2, apexY, palette.primary, lbl));
  });

  if (isPerfectSquare) {
    const x = cxOf(sqrtN);
    parts.push(selfPairLoop(x, chipY, palette.resultStroke));
    parts.push(`<text x="${x}" y="${chipY - 12}" font-family="sans-serif" font-size="9" fill="${palette.resultStroke}" text-anchor="middle" font-style="italic">${sqrtN} \u00B7 ${sqrtN}</text>`);
  }

  factors.forEach((f, i) => {
    const x = startX + i * (chipW + gap);
    const isSqrt = isPerfectSquare && f === sqrtN;
    const fillColor = isSqrt ? palette.result : palette.primary;
    const strokeColor = isSqrt ? palette.resultStroke : palette.primary;
    const fillOp = isSqrt ? 0.45 : 0.15;
    const sw = isSqrt ? 1.6 : 1.4;
    const weight = isSqrt ? 600 : 400;
    parts.push(chip(x, chipY, chipW, chipH, f, fillColor, strokeColor, fillOp, sw, weight, false, fontSize, palette));
  });

  if (isPerfectSquare) {
    const x = cxOf(sqrtN);
    parts.push(`<text x="${x}" y="${chipY + chipH + 16}" font-family="sans-serif" font-size="10" fill="${palette.resultStroke}" text-anchor="middle" font-weight="600">\u221A${n} = ${sqrtN}</text>`);
    parts.push(`<text x="${x}" y="${chipY + chipH + 29}" font-family="sans-serif" font-size="9" fill="${palette.textMuted}" text-anchor="middle" font-style="italic">stop testing here</text>`);
  }

  parts.push(infoPanel(info?.lines, 48, palette));
...
function renderTwoSet(spec) {
  ...
  parts.push(`<text x="30" y="48" font-family="sans-serif" font-size="13" font-weight="600" fill="${palette.primary}">factors of ${setA}</text>`);

  factorsA.forEach(f => {
    const x = slotX(f);
    const isCommon = commonSet.has(f);
    const fillColor = isCommon ? palette.result : palette.primary;
    const strokeColor = isCommon ? palette.resultStroke : palette.primary;
    const fillOp = isCommon ? 0.4 : 0.15;
    const sw = isCommon ? 1.6 : 1.4;
    const weight = isCommon ? 600 : 400;
    parts.push(chip(x, yTop, chipW, chipH, f, fillColor, strokeColor, fillOp, sw, weight, false, fontSize, palette));
  });

  common.forEach(f => {
    const x = slotCx(f);
    parts.push(`<line x1="${x}" y1="${yTop + chipH}" x2="${x}" y2="${yBot}" stroke="${palette.resultStroke}" stroke-width="0.8" stroke-dasharray="3 3" stroke-opacity="0.6"/>`);
  });

  parts.push(`<text x="30" y="135" font-family="sans-serif" font-size="13" font-weight="600" fill="${palette.secondary}">factors of ${setB}</text>`);

  factorsB.forEach(f => {
    const x = slotX(f);
    const isCommon = commonSet.has(f);
    const fillColor = isCommon ? palette.result : palette.secondary;
    const strokeColor = isCommon ? palette.resultStroke : palette.secondary;
    const fillOp = isCommon ? 0.4 : 0.15;
    const sw = isCommon ? 1.6 : 1.4;
    const weight = isCommon ? 600 : 400;
    parts.push(chip(x, yBot, chipW, chipH, f, fillColor, strokeColor, fillOp, sw, weight, false, fontSize, palette));
  });

  const gcdX = slotCx(gcdVal);
  parts.push(`<line x1="${gcdX}" y1="${yBot + chipH}" x2="${gcdX}" y2="${yBot + chipH + 16}" stroke="${palette.resultStroke}" stroke-width="1.2"/>`);
  parts.push(`<text x="${gcdX}" y="${yBot + chipH + 32}" font-family="sans-serif" font-size="12" font-weight="600" fill="${palette.resultStroke}" text-anchor="middle">gcd(${setA}, ${setB}) = ${gcdVal}</text>`);
  parts.push(`<text x="${gcdX}" y="${yBot + chipH + 46}" font-family="sans-serif" font-size="9" fill="${palette.textMuted}" text-anchor="middle" font-style="italic">largest common factor</text>`);
