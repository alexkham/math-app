...
export const processContent = (content) => {
 ...
 const processPart = (part, index) => {
   if (part.startsWith('__SVG_PLACEHOLDER_')) {
     ...
     const mathHtml = katex.renderToString(part.slice(2, -2), {
       throwOnError: false,
       displayMode: true
     });
     return <div key={`block-math-${index}`} dangerouslySetInnerHTML={{ __html: mathHtml }} />;
   } else if (part.startsWith('$') && part.endsWith('$')) {
     const mathHtml = katex.renderToString(part.slice(1, -1), {
       throwOnError: false,
       displayMode: false
     });
     return <span key={`inline-math-${index}`} dangerouslySetInnerHTML={{ __html: mathHtml }} />;
   } else if (part.startsWith('[') && part.includes('](') && part.endsWith(')')) {
     const linkMatch = part.match(/\[(.+?)\]\((.+?)\)/);
     if (linkMatch) {
       const [, text, url] = linkMatch;  
